...
use ir::error::error_into_token_stream;
use ir::generate::autotune::generate_autotune_key;
use ir::parse::{
    ruda_impl::RudaImpl,
    ruda_trait::{RudaTrait, RudaTraitImpl},
...
use crate::ir::{
    ...
    parse::{
        ruda_type::generate_ruda_type, derive_expand::generate_derive_expand,
...
pub fn ruda(args: TokenStream, input: TokenStream) -> TokenStream {
    match ruda_impl(args, input.clone()) {
        Ok(tokens) => tokens,
        Err(e) => error_into_token_stream(e, input.into()).into(),
    }
}

fn ruda_impl(args: TokenStream, input: TokenStream) -> syn::Result<TokenStream> {
    ...
    let tokens = match item.clone() {
        ...
        Item::Trait(kernel_trait) => {
            ...
            let expand_trait = RudaTrait::from_item_trait(kernel_trait, args)?;
        ...
        Item::Impl(item_impl) => {
            if item_impl.trait_.is_some() {
                let mut expand_impl = RudaTraitImpl::from_item_impl(item_impl, &args)?;
                ...
                let mut expand_impl = RudaImpl::from_item_impl(item_impl, &args)?;
                let expand_impl = expand_impl.to_tokens_mut();
        ...
        item => Err(syn::Error::new_spanned(
            item,
            "`#[ruda]` is only supported on traits and functions",
        ))?,
    };
...
#[proc_macro_derive(RudaLaunch, attributes(ruda, launch))]
pub fn module_derive_ruda_launch(input: TokenStream) -> TokenStream {
    gen_ruda_type(input, true)
}

/// Derive macro to define a ruda type that is not launched
#[proc_macro_derive(RudaType, attributes(ruda))]
pub fn module_derive_ruda_type(input: TokenStream) -> TokenStream {
    gen_ruda_type(input, false)
}

fn gen_ruda_type(input: TokenStream, with_launch: bool) -> TokenStream {
    ...
    match generate_ruda_type(input, with_launch) {
        Ok(val) => val.into(),
...
#[proc_macro_attribute]
pub fn derive_ruda_comptime(_metadata: TokenStream, input: TokenStream) -> TokenStream {
...
pub fn intrinsic(_input: TokenStream) -> TokenStream {
    let core = ir::paths::core_path();
    quote![{ #core::unexpanded!() }].into()
}

/// Makes the function return a compile time value
/// Useful in a ruda trait to have a part of the trait return comptime values
///
/// # Example
/// ```ignored
/// #use ruda_kernel_macros::ruda;
...
#[proc_macro_derive(IntoRuntime, attributes(ruda))]
...
#[proc_macro_derive(RudaTypeMut, attributes(ruda))]
pub fn derive_assign(input: TokenStream) -> TokenStream {
    let input = syn::parse(input).unwrap();
    match generate_ruda_type_mut(&input) {
        Ok(tokens) => tokens.into(),
        Err(e) => e.into_compile_error().into(),
    }
}
