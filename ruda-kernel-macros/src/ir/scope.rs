...
use crate::ir::parse::kernel::KernelParam;
...
mod tests {
    ...
    #[test]
    fn plane_count_is_a_runtime_builtin() {
        let context = Context::new(parse_quote!(()), false);
        let plane_count = context.variable(&format_ident!("PLANE_COUNT")).unwrap();
        assert!(plane_count.is_keyword);
        assert!(!plane_count.is_const);
        assert_eq!(plane_count.ty, Some(parse_quote!(u32)));
    }
}
