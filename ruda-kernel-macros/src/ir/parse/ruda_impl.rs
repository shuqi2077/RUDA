...
impl RudaImplItem {
        ...
        let res = match item {
            ImplItem::Fn(func) => {
                ...
                if is_method {
                    ...
                    vec![
                        RudaImplItem::Fn(func),
                        RudaImplItem::MethodExpand(method),
                        RudaImplItem::FnExpand(func_expand),
                    ]
                } else {
                    func.sig.name = func_name_expand;

                    let func_expand = Self::create_func_expand(struct_ty_name, &func, false, args);
                    vec![RudaImplItem::Fn(func), RudaImplItem::FnExpand(func_expand)]
                }
            }
            _ => vec![RudaImplItem::Other],
        };
    ...
    pub fn as_func(&mut self) -> Option<&mut KernelFn> {
        match self {
            RudaImplItem::Fn(func) => Some(func),
            _ => None,
        }
    }
    ...
    pub fn as_func_expand(&mut self) -> Option<&mut KernelFn> {
        match self {
            RudaImplItem::FnExpand(func) => Some(func),
            _ => None,
        }
    }
    ...
    pub fn as_method_expand(&mut self) -> Option<&mut KernelFn> {
        match self {
            RudaImplItem::MethodExpand(func) => Some(func),
            _ => None,
        }
    }
...
impl RudaImpl {
    pub fn from_item_impl(mut item_impl: ItemImpl, args: &KernelArgs) -> syn::Result<Self> {
        ...
        let items = item_impl
            ...
            .iter()
            .cloned()
            .map(|item| RudaImplItem::from_impl_item(&struct_name, item, args))
            .flat_map(|items| {
                let result: Vec<syn::Result<RudaImplItem>> = match items {
