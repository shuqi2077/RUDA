...
impl RudaTypeEnum {
    pub fn generate(&self, with_launch: bool) -> TokenStream {
        if self.runtime_variants {
            ...
            let ruda_type_impl = self.ruda_type_impl();
    ...
    pub(crate) fn expand_ty(&self) -> proc_macro2::TokenStream {
        ...
        let variants = self.variants.iter().map(RudaTypeVariant::expand_variant);
    ...
    fn ruda_type_impl(&self) -> proc_macro2::TokenStream {
...
impl RudaTypeVariant {
    fn expand_variant(&self) -> TokenStream {
        ...
        let fields = self.fields.iter().map(|field| {
            ...
            match &field.ident {
                Some(name) => {
                    quote! {#name: <#ty as #ruda_type>::ExpandType}
                }
                None => quote! {<#ty as #ruda_type>::ExpandType},
            }
        });
        ...
        match self.kind {
            VariantKind::Named => {
                ...
                let args_with_expand_types = self.fields.iter().map(|f| {
                    ...
                    let name = &f.ident;
                    quote! {
                        #name: <#ty as #ruda_type>::ExpandType
                    }
                });
