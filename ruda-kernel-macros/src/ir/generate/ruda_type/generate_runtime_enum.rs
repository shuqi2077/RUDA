...
use crate::ir::{
    parse::ruda_type::{RudaTypeEnum, RudaTypeVariant, VariantKind},
    paths::{core_path, frontend_type, prelude_type},
};
...
impl RudaTypeEnum {
    pub fn generate_runtime(&self, with_launch: bool) -> TokenStream {
        ...
        if with_launch {
            ...
            let ruda_type_impl = self.ruda_type_impl_runtime();
    ...
    fn expand_value_ty(&self) -> proc_macro2::TokenStream {
        let ruda_enum = prelude_type("RudaEnum");
        ...
        quote! {
            #vis struct #name_expand #generics #where_clause {
                ...
                value: <#name_expand #generic_names as #ruda_enum>::RuntimeValue,
            }
        }
    }
    ...
    fn expand_type_impl_runtime(&self) -> proc_macro2::TokenStream {
        ...
        let debug = prelude_type("RudaDebug");
    ...
    fn ruda_type_impl_runtime(&self) -> proc_macro2::TokenStream {
        ...
        quote! {
            impl #generics #ruda_type for #name #generic_names #where_clause {
                type ExpandType = #name_expand #generic_names;
            }
            impl #generics #ruda_enum for #name_expand #generic_names #where_clause {
                type RuntimeValue = <#value_ty as #ruda_type>::ExpandType;
                    ...
                    match self {
                        Self::Comptime { discriminant, value } => {
                            f.debug_struct(stringify!(#name))
                                .field(stringify!(discriminant), &#core::format::DebugRaw(#body_debug_discriminant))
    ...
    fn launch_arg_impl_runtime(&self) -> proc_macro2::TokenStream {
        ...
        quote! {
            impl #generics #launch_arg for #name #generic_names #where_clause {
                ...
                fn expand(arg: &Self::CompilationArg, builder: &mut #kernel_builder) -> <Self as #ruda_type>::ExpandType {
                    match arg {
        ...
        match self.kind {
            ...
            VariantKind::Unnamed => {
                ...
                quote! {
                    ...
                    pub fn #expand_function(_: &mut #scope, value: <#ty as #ruda_type>::ExpandType) -> #ident_ty_expand #generics {
