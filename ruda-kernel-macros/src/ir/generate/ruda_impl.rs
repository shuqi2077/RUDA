use crate::ir::parse::ruda_impl::{RudaImpl, RudaImplItem};
...
impl RudaImpl {
    pub fn to_tokens_mut(&mut self) -> TokenStream {
        ...
        let fns = &self
            .items
            .iter_mut()
            .filter_map(RudaImplItem::as_func)
        ...
        let methods = &self
            .items
            .iter_mut()
            .filter_map(RudaImplItem::as_method_expand)
            .map(|it| it.to_tokens_mut())
            .collect::<Vec<_>>();
        ...
        let methods_tokens = if !methods.is_empty() {
            let fns_expand = if self.expand_only {
                ...
                &self
                    .items
                    .iter_mut()
                    .filter_map(RudaImplItem::as_func_expand)
                    .map(|it| it.to_tokens_mut())
                    .collect::<Vec<_>>()
            };
