#!/usr/bin/env python3
"""Compile and run the EXACT production batch planner with rustc, without Cargo/GPU.
...
def main() -> int:
    ...
    parser.add_argument('--output', type=Path, default=Path('v19-rust-host'))
    ...
    source = ROOT/'ruda-driver-cuda/src/execution/graph_batch.rs'
    binary = out/('graph_batch_tests.exe' if os.name=='nt' else 'graph_batch_tests')
    try:
        for label, command in [('compile', [compiler,'--edition=2024','--test',str(source),'-o',str(binary)]),
            ...
            if label == 'compile': report['production_module_compiled'] = True
                ...
                if not re.search(r'test result: ok\. 14 passed; 0 failed; 0 ignored;', text):
                    raise RuntimeError('missing complete 14-test Rust result')
