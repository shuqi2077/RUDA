...
use crate::runtime::{
    config::{
        RudaRuntimeConfig, RuntimeConfig,
...
use alloc::vec;
use alloc::vec::Vec;
use ruda_core::{backtrace::BackTrace, stub::Arc};
...
impl<Storage: ComputeStorage> MemoryManagement<Storage> {
    ...
    pub fn from_configuration(
        ...
        let config = RudaRuntimeConfig::get().memory.persistent_memory.clone();
    ...
    pub fn mode(&mut self, mode: MemoryAllocationMode) {
        // We override the mode based on the ruda config.
...
mod tests {
    use super::*;
    use crate::runtime::{memory_management::MemoryManagement, storage::BytesStorage};
    use alloc::vec;
