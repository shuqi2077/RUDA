...
/// Configuration for memory settings in `Ruda`.
...
/// Configuration options for persistent memory pools in `Ruda` runtimes.
#[derive(Clone, Debug, serde::Serialize, serde::Deserialize, Default)]
...
/// Log levels for memory-related events in `Ruda`.
