...
use ruda_core::{bytes::Bytes, future::DynFut, profile::ProfileDuration, stream_id::StreamId};
use ruda_core::ir::{
...
use ruda::runtime::{
    ...
    compiler::{CompilationError, RudaTask},
    id::KernelId,
    ...
    server::{
        Binding, ComputeServer, CopyDescriptor, RudaCount, RudaDim, ExecutionMode, Handle,
...
/// The dummy server is used to test the ruda infrastructure.
...
impl KernelMetadata for KernelTask {
    ...
    fn address_type(&self) -> ruda_core::ir::StorageType {
...
impl RudaTask<DummyCompiler> for KernelTask {
    fn compile(
        &self,
        _compiler: &mut DummyCompiler,
        _compilation_options: &<DummyCompiler as ruda::runtime::compiler::Compiler>::CompilationOptions,
        _mode: ExecutionMode,
        _addr_type: StorageType,
    ) -> Result<ruda::runtime::kernel::CompiledKernel<DummyCompiler>, CompilationError> {
        Ok(CompiledKernel {
...
impl ComputeServer for DummyServer {
    type Kernel = Box<dyn RudaTask<DummyCompiler>>;
    ...
    fn utilities(&self) -> Arc<ruda::runtime::server::ServerUtilities<Self>> {
...
impl DummyServer {
    pub fn new(
        ...
        let hardware = HardwareProperties {
            ...
            max_ruda_count: (u16::MAX as u32, u16::MAX as u32, u16::MAX as u32),
            max_units_per_ruda: 1024,
            max_ruda_dim: (1024, 1024, 64),
        ...
        let timing_method = ruda_core::profile::TimingMethod::System;
