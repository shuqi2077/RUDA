...
fn main() {
    ...
    let mut store = RudapackStore::from_file(path_f32.to_str().unwrap()).overwrite(true);
    ...
    let mut store = RudapackStore::from_file(path_f16.to_str().unwrap())
        .overwrite(true)
    ...
    let mut store = RudapackStore::from_file(path_mixed.to_str().unwrap())
        .overwrite(true)
    ...
    let mut load_store =
        RudapackStore::from_file(path_f16.to_str().unwrap()).with_from_adapter(adapter);
