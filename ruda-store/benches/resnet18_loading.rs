...
fn resnet18_path() -> PathBuf {
    ...
    let config_file = temp_dir.join("ruda_resnet18_benchmark").join("path.txt");
    ...
    bencher.bench_local(|| {
        ...
        for key in &keys {
            ...
            let sum = match data.dtype {
                ruda_tensor::api::DType::F32 => data
                    .as_slice::<f32>()
                    .map(|s| s.iter().sum::<f32>())
                    .unwrap_or(0.0) as f64,
                ruda_tensor::api::DType::F64 => data
                    .as_slice::<f64>()
