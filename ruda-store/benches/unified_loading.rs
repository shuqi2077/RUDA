...
//! - RudapackStore (new native format)
...
use ruda_model::record::{FullPrecisionSettings, NamedMpkFileRecorder, Recorder};
// use ruda_io::pytorch::{LoadArgs, PyTorchFileRecorder};
// use ruda_io::safetensors::SafetensorsFileRecorder;
use ruda_nn as nn;
use ruda_store::{
    RudapackStore, ModuleSnapshot, PyTorchToRudaAdapter, PytorchStore, SafetensorsStore,
};
...
/// Generate Rudapack and NamedMpk files from existing SafeTensors file
fn generate_ruda_formats(st_path: &Path, bp_path: &Path, mpk_path: &Path) {
    ...
    // Save as Rudapack
    if !bp_path.exists() {
        println!("  Creating Rudapack file...");
...
fn check_model_files() -> Result<(), String> {
    ...
    // For now, only check safetensors and pytorch files (will generate rudapack/mpk later)
...
fn main() {
    ...
    match check_model_files() {
        Ok(()) => {
            ...
            // First, generate Rudapack and MPK files if they don't exist
            if !bp_path.exists() || !mpk_path.exists() {
                println!("⏳ Generating Rudapack and NamedMpk files from SafeTensors...");
                generate_ruda_formats(&st_path, &bp_path, &mpk_path);
            }
            ...
            if let Some(size) = bp_size {
                println!("  Rudapack: {} ({:.1} MB)", bp_path.display(), size);
            }
            ...
            println!("  1. RudapackStore (new native format - lazy loading)");
...
macro_rules! bench_backend {
    ($backend:ty, $mod_name:ident, $backend_name:literal) => {
        ...
        mod $mod_name {
            ...
            #[divan::bench]
            fn rudapack_store(bencher: Bencher) {
                ...
                bencher
                    ...
                    .bench(|| {
                        ...
                        let mut model = LargeModel::<TestBackend>::new(&device);
                        let mut store = RudapackStore::from_file(bp_path.clone());
