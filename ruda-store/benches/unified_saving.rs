...
use ruda_model::record::{FullPrecisionSettings, NamedMpkFileRecorder};
use ruda_nn as nn;
use ruda_store::{RudapackStore, ModuleSnapshot, SafetensorsStore};
...
fn main() {
    match ensure_output_dir() {
        Ok(()) => {
            ...
            println!("  1. RudapackStore (new native format)");
        ...
        mod $mod_name {
            ...
            fn rudapack_store(bencher: Bencher) {
                bencher.bench(|| {
                    ...
                    let output_path = get_output_dir().join("test_rudapack.bpk");
                    let mut store = RudapackStore::from_file(output_path.clone()).overwrite(true);
                    model
                        .save_into(&mut store)
                        .expect("Failed to save with RudapackStore");
                    // Clean up
