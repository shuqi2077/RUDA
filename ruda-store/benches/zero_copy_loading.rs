...
//! Benchmark comparing zero-copy vs copy loading modes for RudapackStore.
//!
...
use ruda_model::prelude::*;
use ruda_nn as nn;
use ruda_store::{
    RudapackStore, ModuleSnapshot, ModuleStore, PyTorchToRudaAdapter, SafetensorsStore,
};
use ruda_tensor::api::{AllocationProperty, Bytes};
...
type FlexBackend = ruda_tensor_host::Host;

#[cfg(feature = "wgpu")]
type WgpuBackend = ruda_tensor_wgpu::Wgpu;

#[cfg(feature = "cuda")]
type CudaBackend = ruda_tensor_device::cuda::Cuda<f32, i32>;

#[cfg(feature = "tch")]
type TchBackend = ruda_tensor_tch::LibTorch<f32>;

#[cfg(feature = "metal")]
type MetalBackend = ruda_tensor_wgpu::Metal;
...
/// Get path to Rudapack model file
fn get_rudapack_path() -> PathBuf {
    get_model_dir().join("large_model.bpk")
}

/// Generate Rudapack file from existing SafeTensors file if needed
fn ensure_rudapack_file() {
    let bp_path = get_rudapack_path();
    ...
    println!("⏳ Generating Rudapack file from SafeTensors...");
    ...
    let mut store = SafetensorsStore::from_file(&st_path).with_from_adapter(PyTorchToRudaAdapter);
    model
        .load_from(&mut store)
        .expect("Failed to load from SafeTensors");

    // Save as Rudapack
    let mut rudapack_store = RudapackStore::from_file(&bp_path);
    model
        .save_into(&mut rudapack_store)
        .expect("Failed to save as Rudapack");

    println!("✅ Created Rudapack file: {}", bp_path.display());
}
...
fn get_static_model_bytes() -> &'static [u8] {
    STATIC_MODEL_BYTES.get_or_init(|| {
        let bp_path = get_rudapack_path();
        let bytes = fs::read(&bp_path).expect("Failed to read Rudapack file");
...
fn main() {
    // Ensure Rudapack file exists
    ensure_rudapack_file();

    let bp_path = get_rudapack_path();
    let file_size = fs::metadata(&bp_path).unwrap().len() as f64 / 1_048_576.0;

    println!("✅ Found Rudapack model file:");
            ...
            fn file_copy(bencher: Bencher) {
                ...
                bencher
                    ...
                    .bench(|| {
                        ...
                        let mut model = LargeModel::<TestBackend>::new(&device);
                        let mut store = RudapackStore::from_file(&bp_path).zero_copy(false);
            ...
            fn file_zero_copy(bencher: Bencher) {
                ...
                bencher
                    ...
                    .bench(|| {
                        let device: TestDevice = Default::default();
                        let mut model = LargeModel::<TestBackend>::new(&device);
                        let mut store = RudapackStore::from_file(&bp_path).zero_copy(true);
                        model.load_from(&mut store).expect("Failed to load");
                    });
            }
            ...
            fn static_copy(bencher: Bencher) {
                ...
                bencher
                    ...
                    .bench(|| {
                        ...
                        let mut store = RudapackStore::from_bytes(Some(bytes)).zero_copy(false);
                        model.load_from(&mut store).expect("Failed to load");
                    });
            }
            ...
            fn static_zero_copy(bencher: Bencher) {
                ...
                bencher
                    ...
                    .bench(|| {
                        ...
                        let mut store = RudapackStore::from_static(static_bytes);
                        model.load_from(&mut store).expect("Failed to load");
                    });
            }
            ...
            fn memory_shared_zero_copy(bencher: Bencher) {
                ...
                bencher
                    ...
                    .bench(|| {
                        ...
                        let mut store = RudapackStore::from_bytes(Some(bytes)).zero_copy(true);
                        model.load_from(&mut store).expect("Failed to load");
                    });
            }
        }
...
mod verification {
    ...
    fn verify_copy_vs_zero_copy_equality() {
        ...
        let mut store_zc = RudapackStore::from_static(static_bytes);
        model_zc
        ...
        let bytes = Bytes::from_bytes_vec(static_bytes.to_vec());
        let mut store_copy = RudapackStore::from_bytes(Some(bytes)).zero_copy(false);
        model_copy
