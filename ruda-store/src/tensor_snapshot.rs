...
use ruda_tensor::api::quantization::{QPARAM_ALIGN, QuantParam, params_shape};
use ruda_tensor::api::{Bool, DType, Int, Shape, Tensor, TensorData, backend::Backend};
use half::f16;

/// Returns the byte size of a quantization parameter type.
// TODO: Add `size_bytes()` method to `QuantParam` in ruda and use it here.
...
pub struct TensorSnapshot {
    ...
    pub dtype: ruda_tensor::api::DType,
...
mod tests {
    ...
    use alloc::string::ToString;
    use ruda_tensor::api::{BoolStore, DType, shape};
