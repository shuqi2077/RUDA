...
//! # Ruda Store
//!
//! Advanced model storage and serialization infrastructure for the Ruda deep learning framework.
//!
//! This crate provides comprehensive functionality for storing and loading Ruda modules
...
//! ## Key Features
//!
//! - **Rudapack Format**: Native Ruda format with CBOR metadata, ParamId persistence for stateful training, and no-std support
//! - **SafeTensors Format**: Industry-standard format for secure and efficient tensor serialization
//! - **PyTorch Compatibility**: Load PyTorch models directly into Ruda with automatic weight transformation
...
//!
//! ```rust,ignore
//! use ruda_store::{ModuleSnapshot, SafetensorsStore};
//!
//! // Save a model
...
//!
//! ```rust,ignore
//! use ruda_store::PytorchStore;
//!
//! // Load PyTorch model (automatic weight transformation via PyTorchToRudaAdapter)
...
//! ## Core Components
//!
//! - [`ModuleSnapshot`]: Extension trait for Ruda modules providing `collect()` and `apply()` methods
//! - [`RudapackStore`]: Native Ruda format with ParamId persistence for stateful training workflows
...
pub use adapter::{
    RudaToPyTorchAdapter, ChainAdapter, HalfPrecisionAdapter, IdentityAdapter, ModuleAdapter,
    PyTorchToRudaAdapter,
};
...
#[cfg(feature = "rudapack")]
mod rudapack;
#[cfg(feature = "rudapack")]
pub use rudapack::writer::RudapackWriter;
#[cfg(feature = "rudapack")]
pub use rudapack::{base::RudapackError, store::RudapackStore};
