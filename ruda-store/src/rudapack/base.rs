//! Core types and constants for the Rudapack file format.
//!
//! See the [parent module](crate::rudapack) for the complete file format specification.
...
/// Magic number identifying a Rudapack file: 0x4255524E
...
/// Header structure for Rudapack files
#[derive(Debug, Clone, Copy)]
pub struct RudapackHeader {
    /// Magic number (4 bytes): 0x4255524E
    pub magic: u32,
...
impl RudapackHeader {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, RudapackError> {
...
/// Error types for Rudapack operations
#[derive(Debug)]
pub enum RudapackError {
    InvalidHeader,
    InvalidMagicNumber,
    InvalidVersion,
...
impl core::fmt::Display for RudapackError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            RudapackError::InvalidHeader => write!(f, "Invalid header: insufficient bytes"),
            RudapackError::InvalidMagicNumber => write!(f, "Invalid magic number"),
            RudapackError::InvalidVersion => write!(f, "Unsupported version"),
            RudapackError::MetadataSerializationError(e) => {
                write!(f, "Metadata serialization error: {}", e)
            }
            RudapackError::MetadataDeserializationError(e) => {
                write!(f, "Metadata deserialization error: {}", e)
            }
            RudapackError::IoError(e) => write!(f, "I/O error: {}", e),
            RudapackError::TensorNotFound(name) => write!(f, "Tensor not found: {}", name),
            RudapackError::TensorBytesSizeMismatch(e) => {
                write!(f, "Tensor bytes size mismatch: {}", e)
            }
            RudapackError::ValidationError(e) => write!(f, "Validation error: {}", e),
        }
    }
}

impl core::error::Error for RudapackError {}
