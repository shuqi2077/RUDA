use crate::rudapack::{
    base::{
        RudapackError, FORMAT_VERSION, HEADER_SIZE, MAGIC_NUMBER, magic_range, metadata_size_range,
        version_range,
    },
    reader::RudapackReader,
    writer::RudapackWriter,
};

use super::*;
use ruda_tensor::api::{BoolStore, Bytes, DType, TensorData, shape};

#[test]
fn test_reader_from_bytes_empty() {
    // Create empty rudapack data
    let writer = RudapackWriter::new(Vec::new());
    let bytes = writer.to_bytes().unwrap();

    // Read it back
    let reader = RudapackReader::from_bytes(bytes).unwrap();
...
fn test_reader_from_bytes_with_data() {
    ...
    let writer = RudapackWriter::new(vec![snapshot]).with_metadata("test", "value");
...
fn test_reader_invalid_magic_number() {
    ...
    let result = RudapackReader::from_bytes(Bytes::from_bytes_vec(bytes));
    assert!(matches!(result, Err(RudapackError::InvalidMagicNumber)));
}
...
fn test_reader_invalid_version() {
    ...
    let result = RudapackReader::from_bytes(Bytes::from_bytes_vec(bytes));
    assert!(matches!(result, Err(RudapackError::InvalidVersion)));
}
...
fn test_reader_header_too_short() {
    ...
    let result = RudapackReader::from_bytes(Bytes::from_bytes_vec(bytes));
    assert!(matches!(result, Err(RudapackError::InvalidHeader)));
}
...
fn test_reader_get_tensor_not_found() {
    ...
    assert!(matches!(result, Err(RudapackError::TensorNotFound(_))));
}
...
fn test_reader_get_tensor_snapshot() {
    ...
    let writer = RudapackWriter::new(vec![snapshot]);
    let writer_bytes = writer.to_bytes().unwrap();
    let reader = RudapackReader::from_bytes(writer_bytes).unwrap();
...
fn test_reader_multiple_tensors() {
    ...
    let writer = RudapackWriter::new(snapshots);
    let bytes = writer.to_bytes().unwrap();
    let reader = RudapackReader::from_bytes(bytes).unwrap();
...
fn test_reader_from_file() {
    ...
    let writer = RudapackWriter::new(vec![snapshot]).with_metadata("from_file_test", "true");

    writer.write_to_file(&file_path).unwrap();

    // Read from file
    let reader = RudapackReader::from_file(&file_path).unwrap();
...
fn test_reader_from_file_mmap() {
    ...
    // Read using mmap
    let reader = RudapackReader::from_file_mmap(&file_path).unwrap();
...
fn test_reader_from_file_buffered() {
    ...
    let reader = RudapackReader::from_file_buffered(&file_path).unwrap();
...
fn test_reader_data_offsets_validation() {
    ...
    let writer = RudapackWriter::new(vec![snapshot1, snapshot2]);
    let bytes = writer.to_bytes().unwrap();
    let reader = RudapackReader::from_bytes(bytes).unwrap();
...
fn test_reader_out_of_bounds_error() {
    use crate::rudapack::reader::StorageBackend;
    use alloc::rc::Rc;
...
fn test_reader_corrupted_shape_returns_error() {
    ...
    {
        use crate::rudapack::base::{RudapackMetadata, TensorDescriptor};
        ...
        let backend = crate::rudapack::reader::StorageBackend::Memory(Rc::new(data));

        let reader = RudapackReader {
            metadata,
            storage: backend,
            data_offset: 0,
        };
        ...
        assert!(result.is_err());
        let err = result.unwrap_err();
        assert!(matches!(err, RudapackError::ValidationError(_)));
        assert!(
...
fn test_reader_truncated_file_from_bytes() {
    // Create a valid rudapack with tensor data
    ...
    let result = RudapackReader::from_bytes(truncated_bytes);
    assert!(result.is_err());
...
fn test_reader_truncated_file_from_file() {
    ...
    // Create a valid rudapack file with tensor data
    ...
    let result = RudapackReader::from_file(&file_path);
    assert!(result.is_err());
    if let Err(err) = result {
        assert!(matches!(err, RudapackError::ValidationError(_)));
        assert!(err.to_string().contains("File truncated"));
...
fn test_reader_file_size_exactly_correct() {
    ...
    let reader = RudapackReader::from_bytes(bytes);
    assert!(reader.is_ok());
