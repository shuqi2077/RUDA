use crate::rudapack::{reader::RudapackReader, writer::RudapackWriter};
    ...
    F: FnOnce(&mut Vec<TensorSnapshot>, &mut BTreeMap<String, String>),
{
    ...
    let reader = RudapackReader::from_bytes(bytes.clone()).unwrap();
    ...
    let mut writer2 = RudapackWriter::new(snapshots2);
...
fn test_round_trip_file_io() {
    ...
    use crate::rudapack::writer::RudapackWriter;
    ...
    let writer = RudapackWriter::new(vec![snapshot]).with_metadata("test", "round_trip");
...
fn test_param_id_backward_compatibility() {
    ...
    // Create metadata without param_id (simulating old rudapack format)
    ...
    // Create a complete rudapack with header and data
    use crate::rudapack::base::{RudapackHeader, FORMAT_VERSION, MAGIC_NUMBER};

    let metadata_size = metadata_bytes.len() as u32;
    let header = RudapackHeader {
        magic: MAGIC_NUMBER,
    ...
    // Read the old format rudapack
    let reader =
        RudapackReader::from_bytes(ruda_tensor::api::Bytes::from_bytes_vec(full_bytes)).unwrap();
