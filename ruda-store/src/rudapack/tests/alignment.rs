//! Tests for tensor data alignment in rudapack format.
//!
...
use crate::rudapack::{
    base::{
        RudapackHeader, RudapackMetadata, HEADER_SIZE, TENSOR_ALIGNMENT, aligned_data_section_start,
    },
...
fn test_data_section_alignment() {
    ...
    let header = RudapackHeader::from_bytes(&file_bytes[..HEADER_SIZE]).unwrap();
...
fn test_round_trip_maintains_alignment() {
    ...
    // Read back
    let reader = RudapackReader::from_bytes(file_bytes.clone()).unwrap();
...
fn test_tensor_relative_offsets_are_aligned() {
    ...
    let writer = RudapackWriter::new(tensors);
    let file_bytes = writer.to_bytes().unwrap();
