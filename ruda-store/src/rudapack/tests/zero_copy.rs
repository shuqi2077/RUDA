...
use ruda_tensor::api::{AllocationProperty, Bytes, Tensor, backend::Backend};
...
fn test_from_static_enables_zero_copy() {
    ...
    let mut load_store = RudapackStore::from_static(static_bytes);
...
fn test_zero_copy_builder_method() {
    ...
    let ruda_bytes = Bytes::from_shared(shared, AllocationProperty::Other);

    // Create store with zero_copy enabled
    let mut load_store = RudapackStore::from_bytes(Some(ruda_bytes)).zero_copy(true);
...
fn test_zero_copy_disabled_uses_copy() {
    ...
    let mut load_store = RudapackStore::from_static(static_bytes).zero_copy(false);
...
fn test_storage_backend_slice_bytes() {
    ...
    let reader = RudapackReader::from_bytes(ruda_bytes).unwrap();
...
fn test_zero_copy_file_based_works() {
    ...
    let mut save_store = RudapackStore::from_file(path).overwrite(true);
    save_store.collect_from(&module).unwrap();

    // Load with zero_copy=true - should work because mmap is converted to bytes::Bytes
    let mut load_store = RudapackStore::from_file(path).zero_copy(true);
    let mut loaded_module = SimpleModule::<TestBackend>::new_zeros(&device);
