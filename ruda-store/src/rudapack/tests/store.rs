...
fn test_store_from_bytes_round_trip() {
    ...
    // Save to bytes
    let mut save_store = RudapackStore::from_bytes(None);
    save_store.collect_from(&module).unwrap();
    let bytes = save_store.get_bytes().unwrap();

    // Load from bytes
    let mut load_store = RudapackStore::from_bytes(Some(bytes));
...
fn test_store_with_path_filter() {
    ...
    let mut load_store = RudapackStore::from_bytes(Some(bytes)).with_regex("^(weight|bias)$");
...
fn test_store_allow_partial() {
    ...
    let filter = PathFilter::new()
        ...
        .with_full_path("bias");
    let mut save_store = RudapackStore::from_bytes(None).with_filter(filter);
    save_store.collect_from(&module).unwrap();
    let bytes = save_store.get_bytes().unwrap();

    // Load with allow_partial
    let mut load_store = RudapackStore::from_bytes(Some(bytes)).allow_partial(true);
...
fn test_store_match_all() {
    ...
    let mut save_store = RudapackStore::from_bytes(None).match_all();
    save_store.collect_from(&module).unwrap();
    let bytes = save_store.get_bytes().unwrap();
...
fn test_store_clear_metadata() {
    ...
    let mut save_store = RudapackStore::from_bytes(None).clear_metadata();
    save_store.collect_from(&module).unwrap();
    let bytes = save_store.get_bytes().unwrap();
...
fn test_store_validate_disabled() {
    ...
    let mut load_store = RudapackStore::from_bytes(Some(bytes)).validate(false);
    let mut module2 = TestModule::<TestBackend>::new_zeros(&device);
    let result = load_store.apply_to(&mut module2).unwrap();
...
fn test_store_allow_partial_missing_tensors() {
    ...
    let mut load_store = RudapackStore::from_bytes(Some(bytes.clone()));
    let mut module2 = TestModule::<TestBackend>::new_zeros(&device);
...
fn test_store_file_round_trip() {
    ...
    // Save to file
    let mut save_store = RudapackStore::from_file(&path).metadata("test", "value");
    save_store.collect_from(&module).unwrap();
    ...
    // Load from file
    let mut load_store = RudapackStore::from_file(&path);
    let mut module2 = TestModule::<TestBackend>::new_zeros(&device);
    let result = load_store.apply_to(&mut module2).unwrap();
...
fn test_store_overwrite_protection() {
    ...
    let mut save_store = RudapackStore::from_file(&path);
    save_store.collect_from(&module).unwrap();
    assert!(path.exists());

    // Second save without overwrite flag - should fail
    let mut save_store2 = RudapackStore::from_file(&path);
    ...
    let mut save_store3 = RudapackStore::from_file(&path).overwrite(true);
...
fn test_store_auto_extension_with_custom_extension() {
    ...
    assert!(path.exists());
    let rudapack_path = temp_dir.path().join("model.mpk.bpk");
    assert!(!rudapack_path.exists());
...
fn test_store_auto_extension_disabled() {
    ...
    let mut save_store = RudapackStore::from_file(&path).auto_extension(false);
    save_store.collect_from(&module).unwrap();

    // Verify that "model" (without extension) was created
    assert!(path.exists());
    let rudapack_path = temp_dir.path().join("model.bpk");
    assert!(!rudapack_path.exists());

    // Load with auto_extension disabled
    let mut load_store = RudapackStore::from_file(&path).auto_extension(false);
    let mut module2 = TestModule::<TestBackend>::new_zeros(&device);
    let result = load_store.apply_to(&mut module2).unwrap();
    assert!(result.is_success());
}
...
fn test_partial_loading_preserves_lazy_initialization() {
    ...
    let mut load_store = RudapackStore::from_file(&path).filter(filter);
...
struct ForwardTestModel<B: Backend> {
    linear1: ruda_nn::Linear<B>,
    linear2: ruda_nn::Linear<B>,
}
...
impl<B: Backend> ForwardTestModel<B> {
    ...
    fn forward(&self, input: Tensor<B, 2>) -> Tensor<B, 2> {
        ...
        let x = ruda_model::tensor::activation::gelu(x);
        self.linear2.forward(x)
    }
}

#[derive(ruda_model::config::Config, Debug)]
...
impl ForwardTestModelConfig {
    fn init<B: Backend>(&self, device: &B::Device) -> ForwardTestModel<B> {
        ForwardTestModel {
            linear1: ruda_nn::LinearConfig::new(self.input_size, self.hidden_size)
                .with_bias(true)
                .init(device),
            linear2: ruda_nn::LinearConfig::new(self.hidden_size, self.output_size)
                .with_bias(true)
                .init(device),
        }
    }
...
fn test_forward_pass_preservation_after_save_load() {
    ...
    let input = Tensor::<TestBackend, 2>::random(
        [1, 4],
        ruda_tensor::api::Distribution::Uniform(-1.0, 1.0),
        &device,
    );
...
fn test_store_cache_invalidation_on_save() {
    ...
    let mut store = RudapackStore::from_bytes(None);
...
/// Test storing and loading quantized weights with RudapackStore.
/// Regression test for https://github.com/shuqi2077/RUDA/blob/main/THIRD_PARTY_NOTICES.md
#[test]
fn test_store_quantized_module_round_trip() {
    use ruda_model::module::Quantizer;
    use ruda_nn::LinearConfig;
    use ruda_tensor::api::quantization::{
    ...
    let scheme = <<TestBackend as ruda_tensor::api::backend::BackendTypes>::QuantizedTensorPrimitive as QTensorPrimitive>::default_scheme()
...
fn test_store_half_precision_round_trip() {
    ...
    let mut save_store = RudapackStore::from_bytes(None).with_to_adapter(adapter.clone());
    save_store.collect_from(&model).unwrap();
    let bytes = save_store.get_bytes().unwrap();

    // Verify stored tensors are F16
    let mut inspect_store = RudapackStore::from_bytes(Some(bytes.clone()));
    ...
    let mut load_store = RudapackStore::from_bytes(Some(bytes)).with_from_adapter(adapter);
...
fn test_store_half_precision_batch_norm_excluded() {
    use crate::HalfPrecisionAdapter;
    use ruda_nn::{BatchNorm, BatchNormConfig, Linear, LinearConfig};
    use ruda_tensor::api::DType;
    ...
    let adapter = HalfPrecisionAdapter::new();
    let mut save_store = RudapackStore::from_bytes(None).with_to_adapter(adapter);
    save_store.collect_from(&model).unwrap();
    let bytes = save_store.get_bytes().unwrap();

    // Verify: Linear tensors are F16, BatchNorm tensors remain F32
    let mut inspect_store = RudapackStore::from_bytes(Some(bytes));
    let snapshots = inspect_store.get_all_snapshots().unwrap();
...
fn test_store_half_precision_without_module() {
    use crate::HalfPrecisionAdapter;
    use ruda_nn::{LayerNorm, LayerNormConfig, Linear, LinearConfig};
    use ruda_tensor::api::DType;
...
fn test_store_half_precision_chained_with_pytorch() {
    use crate::{HalfPrecisionAdapter, PyTorchToRudaAdapter};
    use ruda_nn::{Linear, LinearConfig};
    use ruda_tensor::api::DType;
    ...
    // Save with chained adapter: RudaToPyTorch then half-precision
    let adapter = crate::RudaToPyTorchAdapter.chain(HalfPrecisionAdapter::new());
    let mut save_store = RudapackStore::from_bytes(None).with_to_adapter(adapter);
    save_store.collect_from(&model).unwrap();
    let bytes = save_store.get_bytes().unwrap();
    ...
    // Weight should be transposed: [4, 2] original -> [2, 4] after RudaToPyTorch
    assert_eq!(weight.shape, shape![2, 4]);

    // Load back with reverse chain: half-precision (F16 -> F32) then PyTorchToRuda
    let adapter = HalfPrecisionAdapter::new().chain(PyTorchToRudaAdapter);
    let mut load_store = RudapackStore::from_bytes(Some(bytes)).with_from_adapter(adapter);
