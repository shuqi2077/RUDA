use crate::rudapack::{
    base::{
        RudapackHeader, RudapackMetadata, FORMAT_VERSION, HEADER_SIZE, MAGIC_NUMBER,
...
use ruda_tensor::api::{BoolStore, DType, TensorData, shape};
use std::rc::Rc;

#[test]
fn test_writer_new() {
    let writer = RudapackWriter::new(vec![]);
...
fn test_writer_to_bytes_empty() {
    ...
    // Parse header
    let header = RudapackHeader::from_bytes(&bytes[..HEADER_SIZE]).unwrap();
    ...
    let metadata: RudapackMetadata = ciborium::de::from_reader(metadata_bytes).unwrap();
...
fn test_writer_to_bytes_with_tensors() {
    ...
    let writer = RudapackWriter::new(vec![snapshot_f32, snapshot_i64])
...
fn test_writer_all_dtypes_round_trip() {
    use crate::rudapack::reader::RudapackReader;
    use half::{bf16, f16};
    ...
    // Read back using RudapackReader
...
fn test_writer_large_tensor() {
    ...
    let header = RudapackHeader::from_bytes(&result[..HEADER_SIZE]).unwrap();
    let metadata: RudapackMetadata = ciborium::de::from_reader(
...
fn test_writer_write_to_file() {
    ...
    let writer = RudapackWriter::new(vec![snapshot]).with_metadata("file_test", "true");
...
fn test_writer_write_into_multiple_tensors() {
    ...
    let writer = RudapackWriter::new(vec![snapshot1, snapshot2]).with_metadata("test", "multiple");
