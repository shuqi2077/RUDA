...
/// Writer for creating Rudapack files
pub struct RudapackWriter {
    /// Tensors to write
...
impl RudapackWriter {
    ...
    fn build_metadata(&self) -> Result<(RudapackMetadata, Vec<u8>), RudapackError> {
    ...
    /// Calculate the total size needed for the rudapack data
    ///
    ...
    pub fn size(&self) -> Result<usize, RudapackError> {
    ...
    /// Write rudapack data into a caller-provided buffer
    ///
    ...
    pub fn write_into(&self, buffer: &mut [u8]) -> Result<(), RudapackError> {
        let (metadata, metadata_bytes) = self.build_metadata()?;
        ...
        for snapshot in &self.snapshots {
            ...
            let data = snapshot.to_data().map_err(|e| {
                RudapackError::IoError(format!("Failed to get tensor data: {:?}", e))
            })?;
    ...
    ///
    /// This allocates a buffer internally and writes the rudapack data.
    /// For more control over buffer allocation, use `size()` + `write_into()`.
    pub fn to_bytes(&self) -> Result<Bytes, RudapackError> {
        let size = self.size()?;
    ...
    #[cfg(feature = "std")]
    pub fn write_to_file<P: AsRef<Path>>(&self, path: P) -> Result<(), RudapackError> {
        let mut file = File::create(path).map_err(|e| RudapackError::IoError(e.to_string()))?;
