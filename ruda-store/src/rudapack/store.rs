...
use super::reader::RudapackReader;
use super::writer::RudapackWriter;
#[cfg(feature = "std")]
use crate::KeyRemapper;
use crate::rudapack::base::RudapackError;
use crate::{
...
use alloc::vec::Vec;
use ruda_model::prelude::Backend;
use ruda_tensor::api::Bytes;

/// Store mode for RudapackStore
enum StoreMode {
    #[cfg(feature = "std")]
    File(PathBuf),
    Bytes(Option<Bytes>),
...
/// RudapackStore - A Ruda-specific file format store using CBOR for metadata
pub struct RudapackStore {
    ...
    /// Adapter applied when loading (source -> Ruda)
    from_adapter: Box<dyn ModuleAdapter>,
    /// Adapter applied when saving (Ruda -> target)
    to_adapter: Box<dyn ModuleAdapter>,
    /// Writer for saving
    writer: Option<RudapackWriter>,
    /// Reader for loading
    reader: Option<RudapackReader>,
    /// Cached tensor snapshots (parsed once, reused)
    snapshots_cache: Option<BTreeMap<String, TensorSnapshot>>,
}

impl RudapackStore {
    /// Get the default metadata that includes Ruda framework information.
    ///
    /// This includes:
    /// - `format`: "rudapack"
    /// - `producer`: "ruda"
    /// - `version`: The version of ruda-store crate (from CARGO_PKG_VERSION)
    ///
    /// These metadata fields are automatically added to all saved models.
    pub fn default_metadata() -> BTreeMap<String, String> {
        let mut metadata = BTreeMap::new();
        metadata.insert("format".into(), "rudapack".into());
        metadata.insert("producer".into(), "ruda".into());
    ...
    ///
    /// ```no_run
    /// # use ruda_store::RudapackStore;
    /// // Automatically appends .bpk
    /// let store = RudapackStore::from_file("model");  // creates "model.bpk"
    ///
    /// // Already has extension, no append
    /// let store = RudapackStore::from_file("model.bpk");  // uses "model.bpk"
    /// let store = RudapackStore::from_file("model.myext");  // uses "model.myext"
    ///
    /// // Disable auto-extension
    /// let store = RudapackStore::from_file("model").auto_extension(false);  // uses "model"
    /// ```
    #[cfg(feature = "std")]
    ...
    /// let store = RudapackStore::from_static(MODEL_DATA);
    /// ```
    pub fn from_static(data: &'static [u8]) -> Self {
        use ruda_tensor::api::AllocationProperty;

        // Create bytes::Bytes from static data (zero-copy, stays in .rodata)
        let shared = bytes::Bytes::from_static(data);

        // Wrap in ruda Bytes with shared-bytes allocation controller
    ...
    /// let store = RudapackStore::from_file("model");  // -> "model.bpk"
    ///
    /// // With auto_extension disabled
    /// let store = RudapackStore::from_file("model")
    ...
    /// Set the adapter for loading tensors (converting from source format to Ruda).
    pub fn with_from_adapter(mut self, adapter: impl ModuleAdapter + 'static) -> Self {
        self.from_adapter = Box::new(adapter);
        self
    }

    /// Set the adapter for saving tensors (converting from Ruda to target format).
    ...
    pub fn get_bytes(&self) -> Result<Bytes, RudapackError> {
        ...
        match &self.mode {
            ...
            _ => Err(RudapackError::IoError("No bytes available".into())),
        }
    }
    ...
    fn ensure_reader(&mut self) -> Result<&RudapackReader, RudapackError> {
        if self.reader.is_none() {
            let reader = match &self.mode {
                #[cfg(feature = "std")]
                StoreMode::File(path) => {
                    let final_path = self.process_path(path);
                    RudapackReader::from_file(&final_path)?
                }
                StoreMode::Bytes(Some(bytes)) => RudapackReader::from_bytes(bytes.clone())?,
                StoreMode::Bytes(None) => {
                    return Err(RudapackError::IoError("No bytes to read from".into()));
                }
            };
        ...
        self.reader
            .as_ref()
            .ok_or_else(|| RudapackError::IoError("Reader not initialized".into()))
    }
}

impl ModuleStore for RudapackStore {
    type Error = RudapackError;
    ...
    fn collect_from<B: Backend, M: ModuleSnapshot<B>>(
        ...
        let mut writer = RudapackWriter::new(snapshots);
    ...
    fn apply_to<B: Backend, M: ModuleSnapshot<B>>(
        ...
        // Rudapack is Ruda's native format, so no enum variant skipping needed
...
impl RudapackStore {
    ...
    fn ensure_snapshots_cache(&mut self) -> Result<(), RudapackError> {
