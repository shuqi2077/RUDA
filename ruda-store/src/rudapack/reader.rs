...
use super::base::{
    RudapackError, RudapackHeader, RudapackMetadata, FORMAT_VERSION, HEADER_SIZE, MAGIC_NUMBER,
...
use ruda_tensor::api::{Bytes, Shape, TensorData};
...
/// Storage backend for RudapackReader
...
impl StorageBackend {
    ...
    pub(crate) fn read_into(&self, bytes: &mut [u8], offset: usize) -> Result<(), RudapackError> {
        match self {
            StorageBackend::Memory(data) => {
                let data_bytes = data.as_ref();
                let end = offset.checked_add(bytes.len()).ok_or_else(|| {
                    RudapackError::IoError(format!(
                ...
                if end > data_bytes.len() {
                    return Err(RudapackError::IoError(format!(
            ...
            StorageBackend::FileBuffered { file } => {
                ...
                file.seek(SeekFrom::Start(offset as u64)).map_err(|e| {
                    RudapackError::IoError(format!("Failed to seek in file: {}", e))
                })?;

                file.read_exact(bytes).map_err(|e| {
                    RudapackError::IoError(format!("Failed to read from file: {}", e))
                })?;
                Ok(())
            }
        }
    ...
    #[allow(dead_code)]
    pub(crate) fn as_bytes(&self) -> Result<&[u8], RudapackError> {
        match self {
            StorageBackend::Memory(data) => Ok(data.as_ref()),
            #[cfg(feature = "std")]
            StorageBackend::FileBuffered { .. } => Err(RudapackError::IoError(
    ...
    pub(crate) fn slice_bytes(&self, start: usize, end: usize) -> Result<Bytes, RudapackError> {
        if end < start {
            return Err(RudapackError::IoError(format!(
        ...
        match self {
            StorageBackend::Memory(data) => {
                ...
                let (_, right) = cloned.split(start).map_err(|(_, e)| {
                    RudapackError::IoError(format!("Failed to split at start {}: {:?}", start, e))
                })?;
...
/// Reader for loading Rudapack files
pub struct RudapackReader {
    /// Parsed metadata
    pub(crate) metadata: RudapackMetadata,
    /// Storage backend
...
impl RudapackReader {
    /// Load from bytes
    pub fn from_bytes(bytes: Bytes) -> Result<Self, RudapackError> {
        // Validate minimum size
        if bytes.len() < HEADER_SIZE {
            return Err(RudapackError::InvalidHeader);
        }

        // Parse header
        let header = RudapackHeader::from_bytes(&bytes[..HEADER_SIZE])?;

        // Verify magic number
        if header.magic != MAGIC_NUMBER {
            return Err(RudapackError::InvalidMagicNumber);
        }

        // Verify version compatibility
        if header.version > FORMAT_VERSION {
            return Err(RudapackError::InvalidVersion);
        }

        // Validate metadata size against security limit
        if header.metadata_size > MAX_METADATA_SIZE {
            return Err(RudapackError::ValidationError(format!(
        ...
        let metadata: RudapackMetadata = ciborium::de::from_reader_with_recursion_limit(
            &bytes[metadata_start..metadata_end],
            MAX_CBOR_RECURSION_DEPTH,
        )
        .map_err(|e| RudapackError::MetadataDeserializationError(e.to_string()))?;
        ...
        if !metadata.tensors.is_empty() {
            ...
            let max_data_offset_usize: usize = max_data_offset.try_into().map_err(|_| {
                RudapackError::ValidationError(format!(
            ...
            let min_file_size =
                metadata_end
                    ...
                    .ok_or_else(|| {
                        RudapackError::ValidationError("File size calculation overflow".into())
                    })?;
    ...
    pub(crate) fn from_file_mmap<P: AsRef<Path>>(path: P) -> Result<Self, RudapackError> {
        let file = File::open(&path).map_err(|e| RudapackError::IoError(e.to_string()))?;

        // Validate maximum file size to prevent resource exhaustion
        let file_size = file
            .metadata()
            .map_err(|e| RudapackError::IoError(e.to_string()))?
            .len();
        ...
        let header = RudapackHeader::from_bytes(&mmap[..HEADER_SIZE])?;
        ...
        let bytes = Bytes::from_shared(shared_bytes, ruda_tensor::api::AllocationProperty::File);
    ...
    #[cfg(feature = "std")]
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self, RudapackError> {
        #[cfg(feature = "memmap")]
        {
    ...
    #[cfg(feature = "std")]
    #[allow(dead_code)]
    pub(crate) fn from_file_buffered<P: AsRef<Path>>(path: P) -> Result<Self, RudapackError> {
        let mut file = File::open(&path).map_err(|e| RudapackError::IoError(e.to_string()))?;
        ...
        let header = RudapackHeader::from_bytes(&header_bytes)?;
    ...
    pub fn get_snapshots(&self) -> Result<Vec<TensorSnapshot>, RudapackError> {
    ...
    pub fn get_snapshots_zero_copy(
        &self,
        zero_copy: bool,
    ) -> Result<Vec<TensorSnapshot>, RudapackError> {
    ...
    fn get_snapshots_internal(
        ...
        for (name, descriptor) in &self.metadata.tensors {
            ...
            let shape: Shape = Shape::from(descriptor
                ...
                .collect::<Result<Vec<usize>, RudapackError>>()?);
    ...
    #[allow(dead_code)]
    pub(crate) fn get_tensor_snapshot(&self, name: &str) -> Result<TensorSnapshot, RudapackError> {
        let snapshots = self.get_snapshots()?;
        snapshots
            .into_iter()
            .find(|s| s.full_path() == name)
            .ok_or_else(|| RudapackError::TensorNotFound(name.to_string()))
    }
    ...
    /// Get metadata
    #[allow(dead_code)]
    pub(crate) fn metadata(&self) -> &RudapackMetadata {
        &self.metadata
    }

    /// Get tensor data as raw bytes
    #[allow(dead_code)]
    pub(crate) fn get_tensor_data(&self, name: &str) -> Result<Vec<u8>, RudapackError> {
        let descriptor = self
            .metadata
            .tensors
            .get(name)
