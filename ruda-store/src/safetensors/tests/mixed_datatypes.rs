...
use ruda_model::module::{Module, Param, ParamId};
use ruda_nn as nn;
    ...
    pub fn new(device: &B::Device) -> Self {
        Self {
            ...
            bool_tensor: Param::initialized(
                ParamId::new(),
                Tensor::from_bool(
                    ruda_model::tensor::TensorData::new(
...
mod tests {
    use ruda_tensor::api::{BoolStore, DType};
        ...
        for snapshot in snapshots {
            ...
            if path.contains("float_tensor") || path.contains("linear") {
                ...
                assert!(
                    matches!(
                        dtype,
                        ruda_model::tensor::DType::I64
                            | ruda_model::tensor::DType::I32
                            | ruda_model::tensor::DType::I16
                            | ruda_model::tensor::DType::I8
                    ),
                ...
                assert_eq!(
                    dtype,
                    ruda_model::tensor::DType::Bool(BoolStore::Native),
    ...
    fn test_mixed_precision_floats() {
        ...
        // (F16, BF16, F32, F64, etc.) in the same file, Ruda's backend architecture currently
    ...
    fn test_mixed_precision_integers() {
        ...
        struct MultiIntModel<B: Backend> {
            // Note: Ruda's Tensor<B, D, Int> uses the backend's default int type
    ...
    fn test_comprehensive_mixed_types() {
        ...
        let mut loaded_model = ComprehensiveModel::<TestBackend> {
            ...
            boolean_mask: Param::initialized(
                ParamId::new(),
                Tensor::from_bool(
                    ruda_model::tensor::TensorData::new(vec![false; 8], [2, 4]),
                    &device,
                ),
            ),
        };
