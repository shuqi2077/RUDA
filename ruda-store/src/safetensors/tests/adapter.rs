...
use crate::{
    RudaToPyTorchAdapter, ModuleSnapshot, ModuleStore, PyTorchToRudaAdapter, SafetensorsStore,
};
...
#[test]
fn pytorch_to_ruda_adapter_linear_transpose() {
    let device = Default::default();
    let model = TestModel::<TestBackend>::new(&device);

    // Save with RudaToPyTorch adapter (will transpose linear weights)
    let mut save_store = SafetensorsStore::from_bytes(None).with_to_adapter(RudaToPyTorchAdapter);
    model.save_into(&mut save_store).unwrap();

    // Load with PyTorchToRuda adapter (will transpose back)
    let mut load_store = SafetensorsStore::from_bytes(None).with_from_adapter(PyTorchToRudaAdapter);
...
#[test]
fn pytorch_to_ruda_adapter_norm_rename() {
    let device = Default::default();
    ...
    // Save with RudaToPyTorch adapter (will rename gamma->weight, beta->bias)
    let mut save_store = SafetensorsStore::from_bytes(None).with_to_adapter(RudaToPyTorchAdapter);
    model.save_into(&mut save_store).unwrap();
    ...
    // Load with PyTorchToRuda adapter (will rename weight->gamma, bias->beta)
    let mut load_store = SafetensorsStore::from_bytes(None).with_from_adapter(PyTorchToRudaAdapter);
    if let SafetensorsStore::Memory(ref mut p) = load_store
        && let SafetensorsStore::Memory(ref p_save) = save_store
    {
...
fn adapter_with_pytorch_import() {
    use crate::PyTorchToRudaAdapter;

    let device = Default::default();

    // Reference the safetensors file from ruda-store
    ...
    // Load with PyTorchToRuda adapter
...
fn half_precision_adapter_without_module() {
    use crate::HalfPrecisionAdapter;
    use ruda_nn::{LayerNorm, LayerNormConfig};
    use ruda_tensor::api::DType;
