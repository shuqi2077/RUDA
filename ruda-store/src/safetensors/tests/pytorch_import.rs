...
fn multi_layer_model_import() {
    ...
    // Reference the safetensors file from ruda-io
    ...
    // Note: PyTorch and Ruda have different conventions for linear layer weights
    // PyTorch stores as [out_features, in_features], Ruda as [in_features, out_features]
    ...
    let mut store = SafetensorsStore::from_file(safetensors_path)
        .with_from_adapter(crate::PyTorchToRudaAdapter) // Use adapter to handle PyTorch format
    ...
    // Note: Due to shape mismatches (PyTorch vs Ruda conventions for linear layers),
