...
impl SafetensorsStore {
    ...
    pub fn default_metadata() -> HashMap<String, String> {
        ...
        metadata.insert("producer".to_string(), "ruda".to_string());
    ...
    /// Clear all metadata including the default Ruda framework metadata.
    ///
    ...
    /// can be matched against Ruda module paths that do include them.
    /// For example, source path "feature.weight" can match Ruda path "feature.BaseConv.weight".
    ///
...
impl safetensors::View for TensorSnapshotAdapter {
    fn dtype(&self) -> safetensors::Dtype {
        // Convert from ruda dtype to safetensors dtype
...
impl ModuleStore for SafetensorsStore {
    ...
    fn collect_from<B: Backend, M: ModuleSnapshot<B>>(
        ...
        // The to_adapter converts from Ruda format to target format for saving
...
mod conversion;
use conversion::{snapshots_to_safetensors, safetensors_to_snapshots_lazy, dtype_to_safetensors};
#[cfg(feature = "std")]
use conversion::safetensors_to_snapshots_lazy_file;
