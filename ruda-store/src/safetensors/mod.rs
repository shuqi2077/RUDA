//! SafeTensors format support for Ruda deep learning framework.
//!
...
//!
//! ```rust,ignore
//! use ruda_store::{SafetensorsStore, ModuleSnapshot};
//!
...
//!
//! ```rust,no_run
//! # use ruda_store::SafetensorsStore;
...
//!
//! ```rust,no_run
//! # use ruda_store::{SafetensorsStore, KeyRemapper};
...
//!     .add_pattern(r"^pytorch\.(.*)", "ruda.$1").expect("valid regex")           // pytorch.layer -> ruda.layer
...
//!
//! ```rust,ignore
//! use ruda_store::{SafetensorsStore, ModuleSnapshot, PyTorchToRudaAdapter, RudaToPyTorchAdapter};
//!
//! // Loading PyTorch model into Ruda
//! let mut store = SafetensorsStore::from_file("pytorch_model.safetensors")
//!     .with_from_adapter(PyTorchToRudaAdapter)  // Transposes linear weights, renames norm params
//!     .allow_partial(true);                     // PyTorch models may have extra tensors
//!
//! let mut ruda_model = Model::new(&device);
//! ruda_model.load_from(&mut store)?;
//!
//! // Saving Ruda model for PyTorch
//! let mut store = SafetensorsStore::from_file("for_pytorch.safetensors")
//!     .with_to_adapter(RudaToPyTorchAdapter);   // Transposes weights back, renames for PyTorch
//!
//! ruda_model.save_into(&mut store)?;
//! ```
//!
...
//! ## Adapter Methods
//!
//! - **`with_from_adapter(adapter)`** - Set adapter for loading (e.g., PyTorchToRudaAdapter)
//! - **`with_to_adapter(adapter)`** - Set adapter for saving (e.g., RudaToPyTorchAdapter)
//!
...
//!
//! ```rust,no_run
//! use ruda_store::{SafetensorsStore, PyTorchToRudaAdapter};
//!
...
//!
//! Migrating a PyTorch model to Ruda with filtering, remapping, and adapters:
//!
//! ```rust,ignore
//! use ruda_store::{SafetensorsStore, ModuleSnapshot, PyTorchToRudaAdapter};
//!
...
//!     .metadata("converted_by", "ruda-store");
//!
