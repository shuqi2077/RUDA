...
use crate::{
    ApplyResult, KeyRemapper, ModuleSnapshot, ModuleStore, PathFilter, PyTorchToRudaAdapter,
...
/// with automatic weight transformation using `PyTorchToRudaAdapter`.
...
impl PytorchStore {
    ...
    /// For example, PyTorch path "feature.weight" can match Ruda path "feature.BaseConv.weight".
    ///
...
impl ModuleStore for PytorchStore {
    ...
    fn apply_to<B: Backend, M: ModuleSnapshot<B>>(
        ...
        // Apply to module with PyTorchToRudaAdapter (always used for PyTorch files)
        // This adapter handles:
        // - Transposing linear weights from PyTorch format to Ruda format
        ...
        let result = module.apply(
            snapshots,
            filter_opt,
            Some(Box::new(PyTorchToRudaAdapter)),
