...
/// Path to ruda-store test data files
...
mod adapter_tests {
    ...
    fn test_pytorch_adapter_always_applied() {
        // Test that PyTorchToRudaAdapter is always applied internally
        let path = pytorch_test_path("linear", "linear.pt");
        ...
        // PyTorchToRudaAdapter is always applied internally
        assert!(
            result.is_ok(),
            "Failed to load with internal PyTorchToRudaAdapter: {:?}",
            result.err()
        );
    ...
    fn test_pytorch_adapter_with_filtering() {
        // Test that PyTorchToRudaAdapter works with filtering
        let path = pytorch_test_path("linear", "linear.pt");
...
mod error_handling_tests {
    ...
    fn test_missing_file() {
        let device = Default::default();
        let mut model = SimpleLinearModel::<Host>::new(&device);
        let mut store = PytorchStore::from_file("nonexistent.pth");

        let result = store.apply_to::<Host, _>(&mut model);
...
mod enum_variant_tests {
    ...
    /// Model with enum field that will have variant names in Ruda paths
    #[derive(Module, Debug)]
    ...
    fn test_enum_variant_path_mismatch() {
        let device = Default::default();
        let mut model = ModelWithEnum::<Host>::new(&device);
        ...
        // Ruda paths:    "feature.BaseConv.weight", "feature.BaseConv.bias", "classifier.weight", "classifier.bias"
        //                         ^^^^^^^^ enum variant name is included in Ruda but not PyTorch
    ...
    fn test_enum_variant_detection_in_container_stack() {
        ...
        let model = ModelWithEnum::<Host>::new(&device);
...
mod map_indices_contiguous_tests {
    ...
    fn test_load_non_contiguous_indexes_with_mapping() {
        ...
        // The Ruda model expects fc.0, fc.1, fc.2, ... (contiguous)
