...
- Ruda path:    "feature.BaseConv.weight"  (includes enum variant "BaseConv")
...
class SimpleModel(nn.Module):
    """
    Simple PyTorch model that represents what a Ruda enum model would look like
    WITHOUT the enum variant names in the path.

    In Ruda, this would be:
    ...
    def __init__(self):
        super().__init__()
        # This represents the "feature" field which is an enum in Ruda
...
def generate_enum_variant_mismatch_test():
    ...
    # Ruda paths:    "feature.BaseConv.weight", "feature.BaseConv.bias", ...
