...
use ruda_tensor::api::{DType, Shape};
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        ...
        if !self.missing.is_empty() {
            ...
            if !enum_variant_missing.is_empty() {
                ...
                writeln!(
                    f,
                    "│      Ruda includes enum variant names in paths, but PyTorch doesn't."
                )?;
                writeln!(
                    f,
                    "│      Example: Ruda has 'field.BaseConv.weight', PyTorch has 'field.weight'"
                )?;
                writeln!(f, "│")?;
                writeln!(
                    f,
