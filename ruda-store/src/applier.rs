...
use ruda_model::module::{ModuleMapper, Param};
use ruda_tensor::api::{Bool, Int, Shape, Tensor, backend::Backend};
...
impl<B: Backend> Applier<B> {
    ...
    where
        K: ruda_tensor::api::TensorKind<B>,
        K: ruda_tensor::api::BasicOps<B>,
    {
            ...
            && let Some(module_type) = self.current_module_type()
        {
            ...
            if let Some(alt_name) = adapter.get_alternative_param_name(param_name, module_type) {
                ...
                *alt_path_stack.last_mut().unwrap() = alt_name.clone();
                ...
                // Don't mark the alternative path as visited - only the original Ruda path
...
mod tests {
    use super::*;
    use ruda_model::module::{ModuleMapper, Param, ParamId};
