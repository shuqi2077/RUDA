...
mod tests {
    ...
    use ruda_tensor::api::{TensorData, shape};

    fn create_test_tensor_snapshot(name: &str) -> TensorSnapshot {
        let data = TensorData {
            bytes: ruda_tensor::api::Bytes::from_bytes_vec(vec![1, 2, 3, 4]),
            shape: shape![2, 2],
            dtype: ruda_tensor::api::DType::F32,
        };
    ...
    fn test_key_remapper_from_patterns() {
        let patterns = vec![(r"^pytorch\.", "ruda."), (r"\.bias$", ".bias_param")];
        ...
        assert!(
            remapped
                .iter()
                .any(|v| v.full_path() == "ruda.linear.bias_param")
        );
    }
