...
use ruda_model::tensor::{Bool, Int, Tensor, backend::Backend, s};
use core::f32;
...
impl RNNTLoss {
    ...
    pub fn forward<B: Backend>(
        ...
        let log_probs = if self.logits {
            ...
            ruda_model::tensor::activation::log_softmax(logits, vocab_dim)
        } else {
            logits
        };
...
mod tests {
    use super::*;
    use ruda_model::tensor::{TensorData, Tolerance};
    ...
    fn logits_equivalence() {
        ...
        let logits = Tensor::<B, 4>::from_data(
            ruda_model::tensor::TensorData::new(data, [bs, time_steps, up1, vocab]),
            &dev,
        );
        ...
        let log_probs = ruda_model::tensor::activation::log_softmax(logits, vocab_dim);
...
mod pytorch_comparison_tests {
    ...
    fn make_logits(bs: usize, t: usize, u: usize, v: usize, dev: &HostDevice) -> Tensor<B, 4> {
