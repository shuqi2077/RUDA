use alloc::format;
use ruda_model::tensor::ops::DeformConvOptions;


use crate::PaddingConfig2d;
use ruda_model::config::Config;
use ruda_model::module::Initializer;
use ruda_model::module::{Content, DisplaySettings, Module, ModuleDisplay, Param};
use ruda_model::tensor::Tensor;
use ruda_model::tensor::backend::Backend;
use ruda_model::tensor::module::deform_conv2d;
...
impl DeformConv2dConfig {
    ...
    pub fn init<B: Backend>(&self, device: &B::Device) -> DeformConv2d<B> {
        ...
        assert!(
            self.offset_groups > 0 && self.channels[0].is_multiple_of(self.offset_groups),
            "Input channels must be divisible by non-zero offset groups."
        );
...
impl<B: Backend> DeformConv2d<B> {
    ...
    ///
    /// See [deform_conv2d](ruda_tensor::api::module::deform_conv2d) for more information.
    ///
    /// # Shapes
    ///
    ...
    pub fn forward(
        ...
        let kernel_size = core::array::from_fn(|axis| {
            crate::padding::dilated_kernel_size(self.kernel_size[axis], self.dilation[axis])
        });
        let ((top, bottom), (left, right)) = self.padding.calculate_padding_2d_pairs(
            height_in, width_in, &kernel_size, &self.stride,
        );
        deform_conv2d(
            input,
            offset,
...
#[cfg(test)]
mod tests {
    use ruda_model::tensor::{ElementConversion, Tolerance, ops::FloatElem};
