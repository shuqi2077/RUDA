...
use ruda_model::tensor::{Tensor, backend::Backend, module::conv1d, ops::PaddedConvOptions};
use ruda_model::{
    config::Config,
...
impl<B: Backend> Conv1d<B> {
    ...
    ///
    /// See [conv1d](ruda_tensor::api::module::conv1d) for more information.
    ///
    /// # Shapes
    ///
    ...
    pub fn forward(&self, input: Tensor<B, 3>) -> Tensor<B, 3> {
        ...
        let kernel_size = crate::padding::dilated_kernel_size(self.kernel_size, self.dilation);

        // Calculate padding as pair - handles Same, Valid, and Explicit uniformly
        let (left, right) =
            self.padding
                .calculate_padding_1d_pair(length, kernel_size, self.stride);
...
#[cfg(test)]
mod tests {
    use ruda_model::tensor::{ElementConversion, ops::FloatElem};
