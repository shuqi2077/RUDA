...
use ruda_model::tensor::module::conv3d;
use ruda_model::tensor::ops::PaddedConvOptions;
...
impl<B: Backend> Conv3d<B> {
    ...
    ///
    /// See [conv3d](ruda_tensor::api::module::conv3d) for more information.
    ///
    /// # Shapes
    ///
    ...
    pub fn forward(&self, input: Tensor<B, 5>) -> Tensor<B, 5> {
        ...
        conv3d(
            ...
            PaddedConvOptions::asymmetric(
                self.stride,
                padding.map(|pair| pair.0),
                padding.map(|pair| pair.1),
                self.dilation,
                self.groups,
            ),
        )
