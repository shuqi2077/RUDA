...
pub(crate) fn dilated_kernel_size(kernel_size: usize, dilation: usize) -> usize {
    assert!(dilation > 0, "dilation must be non-zero");
    kernel_size.checked_sub(1)
        .and_then(|size| size.checked_mul(dilation))
        .and_then(|size| size.checked_add(1))
        .expect("kernel size must be non-zero and its dilated extent must fit usize")
}
...
impl PaddingConfig3d {
    pub(crate) fn calculate_padding_3d_pairs(
        ...
        match self {
            Self::Valid => [(0, 0); 3],
            Self::Same => core::array::from_fn(|axis| {
                calculate_same_padding(kernel_size[axis], stride[axis], size[axis])
            }),
