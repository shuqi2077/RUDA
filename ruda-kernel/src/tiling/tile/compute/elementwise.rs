...
use crate::tiling::tile::compute::{Mask, MaskExpand};
use crate::tiling::tile::{Plane, Tile, TileExpand};

/// Element-wise tile operations on `Tile<E, Plane, ReadWrite>`. Unlike the
/// row-wise primitives in [`crate::tiling::tile::compute::rowwise`], these touch every
...
impl<E: Float> Tile<E, Plane, ReadWrite> {
    ...
    pub fn fill_zero(&mut self) {
        match self {
            ...
            Tile::Bounce(b) => {
                crate::dsl::cmma::fill(&b.cmma.matrix, E::from_int(0));
            }
            Tile::Cmma(t) => {
                crate::dsl::cmma::fill(&t.matrix, E::from_int(0));
            }
