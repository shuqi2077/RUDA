...
use crate::tiling::StageIdent;
use crate::tiling::tile::compute::copy::{cmma_to_whitebox_fragment, whitebox_fragment_to_cmma};
use crate::tiling::tile::compute::mask::{Mask, MaskExpand};
use crate::tiling::tile::compute::rowwise::reducer::{fragment_row_max, fragment_row_sum};
use crate::tiling::tile::data::{
    BounceTile, InnerLayout, RegisterTile, RowWise, RowWiseExpand, UnitTile, WhiteboxFragment,
};
use crate::tiling::tile::{Plane, Tile, TileExpand};
...
fn write_fragment_into<Acc: Float, Lhs: Float>(
    ...
    match softmaxed {
        Tile::Bounce(d) => {
            ...
            sync_ruda();
            crate::dsl::cmma::load(&d.cmma.matrix, &d.smem.to_slice(), stride);
        }
