...
use crate::tiling::tile::data::strided::StridedTile;

/// Kind (family) of the tiles returned by a stage and ingested by a tile matmul reader
pub trait TileKind<IO: SliceVisibility = ReadOnly>: RudaType + Send + Sync + 'static {
    /// Concrete tile instantiated with the element type
    type Tile<E: Numeric, N: Size>: RudaType;
}
