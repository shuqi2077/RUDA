use crate::dsl::{intrinsic, prelude::*};
use crate::library::{Swizzle};

use crate::tiling::{MatrixLayout, stage::StageMemoryConfig, stage::as_swizzle_object};
    ...
    pub fn with_vector_size<N2: Size>(&self) -> StridedTile<ES, N2, IO> {
        ...
        intrinsic!(|scope| {
            ...
            if current < vector_size {
                ...
                let end = crate::dsl::frontend::div::expand(scope, self.end, ratio.into());
                let start = crate::dsl::frontend::div::expand(scope, self.start, ratio.into());
                let stride =
                    crate::dsl::frontend::div::expand(scope, self.stride, (ratio as u32).into());
                out.start = start;
                out.end = end;
                out.stride = stride;
            } else {
                let ratio = (current / vector_size) as u32;
                let start = crate::dsl::frontend::mul::expand(scope, self.start, ratio.into());
                let end = crate::dsl::frontend::mul::expand(scope, self.end, ratio.into());
                let stride = crate::dsl::frontend::mul::expand(scope, self.stride, ratio.into());
                out.start = start;
                out.end = end;
                out.stride = stride;
            }
...
/// vector_size on the ruda slice is preserved, so projecting back via
