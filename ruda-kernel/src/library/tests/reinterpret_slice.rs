...
use crate::library::{ReinterpretSlice, ReinterpretSliceMut};
use half::f16;
        ...
        mod reinterpret_slice_f16 {
            ...
            mod global {
                ...
                fn read_from_i8x1() {
                    ...
                    ruda_kernel::library::tests::reinterpret_slice::run_test_read_global::<TestRuntime>(client, 1);
                }

                #[$crate::library::tests::test_log::test]
                fn read_from_i8x2() {
                    let client = TestRuntime::client(&Default::default());
                    ruda_kernel::library::tests::reinterpret_slice::run_test_read_global::<TestRuntime>(client, 2);
                }

                #[$crate::library::tests::test_log::test]
                fn read_from_i8x4() {
                    let client = TestRuntime::client(&Default::default());
                    ruda_kernel::library::tests::reinterpret_slice::run_test_read_global::<TestRuntime>(client, 4);
                }

                #[$crate::library::tests::test_log::test]
                fn write_into_i8x1() {
                    let client = TestRuntime::client(&Default::default());
                    ruda_kernel::library::tests::reinterpret_slice::run_test_write_global::<TestRuntime>(client, 1);
                }

                #[$crate::library::tests::test_log::test]
                fn write_into_i8x2() {
                    let client = TestRuntime::client(&Default::default());
                    ruda_kernel::library::tests::reinterpret_slice::run_test_write_global::<TestRuntime>(client, 2);
                }

                #[$crate::library::tests::test_log::test]
                fn write_into_i8x4() {
                    let client = TestRuntime::client(&Default::default());
                    ruda_kernel::library::tests::reinterpret_slice::run_test_write_global::<TestRuntime>(client, 4);
                }
            }
            ...
            mod shared_memory {
                ...
                fn read_from_i8x4() {
                    let client = TestRuntime::client(&Default::default());
                    ruda_kernel::library::tests::reinterpret_slice::run_test_read_shared_memory::<TestRuntime>(client);
                }

                #[$crate::library::tests::test_log::test]
                fn write_from_i8x4() {
                    let client = TestRuntime::client(&Default::default());
                    ruda_kernel::library::tests::reinterpret_slice::run_test_write_shared_memory::<TestRuntime>(client);
                }
            }
        }
    };
