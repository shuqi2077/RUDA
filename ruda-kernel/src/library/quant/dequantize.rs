...
#[ruda]
pub fn dequantize_aligned<Q: Scalar, S: RudaPrimitive, F: Numeric, NQ: Size, NF: Size>(
    value: Vector<Q, NQ>,
    scale: S,
    #[comptime] scheme: QuantScheme,
) -> Vector<F, NF> {
    let q_values = unpack_quantized::<Q, F, NQ, NF>(value, scheme);
...
/// Decode stored values without applying scales. `NF` must equal `NQ * scheme.num_quants()`.
#[ruda]
pub fn unpack_quantized<Q: Scalar, F: Numeric, NQ: Size, NF: Size>(
    value: Vector<Q, NQ>,
    #[comptime] scheme: QuantScheme,
) -> Vector<F, NF> {
    match scheme.store {
