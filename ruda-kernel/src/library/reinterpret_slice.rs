...
use ruda_kernel::dsl::{ir::VectorSize, unexpanded};
...
#[derive(RudaType)]
pub struct ReinterpretSlice<S: RudaPrimitive, T: RudaPrimitive> {
...
#[ruda]
impl<S: RudaPrimitive, T: RudaPrimitive> ReinterpretSlice<S, T> {
...
/// Currently, this only work with `ruda(launch_unchecked)` and is not supported on wgpu.
#[derive(RudaType)]
pub struct ReinterpretSliceMut<S: RudaPrimitive, T: RudaPrimitive> {
    slice: SliceMut<S>,
...
#[ruda]
impl<S: RudaPrimitive, T: RudaPrimitive> ReinterpretSliceMut<S, T> {
...
pub fn size_of<S: RudaPrimitive>() -> u32 {
    unexpanded!()
}
...
pub mod size_of {
    ...
    pub fn expand<S: RudaPrimitive>(context: &mut crate::dsl::prelude::Scope) -> u32 {
