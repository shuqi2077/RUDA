...
use ruda_kernel::dsl::{intrinsic, ir::Scope, unexpanded};

use crate::library::tensor::layout::{Coordinates, Layout, LayoutExpand};
...
impl<C: Coordinates, S: Coordinates> RudaType for VirtualLayout<C, S> {
...
impl<C: Coordinates, S: Coordinates> RudaDebug for VirtualLayoutExpand<C, S> {}

// We need to seal the trait to allow us to blanket implement `From<L>` below
mod private {
    pub trait Sealed {}
}
pub trait VirtualLayoutOperationsExpand<C: RudaType, S: RudaType>: private::Sealed {
    fn __expand_to_source_pos_method(
        &self,
        scope: &mut Scope,
        pos: <C as RudaType>::ExpandType,
    ) -> <S as RudaType>::ExpandType;
    fn __expand_to_source_pos_checked_method(
        &self,
        scope: &mut Scope,
        pos: <C as RudaType>::ExpandType,
    ) -> <(S, bool) as RudaType>::ExpandType;
    fn __expand_shape_method(&self, scope: &mut Scope) -> <C as RudaType>::ExpandType;
...
impl<L: LayoutExpand> VirtualLayoutOperationsExpand<L::Coordinates, L::SourceCoordinates> for L {
    fn __expand_to_source_pos_method(
        &self,
        scope: &mut Scope,
        pos: <L::Coordinates as RudaType>::ExpandType,
    ) -> <L::SourceCoordinates as RudaType>::ExpandType {
    ...
    fn __expand_to_source_pos_checked_method(
        ...
        scope: &mut Scope,
        pos: <L::Coordinates as RudaType>::ExpandType,
    ) -> <(L::SourceCoordinates, bool) as RudaType>::ExpandType {
        <L as LayoutExpand>::__expand_to_source_pos_checked_method(self.clone(), scope, pos)
    }

    fn __expand_shape_method(&self, scope: &mut Scope) -> <L::Coordinates as RudaType>::ExpandType {
