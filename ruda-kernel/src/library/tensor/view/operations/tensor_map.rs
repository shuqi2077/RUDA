use super::*;
use crate::library::tensor::layout::*;
use crate::dsl::prelude::*;
use ruda_kernel::dsl::{prelude::barrier::BarrierExpand};
...
macro_rules! impl_tensor_map {
    ($dim: literal, $coords: ty, $($var: ident),*) => {
        paste::paste! {
            impl<T: RudaPrimitive> ViewOperations<T, $coords> for TensorMap<T, Tiled> {}
            impl<T: RudaPrimitive> ViewOperationsExpand<T, $coords> for NativeExpand<TensorMap<T, Tiled>> {
                fn __expand_read_method(
                    &self,
                    _scope: &mut Scope,
                    _pos: <$coords as RudaType>::ExpandType,
                ...
                fn __expand_read_masked_method(
                    ...
                    _scope: &mut Scope,
                    _pos: <$coords as RudaType>::ExpandType,
                    _mask_value: <T as RudaType>::ExpandType,
                ) -> <T as RudaType>::ExpandType {
                    unimplemented!("Can't read from tensor map");
                }
                ...
                fn __expand_to_linear_slice_method(
                    ...
                    _scope: &mut Scope,
                    _pos: <$coords as RudaType>::ExpandType,
                    _end: <$coords as RudaType>::ExpandType,
                ) -> SliceExpand<T, ReadOnly> {
                    unimplemented!("Can't read from tensor map");
                }

                fn __expand_shape_method(&self, _scope: &mut Scope) -> <$coords as RudaType>::ExpandType {
                    unimplemented!("Can't read from tensor map");
                }
                ...
                fn __expand_tensor_map_load_method(
                    ...
                    pos: <$coords as RudaType>::ExpandType,
                ) {
            ...
            impl<T: RudaPrimitive> ViewOperationsMut<T, $coords> for TensorMap<T, Tiled> {}
            impl<T: RudaPrimitive> ViewOperationsMutExpand<T, $coords> for NativeExpand<TensorMap<T, Tiled>> {
                fn __expand_write_method(
                    &self,
                    _scope: &mut Scope,
                    _pos: <$coords as RudaType>::ExpandType,
                    _value: <T as RudaType>::ExpandType,
                ) {
...
macro_rules! impl_tensor_map_im2col {
    ($dim: literal, $coords: ty, $($pos: ident),*; $($offs: ident),*) => {
        paste::paste! {
            impl<T: RudaPrimitive> ViewOperations<T, $coords> for TensorMap<T, Im2col> {}
            impl<T: RudaPrimitive> ViewOperationsExpand<T, $coords> for NativeExpand<TensorMap<T, Im2col>> {
                fn __expand_read_method(
                    &self,
                    _scope: &mut Scope,
                    _pos: <$coords as RudaType>::ExpandType,
...
impl<T: RudaPrimitive, N: RudaPrimitive + Coordinates> ViewOperations<T, Sequence<N>>
    for TensorMap<T, Tiled>
{
}
impl<T: RudaPrimitive, N: RudaPrimitive + Coordinates> ViewOperationsExpand<T, Sequence<N>>
...
impl<T: RudaPrimitive, N: RudaPrimitive + Coordinates> ViewOperationsMut<T, Sequence<N>>
    for TensorMap<T, Tiled>
{
}
impl<T: RudaPrimitive, N: RudaPrimitive + Coordinates> ViewOperationsMutExpand<T, Sequence<N>>
    for NativeExpand<TensorMap<T, Tiled>>
{
    fn __expand_write_method(
        &self,
...
impl<T: RudaPrimitive, P: RudaPrimitive + Coordinates, O: RudaPrimitive + Coordinates>
