...
use super::*;
use crate::library::tensor::layout::{Coordinates, VirtualLayout, VirtualLayoutExpand};
use crate::dsl::prelude::*;
use ruda_kernel::dsl::{prelude::barrier::BarrierExpand};

#[derive(RudaType)]
pub struct VirtualView<T: RudaPrimitive, C: Coordinates, S: Coordinates, V: ViewOperations<T, S>> {
    #[allow(unused)]
    view: V,
    #[allow(unused)]
...
#[ruda]
impl<T: RudaPrimitive, C: Coordinates, S: Coordinates, V: ViewOperations<T, S>>
    VirtualView<T, C, S, V>
{
...
#[ruda]
impl<T: RudaPrimitive, C: Coordinates, S: Coordinates, V: ViewOperationsMut<T, S>>
...
macro_rules! impl_virtual_read {
    ($ty: ident, $expand: ident, $trait: ident) => {
        impl<T: RudaPrimitive, C: Coordinates, S: Coordinates, V> Vectorized for $ty<T, C, S, V> where
            V: $trait<T, S>
        {
        }
        impl<T: RudaPrimitive, C: Coordinates, S: Coordinates, V> VectorizedExpand
            for $expand<T, C, S, V>
        where
            V: $trait<T, S>,
        {
        ...
        impl<T: RudaPrimitive, C: Coordinates, S: Coordinates, V> ViewOperations<T, C>
            for $ty<T, C, S, V>
        where
            V: $trait<T, S>,
        {
        ...
        impl<T: RudaPrimitive, C: Coordinates, S: Coordinates, V> ViewOperationsExpand<T, C>
            for $expand<T, C, S, V>
        where
            V: $trait<T, S>,
        {
...
impl<T: RudaPrimitive, C: Coordinates, S: Coordinates, V> ViewOperationsMut<T, C>
...
impl<T: RudaPrimitive, C: Coordinates, S: Coordinates, V> ViewOperationsMutExpand<T, C>
