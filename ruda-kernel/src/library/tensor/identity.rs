use crate::dsl::frontend::TensorBinding;
...
pub fn launch_ref<R: Runtime>(
    ...
    let ruda_dim = RudaDim::new_2d(16, 16);
    let vectors_x = output.shape[1] as u32 / vectorization_factor as u32;
    let ruda_count_x = vectors_x.div_ceil(ruda_dim.x);
    let ruda_count_y = (output.shape[0] as u32).div_ceil(ruda_dim.y);
    let ruda_count = RudaCount::new_2d(ruda_count_x, ruda_count_y);
