...
use ruda_kernel::dsl::{unexpanded};
...
impl<E: Numeric, N: Size, IO: Clone> VirtualTensor<E, N, IO> {
    ...
    ) -> <ComptimeOption<TensorMap<E, Tiled>> as RudaType>::ExpandType {
    ...
    ) -> <Slice<Vector<E, N>> as RudaType>::ExpandType {
        this.__expand_as_slice_method(context, start, end)
    }
    pub fn __expand_shape(
        scope: &mut Scope,
        this: <Self as RudaType>::ExpandType,
        axis: <usize as RudaType>::ExpandType,
...
impl<E: Numeric, N: Size, IO: Clone> VirtualTensorExpand<E, N, IO> {
    ...
    pub fn __expand_rank_method(self, scope: &mut Scope) -> <usize as RudaType>::ExpandType {
    ...
    pub fn __expand_rank(scope: &mut Scope, this: Self) -> <usize as RudaType>::ExpandType {
    ...
    for VirtualTensorExpand<E, N, ReadWrite>
{
    ...
    ) -> SliceExpand<Vector<E, N>, ruda_kernel::dsl::prelude::ReadWrite> {
...
/// Making [virtual tensors](VirtualTensor) a proper [ruda type](RudaType).
mod __ruda_type {
    use super::*;

    impl<E: Numeric, N: Size, IO: Clone> RudaType for VirtualTensor<E, N, IO> {
    ...
    impl<E: Numeric, N: Size, IO> RudaDebug for VirtualTensorExpand<E, N, IO> {}
}
