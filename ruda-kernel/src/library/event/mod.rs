...
type Call<E> =
    Box<dyn Fn(&mut Scope, &Box<dyn Any>, <E as RudaType>::ExpandType, ComptimeEventBusExpand)>;

struct Payload<E: RudaType> {
...
impl ComptimeEventBus {
        ...
        intrinsic!(|_| {
            ...
            let call =
                |scope: &mut crate::dsl::prelude::Scope,
                 listener: &Box<dyn Any>,
                 event: <L::Event as crate::dsl::prelude::RudaType>::ExpandType,
                 bus: <ComptimeEventBus as crate::dsl::prelude::RudaType>::ExpandType| {
    ...
    pub fn event<E: RudaType + 'static>(&mut self, event: E) {
        intrinsic!(|scope| {
