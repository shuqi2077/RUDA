use crate::dsl::{
    ...
    prelude::{assign, expand_erf, expand_hypot, expand_integer_power, expand_rhypot},
};
use ruda_compiler::optimizer::{IrTransformer, TransformAction};
...
#[derive(Debug)]
pub(crate) struct IntegerPowerTransform;

impl IrTransformer for IntegerPowerTransform {
    fn maybe_transform(&self, scope: &mut Scope, inst: &Instruction) -> TransformAction {
        match &inst.operation {
            Operation::Arithmetic(Arithmetic::Powi(op))
                if matches!(op.lhs.elem_type(), ElemType::Float(_)) =>
            {
                let mut scope = scope.child();
                expand_integer_power(&mut scope, op.lhs, op.rhs, inst.out());
