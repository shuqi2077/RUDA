use ruda_core::{e2m1, e2m1x2, e4m3, e5m2, flex32, tf32, ue8m0};
...
pub trait RudaElement: core::fmt::Debug + Send + Sync + 'static + Clone + bytemuck::Pod {
    ...
    /// Element representation for `ruda`.
    fn ruda_type() -> StorageType;
...
pub trait ScalarArgType: RudaElement + Scalar + num_traits::NumCast {}

impl<E: RudaElement + Scalar + num_traits::NumCast> ScalarArgType for E {}

impl RudaElement for u64 {
    fn type_name() -> &'static str {
        "u64"
    }
    fn as_bytes(slice: &[Self]) -> &[u8] {
...
impl RudaElement for half::bf16 {
    fn type_name() -> &'static str {
        "bf16"
    }
    fn as_bytes(slice: &[Self]) -> &[u8] {
