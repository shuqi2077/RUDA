...
pub fn test_stream<R: Runtime, F: Float + RudaElement>(client: ComputeClient<R>) {
    let client_1 = unsafe {
    ...
    for _ in 0..300 {
        ...
        unsafe {
            big_task::launch::<F, R>(
                &client_1,
                RudaCount::Static(len as u32 / 32, 1, 1),
                RudaDim::new_1d(32),
...
macro_rules! testgen_stream {
    () => {
        ...
        fn test_stream() {
            ...
            ruda_kernel::dsl::runtime_tests::stream::test_stream::<TestRuntime, FloatType>(client);
        }
    };
}
