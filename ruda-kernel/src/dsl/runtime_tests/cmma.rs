...
use crate::dsl::{
    prelude::barrier::Barrier, runtime_tests::binary::assert_equals_approx,
};
...
use ruda_core::{e2m1, e2m1x2, ue8m0};
use ruda_core::ir::MatrixIdent;
use ruda_core::ir::features::{MmaConfig, ScaledMmaConfig};
use half::{bf16, f16};
...
pub fn test_simple_1_vectorized<R: Runtime>(client: ComputeClient<R>, ruda_dimensions: RudaDim) {
...
pub fn test_simple_1<R: Runtime>(client: ComputeClient<R>, ruda_dimensions: RudaDim) {
    if !client.features().matmul.cmma.contains(&MmaConfig {
        a_type: ElemType::Float(FloatKind::F16).into(),
        b_type: ElemType::Float(FloatKind::F16).into(),
        cd_type: ElemType::Float(FloatKind::F32).into(),
...
pub fn test_cmma_cast_f16<R: Runtime>(client: ComputeClient<R>, ruda_dimensions: RudaDim) {
    if !client.features().matmul.cmma.contains(&MmaConfig {
        a_type: ElemType::Float(FloatKind::F16).into(),
        b_type: ElemType::Float(FloatKind::F16).into(),
        cd_type: ElemType::Float(FloatKind::F32).into(),
...
pub fn test_cmma_cast_bf16<R: Runtime>(client: ComputeClient<R>, ruda_dimensions: RudaDim) {
    if !client.features().matmul.cmma.contains(&MmaConfig {
...
pub fn test_simple_tf32<R: Runtime>(client: ComputeClient<R>, ruda_dimensions: RudaDim) {
    if !client.features().matmul.cmma.contains(&MmaConfig {
...
pub fn test_cmma_strided<R: Runtime>(client: ComputeClient<R>, ruda_dimensions: RudaDim) {
...
pub fn test_cmma_manual<
    R: Runtime,
    A: RudaElement + Scalar + NumCast,
    B: RudaElement + Scalar + NumCast,
    CD: RudaElement + Numeric,
>(
    client: ComputeClient<R>,
    ruda_dimensions: RudaDim,
...
>(
    ...
    let a_elem = A::ruda_type();
    let b_elem = B::ruda_type();
    ...
    if !client
        ...
        .contains(&ScaledMmaConfig {
            a_type: a_elem,
            b_type: b_elem,
            cd_type: f32::ruda_type(),
            scales_type: S::ruda_type(),
            m: m as u32,
            n: n as u32,
            k: k as u32,
...
macro_rules! testgen_cmma {
    () => {
        ...
        fn test_cmma_simple_1() {
            ...
            let ruda_dimensions = ruda_dim::<TestRuntime>(&client);
            ruda_kernel::dsl::runtime_tests::cmma::test_simple_1::<TestRuntime>(client, ruda_dimensions);
        }
        ...
        fn test_cmma_simple_1_vectorized() {
            let client = TestRuntime::client(&Default::default());
            let ruda_dimensions = ruda_dim::<TestRuntime>(&client);
            ruda_kernel::dsl::runtime_tests::cmma::test_simple_1_vectorized::<TestRuntime>(
                client,
                ruda_dimensions,
            );
        }
        ...
        fn test_cmma_simple_1_vectorized_offset() {
            let client = TestRuntime::client(&Default::default());
            let ruda_dimensions = ruda_dim::<TestRuntime>(&client);
            ruda_kernel::dsl::runtime_tests::cmma::test_simple_1_vectorized_offset::<TestRuntime>(
                client,
                ruda_dimensions,
            );
        }
        ...
        fn test_cmma_simple_tf32() {
            let client = TestRuntime::client(&Default::default());
            let ruda_dimensions = ruda_dim::<TestRuntime>(&client);
            ruda_kernel::dsl::runtime_tests::cmma::test_simple_tf32::<TestRuntime>(
                client,
                ruda_dimensions,
            );
        }
        ...
        fn test_cmma_cast_f16() {
            let client = TestRuntime::client(&Default::default());
            let ruda_dimensions = ruda_dim::<TestRuntime>(&client);
            ruda_kernel::dsl::runtime_tests::cmma::test_cmma_cast_f16::<TestRuntime>(
                client,
                ruda_dimensions,
            );
        }
        ...
        fn test_cmma_cast_bf16() {
            let client = TestRuntime::client(&Default::default());
            let ruda_dimensions = ruda_dim::<TestRuntime>(&client);
            ruda_kernel::dsl::runtime_tests::cmma::test_cmma_cast_bf16::<TestRuntime>(
                client,
                ruda_dimensions,
            );
        }
        ...
        fn test_cmma_strided() {
            let client = TestRuntime::client(&Default::default());
            let ruda_dimensions = ruda_dim::<TestRuntime>(&client);
            ruda_kernel::dsl::runtime_tests::cmma::test_cmma_strided::<TestRuntime>(
                client,
                ruda_dimensions,
            );
        }
        ...
        fn test_cmma_manual() {
            use ruda_core::*;
            use ruda_kernel::dsl::num_traits::cast::NumCast;
            use half::{bf16, f16};
            ...
            >(
                ...
                let client = TestRuntime::client(&Default::default());
                let ruda_dimensions = ruda_dim::<TestRuntime>(&client);
                ruda_kernel::dsl::runtime_tests::cmma::test_cmma_manual::<TestRuntime, A, B, CD>(
                    client,
                    ruda_dimensions,
                    (m, n, k),
                )
        ...
        fn test_cmma_manual_ldmatrix() {
            ...
            fn test<AB: RudaElement + Numeric, CD: RudaElement + Numeric>(
                m: usize,
                n: usize,
                k: usize,
            ) {
                let client = TestRuntime::client(&Default::default());
                let ruda_dimensions = ruda_dim::<TestRuntime>(&client);
                ruda_kernel::dsl::runtime_tests::cmma::test_cmma_manual_ldmatrix::<TestRuntime, AB, CD>(
                    client,
                    ruda_dimensions,
                    (m, n, k),
                )
        ...
        fn test_cmma_scaled() {
            ...
            fn test<A: RudaElement + Scalar + NumCast, B: RudaElement + Scalar + NumCast>(
                m: usize,
                n: usize,
                k: usize,
                factor: usize,
                ...
                let client = TestRuntime::client(&Default::default());
                let ruda_dimensions = ruda_dim::<TestRuntime>(&client);
                ruda_kernel::dsl::runtime_tests::cmma::test_cmma_scaled::<TestRuntime, A, B>(
                    client,
                    ruda_dimensions,
                    (m, n, k),
                    factor,
        ...
        fn test_cmma_scaled_fp4() {
            ...
            fn test(m: usize, n: usize, k: usize, factor: usize) {
                let client = TestRuntime::client(&Default::default());
                let ruda_dimensions = ruda_dim::<TestRuntime>(&client);
                ruda_kernel::dsl::runtime_tests::cmma::test_cmma_scaled_fp4::<TestRuntime>(
                    client,
                    ruda_dimensions,
                    (m, n, k),
                    factor,
        ...
        fn ruda_dim<R: Runtime>(client: &ComputeClient<R>) -> RudaDim {
            let plane_dim = client.properties().hardware.plane_size_max;
            RudaDim::new_1d(plane_dim)
        }
    };
}
