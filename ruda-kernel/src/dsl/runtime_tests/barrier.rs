...
pub fn test_async_copy<R: Runtime, F: Float + RudaElement>(client: ComputeClient<R>) {
    if !client
        .properties()
        .supports_type(OpaqueType::Barrier(ruda_core::ir::BarrierLevel::Unit))
    {
...
fn two_independent_loads<F: Float, N: Size>(
    ...
    let barrier_0 = barrier::Barrier::shared(RUDA_DIM, UNIT_POS == 0);
    let barrier_1 = barrier::Barrier::shared(RUDA_DIM, UNIT_POS == 0);
    // At the Ruda level, we must sync after barrier creation to make sure they
    // exist for all units
    sync_ruda();
...
pub fn test_memcpy_one_load<R: Runtime, F: Float + RudaElement>(client: ComputeClient<R>) {
    if !client
        .properties()
        .supports_type(OpaqueType::Barrier(ruda_core::ir::BarrierLevel::Ruda))
    {
        // We can't execute the test, skip.
        return;
    }
...
pub fn test_memcpy_two_loads<R: Runtime, F: Float + RudaElement>(
    independent: bool,
...
macro_rules! testgen_barrier {
    () => {
        ...
        fn test_barrier_async_copy() {
            ...
            ruda_kernel::dsl::runtime_tests::barrier::test_async_copy::<TestRuntime, FloatType>(client);
        }

        #[$crate::dsl::runtime_tests::test_log::test]
        fn test_barrier_memcpy_async_one_load() {
            let client = TestRuntime::client(&Default::default());
            ruda_kernel::dsl::runtime_tests::barrier::test_memcpy_one_load::<TestRuntime, FloatType>(
                client,
            );
        }
        ...
        fn test_barrier_memcpy_async_two_loads() {
            let client = TestRuntime::client(&Default::default());
            ruda_kernel::dsl::runtime_tests::barrier::test_memcpy_two_loads::<TestRuntime, FloatType>(
                false, client,
            );
        }
