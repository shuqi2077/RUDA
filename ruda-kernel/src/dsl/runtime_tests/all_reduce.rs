    ...
    for (client, handles) in jobs.iter_mut() {
        for handle in handles.iter() {
            client.all_reduce(
                handle.clone(),
                handle.clone(),
                ruda_core::ir::ElemType::Float(ruda_core::ir::FloatKind::F32),
                device_ids.clone(),
                ruda::runtime::server::ReduceOperation::Sum,
            );
        }
...
macro_rules! testgen_all_reduce {
    () => {
        ...
        fn test_all_reduce_sync_collective() {
            ruda_kernel::dsl::runtime_tests::all_reduce::test_all_reduce_sync_collective::<TestRuntime>(
            );
        }
    };
}
