...
use crate::dsl::{IntoRuntime, as_bytes};
use crate::dsl::prelude::*;
use ruda_kernel_macros::RudaTypeMut;
...
#[derive_ruda_comptime]
#[derive(RudaLaunch, RudaType, RudaTypeMut)]
...
#[derive_ruda_comptime]
#[derive(RudaLaunch, RudaType, IntoRuntime)]
#[ruda(runtime_variants)]
...
#[derive_ruda_comptime]
#[derive(RudaLaunch, RudaType, Default, IntoRuntime)]
pub struct BStruct {
    x: i32,
    y: u32,
}
...
pub fn test_array_float_int<R: Runtime, T: RudaPrimitive + RudaElement>(
    ...
    () => {
        mod enums {
            ...
            fn scalar_enum() {
                ...
                ruda_kernel::dsl::runtime_tests::enums::test_scalar_enum::<TestRuntime>(client);
            }

            #[$crate::dsl::runtime_tests::test_log::test]
            fn runtime_enum_empty() {
                let client = TestRuntime::client(&Default::default());
                ruda_kernel::dsl::runtime_tests::enums::test_runtime_variants_empty::<TestRuntime>(
                    client,
                );
            }
            ...
            fn runtime_enum_value() {
                let client = TestRuntime::client(&Default::default());
                ruda_kernel::dsl::runtime_tests::enums::test_runtime_variants_value::<TestRuntime>(
                    client,
                );
            }
            ...
            fn runtime_enum_empty_wildcard() {
                let client = TestRuntime::client(&Default::default());
                ruda_kernel::dsl::runtime_tests::enums::test_runtime_variants_empty_wildcard::<
                    TestRuntime,
                >(client);
            }
            ...
            fn array_float_int() {
                let client = TestRuntime::client(&Default::default());
                ruda_kernel::dsl::runtime_tests::enums::test_array_float_int::<TestRuntime, f32>(
                    &client, 10.0,
                );
                ruda_kernel::dsl::runtime_tests::enums::test_array_float_int::<TestRuntime, i32>(
                    &client, 20,
                );
            }
            ...
            fn tuple_enum() {
                let client = TestRuntime::client(&Default::default());
                ruda_kernel::dsl::runtime_tests::enums::test_tuple_enum::<TestRuntime>(&client);
            }
        }
    };
}
