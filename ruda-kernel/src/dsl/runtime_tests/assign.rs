...
pub fn test_kernel_assign_scalar<R: Runtime, F: Float + RudaElement>(client: ComputeClient<R>) {
...
pub fn test_kernel_add_assign_array<R: Runtime, F: Float + RudaElement>(client: ComputeClient<R>) {
    let handle = client.create_from_slice(F::as_bytes(&[F::new(0.0), F::new(1.0)]));
...
pub fn test_kernel_add_assign_vector<R: Runtime, F: Float + RudaElement>(client: ComputeClient<R>) {
    let handle = client.create_from_slice(F::as_bytes(&[F::new(0.0), F::new(1.0)]));
...
macro_rules! testgen_assign {
    () => {
        ...
        fn test_assign_scalar() {
            ...
            ruda_kernel::dsl::runtime_tests::assign::test_kernel_assign_scalar::<TestRuntime, FloatType>(
                client,
            );
        }
        ...
        fn test_add_assign_array() {
            let client = TestRuntime::client(&Default::default());
            ruda_kernel::dsl::runtime_tests::assign::test_kernel_add_assign_array::<
                TestRuntime,
                FloatType,
            >(client);
        }
        ...
        fn test_add_assign_vector() {
            let client = TestRuntime::client(&Default::default());
            ruda_kernel::dsl::runtime_tests::assign::test_kernel_add_assign_vector::<
                TestRuntime,
                FloatType,
            >(client);
        }
