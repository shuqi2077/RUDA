...
use ruda_core::ir::{Instruction, ManagedVariable, OpaqueType};
...
use super::{
    RudaPrimitive, RudaType, NativeExpand, ReadOnly, ReadWrite, Slice, SliceExpand, SliceMut,
    TensorMap,
};
...
impl RudaPrimitive for Barrier {
    ...
    fn from_const_value(_value: ruda_core::ir::ConstantValue) -> Self {
...
impl NativeAssign for BarrierToken {
    fn elem_init_mut(_scope: &mut crate::dsl::ir::Scope, elem: ManagedVariable) -> ManagedVariable {
        elem
    }
}
    ...
    ($dim: literal, $($arg: expr),*) => {
        paste! {
            impl Barrier {
                ...
                pub fn [<tma_load_ $dim d>]<C1: RudaPrimitive, C2: RudaPrimitive<Scalar = C1::Scalar>>(
                    &self,
                ...
                pub fn [<__expand_tma_load_ $dim d>]<C1: RudaPrimitive, C2: RudaPrimitive<Scalar = C1::Scalar>>(
                    scope: &mut Scope,
            ...
            impl BarrierExpand {
                #[allow(clippy::too_many_arguments)]
                pub fn [<__expand_tma_load_ $dim d_method>]<C1: RudaPrimitive, C2: RudaPrimitive<Scalar = C1::Scalar>>(
                    &self,
                    scope: &mut Scope,
                    source: NativeExpand<TensorMap<C1, Tiled>>,
                    destination: SliceExpand<C2, ReadWrite>,
    ...
    ($dim: literal, $($arg: expr),*; $($offset: expr),*) => {
        paste! {
            impl Barrier {
                ...
                /// the provided offsets.
                #[allow(unused, clippy::too_many_arguments)]
                pub fn [<tma_load_im2col_ $dim d>]<C1: RudaPrimitive, C2: RudaPrimitive<Scalar = C1::Scalar>>(
                    &self,
                ...
                #[allow(clippy::too_many_arguments)]
                pub fn [<__expand_tma_load_im2col_ $dim d>]<C1: RudaPrimitive, C2: RudaPrimitive<Scalar = C1::Scalar>>(
                    scope: &mut Scope,
                    expand: BarrierExpand,
            ...
            impl BarrierExpand {
                #[allow(clippy::too_many_arguments)]
                pub fn [<__expand_tma_load_im2col_ $dim d_method>]<C1: RudaPrimitive, C2: RudaPrimitive<Scalar = C1::Scalar>>(
                    &self,
                    scope: &mut Scope,
                    source: NativeExpand<TensorMap<C1, Im2col>>,
                    destination: SliceExpand<C2, ReadWrite>,
...
impl Barrier {
    ...
    pub fn local() -> Self {
        intrinsic!(|scope| {
            let variable =
                scope.create_local_mut(OpaqueType::Barrier(ruda_core::ir::BarrierLevel::Unit));
    ...
    /// Create a shared memory barrier that can be accesses by all units in the ruda. Initialized
    ...
    ///
    /// If all units in the ruda arrive on the barrier, use `RUDA_DIM` as the arrival count. For
    /// other purposes, only a subset may need to arrive.
    #[allow(unused_variables)]
    pub fn shared(arrival_count: u32, is_elected: bool) -> Shared<Barrier> {
        intrinsic!(|scope| {
            let variable = scope.create_shared(OpaqueType::Barrier(ruda_core::ir::BarrierLevel::Ruda));
            scope.register(BarrierOps::Init {
                barrier: *variable,
    ...
    /// Create a shared memory barrier that can be accesses by all units in the ruda. Only declared,
    /// but not initialized.
    ...
    /// ensure only one unit performs the initialization, and synchronize the ruda afterwards. There
...
impl Barrier {
    ...
    /// make sure source length <= destination length
    #[allow(unused_variables)]
    pub fn memcpy_async_cooperative<C: RudaPrimitive>(
        &self,
        source: &Slice<C>,
    ...
    /// make sure source length <= destination length
    #[allow(unused_variables)]
    pub fn memcpy_async_tx<C: RudaPrimitive>(
        &self,
        source: &Slice<C>,
        destination: &mut SliceMut<C>,
    ) {
...
pub fn copy_async<C: RudaPrimitive>(
    _source: &Slice<C>,
...
pub fn copy_async_checked<C: RudaPrimitive>(
    _source: &Slice<C>,
    _destination: &mut SliceMut<C>,
    _copy_size: u32,
) {
