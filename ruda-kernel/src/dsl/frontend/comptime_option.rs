...
// Separate implementation so we don't need `RudaType` for `ComptimeOption` itself.
// This is important because `&T where T: RudaType` does not necessarily implement
// `RudaType`, but we need to support it in `as_ref`/`as_mut`.
#[derive_expand(RudaType)]
pub enum ComptimeOption<T: RudaType> {
    None,
    Some(T),
}

#[allow(clippy::derivable_impls)]
impl<T: RudaType> Default for ComptimeOptionExpand<T> {
    fn default() -> Self {
        Self::None
    }
}
...
impl<T: RudaType> ComptimeOptionExpand<T> {
    ...
    pub fn unwrap(self) -> T::ExpandType {
        match self {
            Self::Some(val) => val,
            Self::None => panic!("Unwrap on a None RudaOption"),
        }
    }
...
mod impls {
    ...
    mod expand {
        ...
        impl<T: RudaType> ComptimeOptionExpand<T> {
            ...
            where
                U: RudaType + Default + Into<U::ExpandType>,
                F: FnOnce(&mut Scope, T::ExpandType) -> U::ExpandType,
            {
                match self {
                    Some(t) => f(scope, t),
            ...
            where
                T: Deref<Target: RudaType + Sized>,
                T::ExpandType: Deref<Target = <T::Target as RudaType>::ExpandType>,
            {
            ...
            where
                T: DerefMut<Target: RudaType + Sized>,
                T::ExpandType: Deref<Target = <T::Target as RudaType>::ExpandType>,
            {
                self.__expand_map_method(scope, |_, it| (*it).clone())
            }
        ...
        impl<T: RudaType> ComptimeOptionExpand<T> {
            pub fn __expand_is_none_method(self, scope: &mut crate::dsl::prelude::Scope) -> bool {
                !self.__expand_is_some_method(scope)
            }
            pub fn __expand_unwrap_or_method(
                self,
                _scope: &mut crate::dsl::prelude::Scope,
                default: <T as crate::dsl::prelude::RudaType>::ExpandType,
            ) -> <T as crate::dsl::prelude::RudaType>::ExpandType {
                {
                    match self.clone() {
            ...
            pub fn __expand_and_method<U>(
                self,
                scope: &mut crate::dsl::prelude::Scope,
                optb: <Option<U> as crate::dsl::prelude::RudaType>::ExpandType,
            ) -> <Option<U> as crate::dsl::prelude::RudaType>::ExpandType
            where
                U: RudaType,
            {
                {
            ...
            pub fn __expand_or_method(
                self,
                _scope: &mut crate::dsl::prelude::Scope,
                optb: <Option<T> as crate::dsl::prelude::RudaType>::ExpandType,
            ) -> <Option<T> as crate::dsl::prelude::RudaType>::ExpandType {
                {
                    match self.clone() {
            ...
            pub fn __expand_zip_method<U>(
                self,
                scope: &mut crate::dsl::prelude::Scope,
                other: <Option<U> as crate::dsl::prelude::RudaType>::ExpandType,
            ) -> <Option<(T, U)> as crate::dsl::prelude::RudaType>::ExpandType
            where
                U: RudaType,
            {
                {
    ...
    impl<T: RudaType, U: RudaType> ComptimeOptionExpand<(T, U)> {
        pub fn __expand_unzip_method(
            self,
            scope: &mut crate::dsl::prelude::Scope,
        ) -> <(Option<T>, Option<U>) as crate::dsl::prelude::RudaType>::ExpandType {
            {
                match self.clone() {
