use ruda_core::ir::{ConstantValue, Type};

use crate::dsl::frontend::{RudaType, Numeric};
use crate::dsl::ir::{ElemType, IntKind, Scope};
use crate::dsl::prelude::*;

use super::{__expand_new, RudaPrimitive, IntoMut, IntoRuntime, NativeAssign, NativeExpand};
    ...
    + core::cmp::Eq
{
    ...
    fn new(val: i64) -> Self;
    fn __expand_new(scope: &mut Scope, val: i64) -> <Self as RudaType>::ExpandType {
...
macro_rules! impl_int {
    ($type:ident, $kind:ident) => {
        impl RudaType for $type {
            type ExpandType = NativeExpand<Self>;
        }

        impl Scalar for $type {}
        impl RudaPrimitive for $type {
            type Scalar = Self;
            type Size = Const<1>;
