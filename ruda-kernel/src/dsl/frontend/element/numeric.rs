use ruda_core::ir::{ConstantValue, ManagedVariable};
use ruda::runtime::backend::Runtime;
use num_traits::{NumCast, One, Zero};

use crate::dsl::compute::KernelLauncher;
use crate::dsl::{IntoRuntime, ScalarArgType, compute::KernelBuilder};
use crate::dsl::{
    frontend::{Abs, Remainder, VectorSum},
    unexpanded,
};
use crate::dsl::{
    frontend::{RudaPrimitive, RudaType},
    prelude::InputScalar,
};
use crate::dsl::{
    ir::{Scope, Variable},
    ...
    + bytemuck::Zeroable
{
    ...
    fn __expand_min_value(scope: &mut Scope) -> <Self as RudaType>::ExpandType {
    ...
    fn __expand_max_value(scope: &mut Scope) -> <Self as RudaType>::ExpandType {
        let elem = Self::as_type(scope).elem_type();
...
/// trait.
pub trait ScalarArgSettings: Send + Sync + RudaPrimitive {
...
pub trait ZeroExpand: RudaType + Zero {
    fn __expand_zero(scope: &mut Scope) -> Self::ExpandType;
}

pub trait OneExpand: RudaType + One {
    fn __expand_one(scope: &mut Scope) -> Self::ExpandType;
}

impl<T: RudaType + Zero + IntoRuntime> ZeroExpand for T {
    fn __expand_zero(scope: &mut Scope) -> Self::ExpandType {
        T::zero().__expand_runtime_method(scope)
    }
}

impl<T: RudaType + One + IntoRuntime> OneExpand for T {
    fn __expand_one(scope: &mut Scope) -> Self::ExpandType {
