use super::{RudaPrimitive, Numeric};
use crate::dsl::{
...
use ruda_core::{e2m1, e2m1x2, e2m3, e3m2, e4m3, e5m2, flex32, tf32, ue8m0};
use ruda_core::ir::{ManagedVariable, VectorSize};
use ruda::runtime::backend::Runtime;
use half::{bf16, f16};
...
/// the generated code.
#[diagnostic::on_unimplemented(note = "Consider using `#[derive(RudaType)]` on `{Self}`")]
pub trait RudaType {
    type ExpandType: Clone + IntoMut + RudaDebug;
}

pub trait RudaEnum: Sized {
    type RuntimeValue: Clone + RudaDebug;
...
impl<T: RudaPrimitive> Assign for T {
...
pub trait IntoRuntime: RudaType + Sized {
...
/// Note that a type doesn't need to implement `RudaComptime` to be used as
/// a comptime argument. However, this facilitate the declaration of generic ruda types.
///
/// # Example
///
/// ```ignore
...
/// }
/// ```
pub trait RudaComptime: core::fmt::Debug + core::hash::Hash + Eq + Clone + Copy {}
impl<T> RudaComptime for T where T: core::fmt::Debug + core::hash::Hash + Eq + Clone + Copy {}
...
/// as an output.
#[diagnostic::on_unimplemented(note = "Consider using `#[derive(RudaLaunch)]` on `{Self}`")]
pub trait LaunchArg: RudaType + Send + Sync + 'static {
...
macro_rules! launch_tuple {
    ($(($T:ident, $t:ident)),*) => {
        impl<$($T: LaunchArg),*> LaunchArg for ($($T),*) {
            ...
            fn expand(arg: &Self::CompilationArg, builder: &mut KernelBuilder) -> ($(<$T as RudaType>::ExpandType),*) {
                let ($($t),*) = arg;
                ($($T::expand($t, builder)),*)
            }

            fn expand_output(arg: &Self::CompilationArg, builder: &mut KernelBuilder) -> ($(<$T as RudaType>::ExpandType),*) {
                let ($($t),*) = arg;
...
#[derive(new)]
pub struct NativeExpand<T: RudaType> {
    pub expand: ManagedVariable,
    pub(crate) _type: PhantomData<T>,
}

impl<T: RudaType> NativeExpand<T> {
...
impl<T: RudaType> From<&NativeExpand<T>> for NativeExpand<T> {
    fn from(value: &NativeExpand<T>) -> Self {
        value.clone()
    }
}

impl<T: RudaType> From<NativeExpand<T>> for Variable {
    fn from(value: NativeExpand<T>) -> Self {
        value.expand.into()
    }
}

impl<T: RudaType> From<&mut NativeExpand<T>> for NativeExpand<T> {
...
macro_rules! tuple_ruda_type {
    ($($P:ident),*) => {
        impl<$($P: RudaType),*> RudaType for ($($P,)*) {
...
macro_rules! tuple_debug {
    ($($P:ident),*) => {
        impl<$($P: RudaDebug),*> RudaDebug for ($($P,)*) {}
    }
}
...
all_tuples!(tuple_ruda_type, 0, 12, P);
...
impl<T: RudaType> RudaDebug for NativeExpand<T> {
    fn set_debug_name(&self, scope: &mut Scope, name: &'static str) {
...
impl<T: RudaType> RudaDebug for &mut NativeExpand<T> {
    fn set_debug_name(&self, scope: &mut Scope, name: &'static str) {
        scope.update_variable_name(*self.expand, name);
    }
}
...
impl<T: RudaType> Clone for NativeExpand<T> {
    fn clone(&self) -> Self {
        Self {
...
impl<T: RudaType> From<ManagedVariable> for NativeExpand<T> {
...
impl<T: RudaType> From<NativeExpand<T>> for ManagedVariable {
    fn from(value: NativeExpand<T>) -> Self {
        value.expand
    }
}

impl<T: RudaPrimitive> NativeExpand<T> {
...
impl<T: RudaDebug> RudaDebug for Vec<T> {}
...
pub trait DefaultExpand: RudaType {
    fn __expand_default(scope: &mut Scope) -> Self::ExpandType;
}

impl<T: RudaType + Default + IntoRuntime> DefaultExpand for T {
...
macro_rules! define_scalar {
    ($vis: vis $name: ident) => {
        $crate::dsl::__private::paste! {
            $vis struct [<__ $name>];
            $vis type $name = $crate::dsl::prelude::DynamicScalar<[<__ $name>]>;
        }
    };
}
...
macro_rules! define_size {
    ($vis: vis $name: ident) => {
        $crate::dsl::__private::paste! {
            $vis struct [<__ $name>];
            $vis type $name = $crate::dsl::prelude::DynamicSize<[<__ $name>]>;
        }
    };
}
