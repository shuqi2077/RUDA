...
use crate::dsl::{
    Assign, IntoRuntime,
    prelude::{Const, RudaDebug, IntoMut, Size},
};
use ruda_core::ir::{ConstantValue, ManagedVariable, StorageType, Type, features::TypeUsage};
use ruda_kernel_macros::{comptime_type, ruda, intrinsic};
use ruda::runtime::{client::ComputeClient, backend::Runtime};
use enumset::EnumSet;

use crate::dsl::frontend::RudaType;
use crate::dsl::ir::Scope;

use super::{NativeAssign, NativeExpand};

/// Form of `RudaType` that encapsulates all primitive types:
/// Numeric, `UInt`, Bool
...
impl<T: RudaPrimitive> RudaPrimitiveExpand for NativeExpand<T> {
    type Scalar = NativeExpand<T::Scalar>;
    type WithScalar<S: Scalar> = NativeExpand<T::WithScalar<S>>;
}

/// Marker trait for scalar primitives. Should be implemented for all scalar `RudaPrimitive`s, but
/// **not** for `Vector` or non-standard primitives like `Barrier`. Alternatively, treat these as
/// types that can be stored in a [`Vector`]
pub trait Scalar:
    RudaPrimitive<Scalar = Self, Size = Const<1>> + Default + IntoRuntime + Debug
{
}

#[ruda]
pub fn type_of<E: RudaPrimitive>() -> comptime_type!(Type) {
    intrinsic!(|scope| { E::as_type(scope) })
}

#[ruda]
pub fn storage_type_of<E: RudaPrimitive>() -> comptime_type!(StorageType) {
