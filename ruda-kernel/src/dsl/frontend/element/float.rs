use ruda_core::ir::{ConstantValue, Scope, StorageType, Type};
use half::{bf16, f16};
    ...
    + core::cmp::PartialEq
{
    ...
    fn new(val: f32) -> Self;
    fn __expand_new(scope: &mut Scope, val: f32) -> <Self as RudaType>::ExpandType {
        __expand_new(scope, val)
    }
}

#[ruda]
pub trait FloatOps: RudaPrimitive + PartialOrd + Sized {
    fn min(self, other: Self) -> Self {
        crate::dsl::prelude::min(self, other)
    }

    fn max(self, other: Self) -> Self {
        crate::dsl::prelude::max(self, other)
    }
...
impl<T: FloatOps + RudaPrimitive> FloatOpsExpand for NativeExpand<T> {
