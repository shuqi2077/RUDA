...
use ruda_core::ir::RudaFnSource;

use crate::dsl::ir::{NonSemantic, Scope, Variable};
...
pub fn debug_source_expand(
    ...
    scope.update_source(RudaFnSource {
...
#[track_caller]
pub fn debug_var_expand<E: RudaDebug>(scope: &mut Scope, name: &'static str, expand: E) -> E {
...
macro_rules! debug_print {
    ...
    ($format:literal, $($args:expr,)*) => {
        $crate::dsl::debug_print!($format, $($args),*);
    };
}
...
macro_rules! debug_print_expand {
    ($scope:expr, $format:expr, $($args:expr),*) => {
        {
            let args = $crate::dsl::__private::vec![$(*$crate::dsl::ir::ManagedVariable::from($args)),*];
            $crate::dsl::frontend::printf_expand($scope, $format, args);
        }
    };
    ($format:literal, $($args:expr,)*) => {
        $crate::dsl::debug_print_expand!($format, $($args),*)
    };
}

pub mod ruda_comment {
    use alloc::string::ToString;

    use crate::dsl::ir::NonSemantic;
