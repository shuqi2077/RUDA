use ruda_core::ir::{Bitwise, ManagedVariable, Operator, Scope};
...
pub mod cast {
    ...
    use crate::dsl::prelude::NativeExpand;

    use self::ir::UnaryOperator;

    use super::*;

    pub fn expand<From: RudaType, To: RudaType>(
        scope: &mut Scope,
...
pub mod assign {
    ...
    /// [`expand_no_check()`].
    pub fn expand<C: RudaType>(scope: &mut Scope, input: NativeExpand<C>, output: NativeExpand<C>) {
    ...
    pub fn expand_no_check<C: RudaType>(
        scope: &mut Scope,
...
pub mod index_assign {
    ...
    pub fn expand<A: RudaIndexMutExpand<Output = NativeExpand<V>>, V: RudaPrimitive>(
        scope: &mut Scope,
        expand: A,
        index: A::Idx,
    ...
    macro_rules! impl_index {
        ($type:ident) => {
            impl<E: RudaPrimitive> RudaIndexMut for $type<E> {}

            impl<E: RudaPrimitive> RudaIndexMutExpand for NativeExpand<$type<E>> {
                fn expand_index_mut(
                    self,
                    scope: &mut Scope,
    ...
    impl<E: Scalar, N: Size> RudaIndexMut for Vector<E, N> {}

    impl<E: Scalar, N: Size> RudaIndexMutExpand for NativeExpand<Vector<E, N>> {
        fn expand_index_mut(
            self,
            scope: &mut Scope,
            index: NativeExpand<usize>,
...
pub mod index {
    ...
    pub fn expand<A: RudaIndexExpand<Output = NativeExpand<V>>, V: RudaType>(
        scope: &mut Scope,
        expand: A,
        index: A::Idx,
    ) -> NativeExpand<V> {
    ...
    pub fn expand_with<A: RudaIndexExpand<Output = NativeExpand<V>>, V: RudaType>(
        scope: &mut Scope,
        expand: A,
        index: A::Idx,
    ) -> NativeExpand<V> {
    ...
    macro_rules! impl_index {
        ($type:ident) => {
            impl<E: RudaPrimitive> RudaIndex for $type<E> {
                type Output = E;
                type Idx = usize;
            }

            impl<E: RudaPrimitive> RudaIndexExpand for NativeExpand<$type<E>> {
    ...
    impl<E: Scalar, N: Size> RudaIndex for Vector<E, N> {
        type Output = E;
        type Idx = usize;
    }
    impl<E: Scalar, N: Size> RudaIndexExpand for NativeExpand<Vector<E, N>> {
        type Output = NativeExpand<E>;
        type Idx = NativeExpand<usize>;
...
pub mod index_unchecked {
    use super::*;
    use crate::dsl::prelude::{RudaIndexExpand, NativeExpand};
...
pub mod add_assign_array_op {
    ...
    use super::*;
    use crate::dsl::prelude::{RudaType, NativeExpand, array_assign_binary_op_expand};

    pub fn expand<A: RudaType + RudaIndex>(
        scope: &mut Scope,
...
pub mod add_assign_op {
    ...
    use crate::dsl::{
        ...
        prelude::{RudaType, NativeExpand},
    };

    use super::*;

    pub fn expand<C: RudaType + AddAssign>(
        scope: &mut Scope,
        lhs: NativeExpand<C>,
        rhs: NativeExpand<C>,
    ) -> NativeExpand<C> {
...
pub mod sub_assign_op {
    use self::ir::Arithmetic;
    use super::*;
    use crate::dsl::{frontend::operation::base::assign_op_expand, prelude::NativeExpand};

    pub fn expand<C: RudaType>(
        scope: &mut Scope,
        lhs: NativeExpand<C>,
        rhs: NativeExpand<C>,
    ) -> ManagedVariable {
...
pub mod add_assign {
    use ruda_core::ir::Arithmetic;

    use super::*;
    use crate::dsl::prelude::{RudaPrimitive, NativeExpand, assign_op_expand};
