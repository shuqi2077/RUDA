use core::ops::Not;
use ruda_core::{e2m1, e2m1x2, e4m3, e5m2, ue8m0};
use ruda_core::ir::{Bitwise, Comparison, Operator};
use half::{bf16, f16};

use crate::dsl::{
    flex32,
    ir::{Arithmetic, ManagedVariable, Scope},
    prelude::{RudaPrimitive, RudaPrimitiveExpand, RudaType, NativeExpand, Reinterpret},
    tf32, unexpanded,
};
...
pub mod not {
    ...
    pub fn expand<T: RudaNot>(scope: &mut Scope, x: NativeExpand<T>) -> NativeExpand<T> {
...
pub mod neg {
    ...
    pub fn expand<E: RudaPrimitive>(scope: &mut Scope, x: NativeExpand<E>) -> NativeExpand<E> {
...
macro_rules! impl_unary_func {
    ($trait_name:ident, $method_name:ident, $operator:expr, $($type:ty),*) => {
        paste::paste! {
            ...
            impl<T: $trait_name + RudaPrimitive> [<$trait_name Expand>] for NativeExpand<T> {
                fn [<__expand_ $method_name _method>](self, scope: &mut Scope) -> Self {
...
macro_rules! impl_unary_func_fixed_out_ty {
    ($trait_name:ident, $method_name:ident, $out_ty: ty, $operator:expr, $($type:ty),*) => {
        paste::paste! {
            pub trait $trait_name: RudaPrimitive + RudaType<ExpandType: [<$trait_name Expand>]
            + RudaPrimitiveExpand<WithScalar<$out_ty> = NativeExpand<Self::WithScalar<$out_ty>>>> + Sized {
            ...
            impl<T: $trait_name + RudaPrimitive> [<$trait_name Expand>] for NativeExpand<T> {
                fn [<__expand_ $method_name _method>](self, scope: &mut Scope) -> Self::WithScalar<$out_ty> {
                    let expand_element: ManagedVariable = self.into();
                    let item = <$out_ty as RudaPrimitive>::as_type(scope).with_vector_size(expand_element.ty.vector_size());
                    unary_expand_fixed_output(scope, expand_element, item, $operator).into()
                }
            }
        }
...
macro_rules! impl_not {
    ($trait_name:ident, $method_name:ident, $($type:ty),*) => {
        paste::paste! {
            pub trait [<Ruda $trait_name>]: $trait_name<Output = Self> + RudaPrimitive + RudaType<ExpandType: [<$trait_name Expand>]> {
                fn [<__expand_ $method_name>](scope: &mut Scope, x: NativeExpand<Self>) -> NativeExpand<Self> {
                    x.[<__expand_ $method_name _method>](scope)
                }
            }
            ...
            $(impl [<Ruda $trait_name>] for $type {})*
            impl<T: [<Ruda $trait_name>] + RudaPrimitive> [<$trait_name Expand>] for NativeExpand<T> {
                fn [<__expand_ $method_name _method>](self, scope: &mut Scope) -> Self {
...
pub trait FloatBits:
    RudaPrimitive + RudaType<ExpandType: FloatBitsExpand<Bits = Self::Bits>>
{
    type Bits: RudaPrimitive;
