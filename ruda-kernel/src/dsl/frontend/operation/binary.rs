use crate::dsl::ir::{Arithmetic, Bitwise, ManagedVariable, Operator, Scope};
use crate::dsl::{
    flex32,
    frontend::{RudaPrimitive, NativeExpand},
    prelude::*,
};
use crate::dsl::{frontend::RudaType, tf32};
use crate::dsl::{
    frontend::operation::base::{binary_expand, binary_expand_fixed_output},
    unexpanded,
};
use core::{cmp::Ordering, ops::*};
use ruda_core::{e2m1, e4m3, e5m2, ue8m0};
use ruda_core::ir::ClampOperator;
use ruda_kernel_macros::derive_expand;
use half::{bf16, f16};
...
pub mod clamp {
    ...
    pub fn expand<C: PartialOrd + RudaPrimitive>(
        scope: &mut Scope,
...
pub fn min<T: PartialOrd + RudaPrimitive>(lhs: T, rhs: T) -> T {
    clamp_max(lhs, rhs)
}
...
pub fn max<T: PartialOrd + RudaPrimitive>(lhs: T, rhs: T) -> T {
    clamp_min(lhs, rhs)
}
...
macro_rules! impl_binary_func {
    ($trait_name:ident, $method_name:ident, $operator:expr, $($type:ty),*) => {
        paste::paste! {
            pub trait $trait_name: RudaPrimitive + RudaType<ExpandType: [<$trait_name Expand>]> + Sized {
            ...
            impl<T: RudaPrimitive + $trait_name> [<$trait_name Expand>] for NativeExpand<T> {
                fn [<__expand_ $method_name _method>](self, scope: &mut Scope, rhs: Self) -> Self {
...
macro_rules! impl_binary_func_scalar_out {
    ($trait_name:ident, $method_name:ident, $operator:expr, $($type:ty),*) => {
        paste::paste! {
            pub trait $trait_name: RudaPrimitive
                + RudaType<ExpandType: [<$trait_name Expand>]
                + RudaPrimitiveExpand<Scalar = NativeExpand<Self::Scalar>>>
                + Sized {
            ...
            pub trait [<$trait_name Expand>]: RudaPrimitiveExpand {
...
macro_rules! impl_binary_func_mixed_types {
    ($trait_name:ident, $method_name:ident, $rhs_ty: ident, $operator:expr, $($type:ty),*) => {
        paste::paste! {
            pub trait $trait_name<Rhs: RudaPrimitive + RudaType<ExpandType: Into<ManagedVariable>> + Sized>:
                RudaPrimitive + RudaType<ExpandType: [<$trait_name Expand>]<Rhs>> + Sized {
            ...
            pub trait [<$trait_name Expand>]<Rhs: RudaType>{
                fn [<__expand_ $method_name _method>](self, scope: &mut Scope, rhs: Rhs::ExpandType) -> Self;
            }

            $(impl $trait_name<$rhs_ty> for $type {})*
            impl<Rhs: RudaPrimitive, T: RudaPrimitive + $trait_name<Rhs>> [<$trait_name Expand>]<Rhs> for NativeExpand<T> {
...
macro_rules! impl_core_binop {
    ($trait: ident, $method: ident, $op: expr) => {
        paste::paste! {
            pub trait [<Ruda $trait>]: $trait<Output = Self> + RudaPrimitive + RudaType<ExpandType: [<$trait Expand>]> + Sized {
                fn [<__expand_ $method>](
                    scope: &mut Scope,
                    lhs: NativeExpand<Self>,
                    rhs: NativeExpand<Self>,
            ...
            impl<T: $trait<Output = T> + RudaPrimitive> [<Ruda $trait>] for T {}
            impl<T: $trait<Output = T> + RudaPrimitive> [<$trait Expand>] for NativeExpand<T> {
                fn [<__expand_ $method _method>](self, scope: &mut Scope, rhs: Self) -> Self {
...
macro_rules! impl_core_assign_binop {
    ($trait: ident, $method: ident, $op: expr) => {
        paste::paste! {
            pub trait [<Ruda $trait>]: $trait + RudaPrimitive + RudaType<ExpandType: [<$trait Expand>]> + Sized {
                fn [<__expand_ $method>](
                    scope: &mut Scope,
                    lhs: NativeExpand<Self>,
                    rhs: NativeExpand<Self>,
            ...
            impl<T: $trait + RudaPrimitive> [<Ruda $trait>] for T {}
            impl<T: $trait + RudaPrimitive> [<$trait Expand>] for NativeExpand<T> {
                fn [<__expand_ $method _method>](self, scope: &mut Scope, rhs: Self) {
...
#[derive_expand(RudaType, RudaTypeMut, IntoRuntime)]
#[ruda(runtime_variants, no_constructors)]
pub enum Ordering {
    Less = -1,
    Equal = 0,
    Greater = 1,
...
pub trait RudaOrd: Ord + RudaType<ExpandType: OrdExpand> + Sized {
    fn __expand_cmp(
        scope: &mut Scope,
        lhs: Self::ExpandType,
        rhs: Self::ExpandType,
...
impl<T: Ord + RudaPrimitive> RudaOrd for T {}
impl<T: Ord + RudaPrimitive> OrdExpand for NativeExpand<T> {
    fn __expand_cmp_method(self, scope: &mut Scope, rhs: Self) -> OrderingExpand {
