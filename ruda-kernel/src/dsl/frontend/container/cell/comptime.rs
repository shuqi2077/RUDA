...
use crate::dsl::prelude::{RudaDebug, RudaType, IntoMut};

#[derive(Debug, Clone)]
/// A cell that can store and mutate a ruda type during comptime.
pub struct ComptimeCell<T: RudaType> {
    pub(super) value: Rc<RefCell<T>>,
}

/// Expand type of [`ComptimeCell`].
pub struct ComptimeCellExpand<T: RudaType> {
...
impl<T: RudaType + Clone> ComptimeCell<T> {
...
impl<T: RudaType + Clone> ComptimeCellExpand<T> {
...
impl<T: RudaType> IntoMut for ComptimeCellExpand<T> {
    fn into_mut(self, _scope: &mut Scope) -> Self {
        self
    }
}

impl<T: RudaType> RudaDebug for ComptimeCellExpand<T> {}

impl<T: RudaType> Clone for ComptimeCellExpand<T> {
    fn clone(&self) -> Self {
        Self {
