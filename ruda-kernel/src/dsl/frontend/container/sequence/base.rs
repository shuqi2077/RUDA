...
use crate::dsl::{
    frontend::{RudaType, IntoMut, NativeExpand, branch::Iterable},
    prelude::{RudaDebug, RudaIndex, RudaIndexExpand},
};
use alloc::rc::Rc;
use core::{cell::RefCell, ops::Deref};

/// A sequence of [ruda types](RudaType) that is inlined during compilation.
///
...
pub struct Sequence<T: RudaType> {
    values: Vec<T>,
}

impl<T: RudaType> Default for Sequence<T> {
    fn default() -> Self {
        Self::new()
    }
}

impl<T: RudaType> IntoMut for Sequence<T> {
    fn into_mut(self, _scope: &mut Scope) -> Self {
        self
    }
}
impl<T: RudaType> RudaDebug for Sequence<T> {}

impl<T: RudaType + Clone> Sequence<T> {
...
impl<T: RudaType> RudaIndex for Sequence<T> {
    type Output = T;
    type Idx = usize;
}

impl<T: RudaType> Deref for Sequence<T> {
    type Target = [T];
...
impl<T: RudaType> RudaIndexExpand for SequenceExpand<T> {
...
pub struct SequenceExpand<T: RudaType> {
...
impl<T: RudaType> Iterable<T> for SequenceExpand<T> {
    fn expand(self, scope: &mut Scope, func: impl FnMut(&mut Scope, <T as RudaType>::ExpandType)) {
    ...
    fn expand_unroll(
        self,
        scope: &mut Scope,
        mut func: impl FnMut(&mut Scope, <T as RudaType>::ExpandType),
    ) {
        for elem in self {
            func(scope, elem);
        }
...
impl<T: RudaType> IntoMut for SequenceExpand<T> {
...
impl<T: RudaType> RudaDebug for SequenceExpand<T> {}

impl<T: RudaType> Clone for SequenceExpand<T> {
    fn clone(&self) -> Self {
        Self {
...
impl<T: RudaType> IntoIterator for Sequence<T> {
    type Item = T;
...
impl<T: RudaType> IntoIterator for SequenceExpand<T> {
...
impl<T: RudaType> SequenceExpand<T> {
