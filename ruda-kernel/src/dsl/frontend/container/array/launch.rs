...
use crate::dsl::{
    ...
    ir::Id,
    prelude::{RudaPrimitive, LaunchArg, NativeExpand, TensorBinding},
};
...
/// Tensor representation with a reference to the [server handle](ruda::runtime::server::Handle).
pub struct ArrayBinding<R: Runtime> {
    pub handle: ruda::runtime::server::Binding,
...
impl<R: Runtime> ArrayArg<R> {
    ...
    pub unsafe fn from_raw_parts(handle: ruda::runtime::server::Handle, length: usize) -> Self {
        unsafe {
            ArrayArg::Handle {
    ...
    pub unsafe fn from_raw_parts_binding(
        binding: ruda::runtime::server::Binding,
        length: usize,
    ) -> Self {
        unsafe {
            ArrayArg::Handle {
...
impl<R: Runtime> ArrayBinding<R> {
    ...
    pub unsafe fn from_raw_parts_binding(
        handle: ruda::runtime::server::Binding,
        length: usize,
    ) -> Self {
        Self {
            handle,
...
impl<C: RudaPrimitive> LaunchArg for Array<C> {
