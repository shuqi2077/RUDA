...
use ruda_core::ir::{ManagedVariable, Scope, VectorSize};

use crate::dsl::frontend::{RudaPrimitive, NativeExpand};
...
mod new {
    ...
    use super::*;
    use crate::dsl::ir::Variable;

    #[ruda]
    impl<T: RudaPrimitive + Clone> Array<T> {
    ...
    impl<T: RudaPrimitive + Clone> Array<T> {
        ...
        #[allow(unused_variables)]
        pub fn from_data<C: RudaPrimitive>(data: impl IntoIterator<Item = C>) -> Self {
            intrinsic!(|scope| {
                scope
        ...
        pub fn __expand_from_data<C: RudaPrimitive>(
            scope: &mut Scope,
            data: ArrayData<C>,
    ...
    impl<C: RudaPrimitive + Into<NativeExpand<C>>, T: IntoIterator<Item = C>> From<T> for ArrayData<C> {
...
mod vector {
    ...
    impl<P: RudaPrimitive> Array<P> {
        ...
        pub fn __expand_vector_size(
            expand: <Self as RudaType>::ExpandType,
            scope: &mut Scope,
        ) -> VectorSize {
...
mod metadata {
    use crate::dsl::{ir::Instruction, prelude::expand_length_native};

    use super::*;

    #[ruda]
    impl<E: RudaType> Array<E> {
...
impl<C: RudaType> IntoMut for NativeExpand<Array<C>> {
...
impl<T: RudaPrimitive> SizedContainer for Array<T> {
    type Item = T;
}

impl<T: RudaType> Iterator for &Array<T> {
    type Item = T;
...
impl<T: RudaPrimitive> List<T> for Array<T> {
    fn __expand_read(
        scope: &mut Scope,
...
impl<T: RudaPrimitive> Deref for Array<T> {
    type Target = [T];
...
impl<T: RudaPrimitive> DerefMut for Array<T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        unexpanded!()
    }
}

impl<T: RudaPrimitive> ListExpand<T> for NativeExpand<Array<T>> {
...
impl<T: RudaPrimitive> Vectorized for Array<T> {}
impl<T: RudaPrimitive> VectorizedExpand for NativeExpand<Array<T>> {
    fn vector_size(&self) -> VectorSize {
        self.expand.ty.vector_size()
    }
}

impl<T: RudaPrimitive> ListMut<T> for Array<T> {
    fn __expand_write(
        scope: &mut Scope,
        this: NativeExpand<Array<T>>,
        idx: NativeExpand<usize>,
...
impl<T: RudaPrimitive> ListMutExpand<T> for NativeExpand<Array<T>> {
