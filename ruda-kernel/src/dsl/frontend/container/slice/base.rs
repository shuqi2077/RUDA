...
use ruda_core::ir::{Branch, ElemType, FloatKind, ManagedVariable, RangeLoop, Variable, VectorSize};
...
#[derive(Clone, Copy)]
pub struct Slice<E: RudaPrimitive, IO: SliceVisibility = ReadOnly> {
    _e: PhantomData<E>,
    _io: PhantomData<IO>,
...
#[derive(RudaType)]
pub enum SliceOrigin<E: RudaPrimitive> {
    Tensor(Tensor<E>),
    Array(Array<E>),
    SharedMemory(SharedMemory<E>),
}

impl<E: RudaPrimitive> SliceOriginExpand<E> {
...
impl<E: RudaPrimitive, IO: SliceVisibility> Iterator for Slice<E, IO> {
    type Item = E;
...
pub struct SliceExpand<E: RudaPrimitive, IO: SliceVisibility> {
...
impl<E: Scalar, N: Size, IO: SliceVisibility> Slice<Vector<E, N>, IO> {
    ...
    /// # Warning
    ///
    /// Currently, this only work with `ruda(launch_unchecked)` and is not supported on wgpu.
    ...
    pub fn with_vector_size<N2: Size>(&self) -> Slice<Vector<E, N2>, IO> {
        intrinsic!(|scope| {
            ...
            if current < vector_size {
                ...
                let length = crate::dsl::frontend::div::expand(scope, self.length, ratio.into());
                let offset = crate::dsl::frontend::div::expand(scope, self.offset, ratio.into());
                out.length = length;
                out.offset = offset;
            } else {
                let ratio = current / vector_size;
                let length = crate::dsl::frontend::mul::expand(scope, self.length, ratio.into());
                let offset = crate::dsl::frontend::mul::expand(scope, self.offset, ratio.into());
                out.length = length;
                out.offset = offset;
            }
...
#[ruda]
impl<E: RudaPrimitive, IO: SliceVisibility> Slice<E, IO> {
    ...
    pub fn downcast<T: RudaPrimitive>(&self) -> Slice<T, IO> {
        intrinsic!(|scope| {
    ...
    pub unsafe fn downcast_unchecked<T: RudaPrimitive>(&self) -> Slice<T, IO> {
        intrinsic!(|scope| {
            SliceExpand::<T, IO> {
                origin: self.origin.cast_unchecked(),
                io: self.io.clone(),
...
#[ruda]
impl<E: RudaPrimitive> Slice<E, ReadOnly> {
...
impl<E: RudaPrimitive> SliceOriginExpand<E> {
    fn cast_unchecked<T: RudaPrimitive>(self) -> SliceOriginExpand<T> {
        match self {
...
impl<E: RudaPrimitive, IO: SliceVisibility> Slice<E, IO> {
    ...
    pub fn __expand_new_expand(
        ...
        let length = crate::dsl::frontend::sub::expand(scope, end, start.clone());
...
impl<E: RudaPrimitive, IO: SliceVisibility> IntoMut for SliceExpand<E, IO> {
    fn into_mut(self, _scope: &mut ruda_core::ir::Scope) -> Self {
        self
    }
}
...
// TODO: Fix
impl<E: RudaPrimitive> SizedContainer for Slice<E, ReadOnly> {
    type Item = E;
}
...
impl<E: RudaPrimitive> Iterable<E> for SliceExpand<E, ReadOnly> {
    fn expand(
        self,
        scope: &mut Scope,
        mut body: impl FnMut(&mut Scope, <E as RudaType>::ExpandType),
    ) {
...
impl<E: RudaPrimitive, IO: SliceVisibility> RudaIndex for Slice<E, IO> {
    type Output = E;
    type Idx = usize;
...
impl<E: RudaPrimitive, IO: SliceVisibility> List<E> for Slice<E, IO> {}
impl<E: RudaPrimitive, IO: SliceVisibility> ListExpand<E> for SliceExpand<E, IO> {
    fn __expand_read_method(
        &self,
        scope: &mut ruda_core::ir::Scope,
        index: NativeExpand<usize>,
...
impl<T: RudaPrimitive, IO: SliceVisibility> Deref for Slice<T, IO> {
    type Target = [T];
...
impl<T: RudaPrimitive> DerefMut for Slice<T, ReadWrite> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        unexpanded!()
    }
}

impl<E: RudaPrimitive, IO: SliceVisibility> Vectorized for Slice<E, IO> {}
...
impl<E: RudaPrimitive> RudaIndexMut for Slice<E, ReadWrite> {
    fn expand_index_mut(
        scope: &mut Scope,
        array: Self::ExpandType,
        index: NativeExpand<usize>,
...
impl<E: RudaPrimitive> ListMut<E> for Slice<E, ReadWrite> {}
...
mod read_offset {
    ...
    pub fn expand<E: RudaPrimitive>(
        scope: &mut crate::dsl::prelude::Scope,
        origin: SliceOriginExpand<E>,
        offset: <usize as crate::dsl::prelude::RudaType>::ExpandType,
        index: <usize as crate::dsl::prelude::RudaType>::ExpandType,
        vector_size: Option<VectorSize>,
        checked: bool,
    ) -> <E as crate::dsl::prelude::RudaType>::ExpandType {
        let index = crate::dsl::frontend::add::expand(scope, offset, index);
...
mod write_offset {
    ...
    pub fn expand<E: RudaPrimitive>(
        ...
        offset: <usize as crate::dsl::prelude::RudaType>::ExpandType,
        index: <usize as crate::dsl::prelude::RudaType>::ExpandType,
        value: <E as crate::dsl::prelude::RudaType>::ExpandType,
        vector_size: Option<VectorSize>,
    ) {
        let index = crate::dsl::frontend::add::expand(scope, offset, index);
