use crate::dsl::{
    frontend::{RudaPrimitive, RudaType, NativeExpand, SizedContainer},
    ir::{Metadata, Scope},
    prelude::*,
    unexpanded,
};
...
/// The tensor type is similar to the [array type](crate::dsl::prelude::Array), however it comes with more
/// metadata such as [stride](Tensor::stride) and [shape](Tensor::shape).
#[derive(new, Clone, Copy)]
pub struct Tensor<T: RudaType> {
    _val: PhantomData<T>,
}
...
impl<T: RudaPrimitive> SizedContainer for Tensor<T> {
    type Item = T;
}

impl<T: RudaType> Iterator for &Tensor<T> {
    type Item = T;
...
impl<C: RudaType> IntoMut for NativeExpand<Tensor<C>> {
    fn into_mut(self, _scope: &mut Scope) -> Self {
        self
    }
}

impl<T: RudaPrimitive> List<T> for Tensor<T> {
    fn __expand_read(
        scope: &mut Scope,
...
impl<T: RudaPrimitive> Deref for Tensor<T> {
    type Target = [T];
...
impl<T: RudaPrimitive> DerefMut for Tensor<T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        unexpanded!()
    }
}

impl<T: RudaPrimitive> ListExpand<T> for NativeExpand<Tensor<T>> {
...
impl<T: RudaPrimitive> Vectorized for Tensor<T> {}
impl<T: RudaPrimitive> VectorizedExpand for NativeExpand<Tensor<T>> {
    fn vector_size(&self) -> VectorSize {
        self.expand.ty.vector_size()
    }
}

impl<T: RudaPrimitive> ListMut<T> for Tensor<T> {
    fn __expand_write(
        scope: &mut Scope,
        this: NativeExpand<Tensor<T>>,
        idx: NativeExpand<usize>,
...
impl<T: RudaPrimitive> ListMutExpand<T> for NativeExpand<Tensor<T>> {
