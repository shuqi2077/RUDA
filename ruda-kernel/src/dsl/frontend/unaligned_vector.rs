use crate::dsl::intrinsic;
use crate::dsl::ir::{IndexAssignOperator, IndexOperator, Instruction, Operator};
use crate::dsl::{prelude::*};

/// An extension trait for expanding the ruda frontend with the ability to
...
#[ruda]
pub trait UnalignedVector<E: Scalar, N: Size>: RudaType + Sized {
...
fn unaligned_vector_read<T: RudaType<ExpandType = NativeExpand<T>>, E: Scalar, N: Size>(
    this: &T,
    index: usize,
) -> Vector<E, N> {
    intrinsic!(|scope| {
        if !matches!(this.expand.ty, crate::dsl::ir::Type::Scalar(_)) {
...
#[ruda]
#[allow(unused_variables)]
fn unaligned_vector_write<T: RudaType<ExpandType = NativeExpand<T>>, E: Scalar, N: Size>(
    this: &mut T,
    index: usize,
    value: Vector<E, N>,
) {
