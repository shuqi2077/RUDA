...
use ruda_tensor::graph::{OperationIr, TensorIr};
...
fn write_dtype(out: &mut String, dtype: &ruda_tensor::DType) {
...
fn write_shape(out: &mut String, shape: &ruda_tensor::Shape) {
    out.push('[');
    let mut first = true;
    for d in shape.iter() {
        if !first {
...
mod tests {
    use super::*;
    use ruda_tensor::{DType, Shape};
    use ruda_tensor::graph::{BaseOperationIr, TensorId, TensorIr, TensorStatus, UnaryOpIr};
    ...
    fn op_kind_includes_inner_variant() {
        ...
        let op = OperationIr::BaseFloat(BaseOperationIr::Permute(ruda_tensor::graph::PermuteOpIr {
            input,
            out,
            axes: vec![1, 0],
        }));
    ...
    fn op_kind_handles_unary_inside_float() {
        ...
        let op = OperationIr::Float(
            DType::F32,
            ruda_tensor::graph::FloatOperationIr::Exp(UnaryOpIr { input, out }),
        );
    ...
    fn format_table_puts_name_and_score_in_block_header() {
        let op = OperationIr::Float(
            DType::F32,
            ruda_tensor::graph::FloatOperationIr::Exp(UnaryOpIr {
                input: tensor(1, &[4]),
                out: tensor(2, &[4]),
            }),
        );
