...
impl RelativeOps for FloatOperationIr {
    fn to_relative(&self, converter: &mut OperationConverter) -> Self {
        match self {
            ...
            FloatOperationIr::Rsqrt(desc) => FloatOperationIr::Rsqrt(UnaryOpIr {
                input: desc.input.to_relative(converter),
                out: desc.out.to_relative(converter),
            }),
            FloatOperationIr::Silu(desc) => FloatOperationIr::Silu(UnaryOpIr {
                input: desc.input.to_relative(converter),
                out: desc.out.to_relative(converter),
            }),
            ...
            FloatOperationIr::QuantizeDynamic(desc) => FloatOperationIr::QuantizeDynamic(CastOpIr {
                input: desc.input.to_relative(converter),
                out: desc.out.to_relative(converter),
            }),
    ...
    fn to_relative(&self, converter: &mut OperationConverter) -> Self {
        match self {
            ...
            NumericOperationIr::PowiScalar(desc) => {
                let lhs = desc.lhs.to_relative(converter);
                let rhs = desc.rhs.to_relative(converter);
                let rhs = match (desc.lhs.dtype.is_float(), desc.rhs, rhs) {
                    (true, ScalarIr::Int(value), ScalarIr::UInt(id)) => {
                        let dtype = if i32::try_from(value).is_ok() {
                            ruda_tensor::DType::I32
                        } else {
                            ruda_tensor::DType::I64
                        };
                        ScalarIr::Reference { id, dtype }
                    }
                    (true, ScalarIr::UInt(value), ScalarIr::UInt(id)) => {
                        let dtype = if u32::try_from(value).is_ok() {
                            ruda_tensor::DType::U32
                        } else {
                            ruda_tensor::DType::U64
                        };
                        ScalarIr::Reference { id, dtype }
                    }
                    (_, _, rhs) => rhs,
                };
                NumericOperationIr::PowiScalar(ScalarOpIr {
                    lhs,
                    rhs,
                    out: desc.out.to_relative(converter),
                })
...
impl RelativeOps for ScalarIr {
    fn to_relative(&self, converter: &mut OperationConverter) -> Self {
        assert!(!matches!(self, ScalarIr::Reference { .. }), "Scalar is already a relative reference");
...
mod tests {
    ...
    use ruda_tensor::graph::{TensorId, TensorIr, TensorStatus};
    ...
    fn context_fork_output_handles_are_isolated() {
        ...
        // forks are independent sandboxes. `ruda-fusion`'s `TuneInput` layer is
