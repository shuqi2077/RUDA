...
use ruda_tensor::graph::{BackendIr, OperationIr, TensorHandle};
...
impl<B: FusionBackend> Backend for Fusion<B> {
    ...
    where
        Iter: Iterator<Item = &'a mut ruda_tensor::TensorData>,
    {
    ...
    fn dtype_usage(device: &Self::Device, dtype: DType) -> ruda_tensor::DTypeUsageSet {
    ...
    BackendIr<Handle = FusionHandle<Self::FusionRuntime>, Device = FusionDevice<Self::FusionRuntime>>
{
    ...
    /// Scheme produced by `q_permute`, without executing the operation.
    /// Override when the backend changes quantization layout rather than requantizing.
    fn q_permute_scheme(scheme: QuantScheme, _axes: &[usize]) -> QuantScheme {
        scheme
    }
