...
impl<B: FusionBackend> FloatTensorOps<Self> for Fusion<B> {
    ...
    fn float_from_data(data: TensorData, device: &Device<Self>) -> FloatTensor<Self> {
        ...
        let shape = ruda_tensor::TensorMetadata::shape(&tensor);
    ...
    fn float_powi_scalar(lhs: FloatTensor<Self>, rhs: Scalar) -> FloatTensor<Self> {
        ...
        match rhs.elem::<i64>() {
            0 => Self::float_ones(lhs.shape.clone(), &Self::float_device(&lhs), lhs.dtype.into()),
            1 => lhs,
    ...
    fn float_powi_scalar_impl(lhs: FloatTensor<Self>, rhs: Scalar) -> FloatTensor<Self> {
        scalar_float_ops!(PowiOps, B::float_powi_scalar_impl);
        let rhs = match rhs {
            Scalar::UInt(value) => Scalar::UInt(value),
            value => Scalar::Int(value.elem::<i64>()),
        };
        let streams = OperationStreams::with_inputs([&lhs]);
        let client = lhs.client.clone();
        ...
        client
            .register(
                streams,
                OperationIr::NumericFloat(desc.out.dtype, NumericOperationIr::PowiScalar(desc.clone())),
                PowiOps::<B>::new(desc),
            )
            .output()
    }
    ...
    fn float_rsqrt(tensor: FloatTensor<Self>) -> FloatTensor<Self> {
        unary_float_ops!(RsqrtOps, B::float_rsqrt);
        ...
        client
            .register(
                streams,
                OperationIr::Float(desc.out.dtype, FloatOperationIr::Rsqrt(desc.clone())),
                RsqrtOps::<B>::new(desc),
            )
            .output()
    }
    ...
    fn float_powi(lhs: FloatTensor<Self>, rhs: IntTensor<Self>) -> FloatTensor<Self> {
        #[derive(new, Debug)]
        struct PowiOps<B: FusionBackend> {
            desc: BinaryOpIr,
            _b: PhantomData<B>,
        }

        impl<B: FusionBackend> Operation<B::FusionRuntime> for PowiOps<B> {
            fn execute(&self, handles: &mut HandleContainer<B::Handle>) {
                let lhs = handles.get_float_tensor::<B>(&self.desc.lhs);
        ...
        let out = TensorIr::uninit(client.create_empty_handle(), shape, lhs.dtype);
        let desc = BinaryOpIr { lhs: lhs.into_ir(), rhs: rhs.into_ir(), out };
        client
            .register(
                streams,
                OperationIr::NumericFloat(desc.out.dtype, NumericOperationIr::Powi(desc.clone())),
                PowiOps::<B>::new(desc),
            )
            .output()
    }
    ...
    fn float_cast(tensor: FloatTensor<Self>, dtype: ruda_tensor::FloatDType) -> FloatTensor<Self> {
        #[derive(new, Debug)]
        struct CastOps<B: FusionBackend> {
            desc: CastOpIr,
            dtype: ruda_tensor::FloatDType,
            _b: PhantomData<B>,
        }
