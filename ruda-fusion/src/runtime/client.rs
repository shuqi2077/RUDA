...
use ruda_tensor::{Device, DeviceHandle, DeviceId, DeviceService, DeviceServiceStage};
#[cfg(feature = "distributed")]
use ruda_core::device::CommunicationId;
...
impl<R: FusionRuntime> DeviceService for FusionServer<R> {
    ...
    fn utilities(&self) -> ruda_tensor::ServerUtilitiesHandle {
    ...
    R: FusionRuntime + 'static,
{
        ...
        B: FusionBackend<FusionRuntime = R>,
    {
        self.server
            .submit_blocking(move |server| {
                ...
                server.read_float::<B>(tensor.into_ir(), stream)
            })
            .unwrap()
    }
        ...
        B: FusionBackend<FusionRuntime = R>,
    {
        self.server
            .submit_blocking(move |server| {
                let stream = tensor.stream;
                server.read_int::<B>(tensor.into_ir(), stream)
            })
            .unwrap()
    }
        ...
        B: FusionBackend<FusionRuntime = R>,
    {
        self.server
            .submit_blocking(move |server| {
                let stream = tensor.stream;
                server.read_bool::<B>(tensor.into_ir(), stream)
            })
            .unwrap()
    }

    /// Resolve the given quantized tensor to a primitive tensor.
    pub fn resolve_tensor_quantized<B>(&self, tensor: FusionTensor<R>) -> B::QuantizedTensorPrimitive
    where
        B: FusionBackend<FusionRuntime = R>,
    {
        self.server
            .submit_blocking(move |server| {
                let stream = tensor.stream;
                server.read_quantized::<B>(tensor.into_ir(), stream)
            })
            .unwrap()
    }
        ...
        B: FusionBackend<FusionRuntime = R> + DistributedBackend,
    {
        ...
        if utilities.initialized_comms.read().unwrap().contains(&id) {
            ...
            // This is already handled by ruda, but since fusion adds another layer of streams and asynchronous submits,
