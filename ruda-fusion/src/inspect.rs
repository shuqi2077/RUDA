...
//! Handle snapshots from the backing [`HandleContainer`](ruda_tensor::graph::HandleContainer)
...
//!
//! ```ignore
//! # use ruda_fusion::inspect::FusionInspector;
//! # use ruda_tensor::StreamId;
...
//!
//! [`HandleContainer`](ruda_tensor::graph::HandleContainer) is shared per-device across the
...
impl FusionReport {
    ...
    /// [`FusionLogLevel::Full`](ruda_tensor_config::fusion::FusionLogLevel). Useful for
...
pub struct FusionInspector {
    ...
    /// [`HandleContainer`](ruda_tensor::graph::HandleContainer) at the most recent quiescent
...
impl FusionInspector {
    ...
    /// The most recent [`HandleContainer`](ruda_tensor::graph::HandleContainer) size observed by
    ...
    /// [`HandleContainer`](ruda_tensor::graph::HandleContainer), or `None` if nothing has been
...
pub mod matchers {
    ...
    use ruda_tensor::graph::{FloatOperationIr, NumericOperationIr, OperationIr};
