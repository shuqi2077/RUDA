...
use ruda_tensor::graph::{HandleContainer, TensorId, TensorIr};
...
pub(crate) struct HandleCollector<R: Runtime>(spin::Mutex<HashMap<TensorId, RudaFusionHandle<R>>>);
...
impl<R: Runtime> HandleCollector<R> {
    ...
    fn capture(&self, handles: &HandleContainer<RudaFusionHandle<R>>) {
    ...
    fn take(&self) -> HashMap<TensorId, RudaFusionHandle<R>> {
        core::mem::take(&mut *self.0.lock())
    }
}

/// Fusion input for autotuning. Thread the caller's `&mut Context` through the tuning
/// pipeline without `unsafe` by riding ruda's `'a` lifetime parameter.
...
enum TuneState<'a, R: Runtime> {
    ...
    /// `Original` on drop.
    Fork {
        context: Box<Context<RudaFusionHandle<R>>>,
        new_handles: Option<Arc<HandleCollector<R>>>,
    },
}

impl<'a, R: Runtime, O> TuneInput<'a, R, O> {
    pub(crate) fn new(context: &'a mut Context<RudaFusionHandle<R>>, optimization: O) -> Self {
        Self {
    ...
    pub(crate) fn handles(&self) -> &HandleContainer<RudaFusionHandle<R>> {
    ...
    where
        F: FnOnce(&mut Context<RudaFusionHandle<R>>, &O) -> T,
    {
