...
use crate::device::engine::{codegen::ir::QuantSchemeFuse, scoring::Scoring};
use crate::{FuserProperties, FuserStatus, OperationFuser};
use ruda_tensor::graph::{
    BaseOperationIr, BinaryOpIr, FloatOperationIr, NumericOperationIr, OperationIr, ScalarIr, ScalarOpIr,
    TensorIr, UnaryOpIr,
};
use ruda_core::tensor::{DType, Shape};
use ruda_tensor_config::{fusion::FusionLogLevel, log_fusion};
...
impl TraceOperationFuser {
    ...
    fn fuse_float(&mut self, ops: &FloatOperationIr) -> bool {
        match ops {
            ...
            FloatOperationIr::Rsqrt(desc) => self.fuse_unary_ops(desc, |input, out| {
                FuseOp::Rsqrt(UnaryFuseArgs { input, out })
            }),
            FloatOperationIr::Silu(desc) => {
                if !matches!(desc.input.dtype, DType::F32 | DType::F16 | DType::BF16) {
                    return false;
                }
                self.fuse_unary_ops(desc, |input, out| {
                    FuseOp::Silu(UnaryFuseArgs { input, out })
                })
            }
    ...
    fn fuse_numeric(&mut self, op: &NumericOperationIr) -> bool {
        match op {
            NumericOperationIr::Powi(desc) if desc.lhs.dtype.is_float() => {
                self.fuse_binary_ops(desc, |lhs, rhs, out| {
                    FuseOp::Powi(BinaryFuseArgs { lhs, rhs, out })
                })
            }
            NumericOperationIr::PowiScalar(desc) if desc.lhs.dtype.is_float() => {
                ...
                self.fuser.fuse(|build| {
                    ...
                    let out = build.output(&desc.out)?;
                    build.fuse_operation(FuseOp::Powi(BinaryFuseArgs { lhs, rhs, out }));
                    Some(())
                })
            }
