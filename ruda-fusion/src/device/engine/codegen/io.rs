...
use ruda_core::tensor::quantization::QuantScheme;
use ruda_kernel::dsl::quant::scheme::QuantLevel;
use ruda_kernel::dsl::{
    intrinsic,
    ir::{ManagedVariable, Variable},
    prelude::*,
};
use ruda_kernel::library::{FastDivmod, tensor::View};
use ruda_kernel::quantization::layout::{BlockScaledLayout, PerTensorLayout, ScalesLayout};
...
/// Reads the value from the [arg](FuseArg) and cast it to the generic ruda primitive.
///
/// # Notes
///
...
/// Maps a logical reference element to its stored word, packed lane and scale-view index.
#[ruda]
pub fn quantized_element_position(
    inputs: &GlobalArgs,
    locals: &LocalArgs,
    ref_element: usize,
    #[comptime] pos: usize,
    ...
    let packed_axis = comptime![scheme.packing_dim().map(|dim| rank - dim - 1).unwrap_or(rank)];
    ...
    #[unroll]
    for axis in 0..rank {
        let shape = tensor.tensor.shape(axis);
        let coordinate = (ref_element / locals.ref_strides[axis]) % shape;
        logical_offset = logical_offset * shape + coordinate;
        if comptime![axis == packed_axis] {
            stored_offset += (coordinate / scheme.num_quants()) * tensor.tensor.stride(axis);
            packed_lane = coordinate % scheme.num_quants();
        } else {
            stored_offset += coordinate * tensor.tensor.stride(axis);
        }
    }

    (stored_offset, packed_lane, logical_offset)
}
...
pub fn read_quantized<C: Scalar, N: Size>(
    ...
    match arg {
        FuseArg::Input(pos, _precision, _layout) => {
            let (stored_offset, _, _) = quantized_element_position(
                inputs,
                locals,
            ...
            let global = inputs.tensors.index(pos);
            set_polyfill::<DynElem, DynSize>(comptime![global.ty]);
            let offset = stored_offset / global.tensor.vector_size();
...
#[ruda]
pub fn read_input_window<C: RudaPrimitive>(
    inputs: &GlobalArgs,
    #[comptime] pos: usize,
    start: usize,
    end: usize,
...
#[ruda]
pub fn input_as_slice<C: RudaPrimitive>(inputs: &GlobalArgs, #[comptime] pos: usize) -> Slice<C> {
    set_polyfill_typed::<C, DynElem, DynSize>();
    let tensor = inputs.tensors.index(pos);
...
pub fn input_as_scales_view<C: Scalar, N: Size>(
    ...
    let mut tensor_len = 1;
    #[unroll]
    for axis in 0..rank {
        tensor_len *= tensor.tensor.shape(axis);
    }
...
/// Generic way to construct any [`RudaPrimitive`] from an int. Used for fusion.
#[allow(unused_variables)]
#[ruda]
fn from_const_int<C: RudaPrimitive>(#[comptime] value: usize) -> C {
    intrinsic!(|scope| {
...
pub(crate) fn set_polyfill_typed<C: RudaPrimitive, Dyn: Scalar, DynSize: Size>() {
    intrinsic!(|scope| {
