...
pub trait TraceRunner<R: Runtime>: Vectorization<R> {
    ...
    /// in the [optimization builder](crate::OptimizationBuilder).
    fn run<'a>(
        &'a self,
...
pub enum VectorizationHandle<'a, R: Runtime> {
    NormalInput(&'a RudaFusionHandle<R>, &'a TensorIr),
    QuantValues(&'a RudaFusionHandle<R>, &'a TensorIr),
    QuantParams,
}
...
pub trait Vectorization<R: Runtime> {
    ...
    fn vectorization<'a>(
        &self,
        _context: &Context<RudaFusionHandle<R>>,
