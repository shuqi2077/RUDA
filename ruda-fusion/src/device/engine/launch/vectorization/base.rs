...
use ruda_kernel::dsl::{Runtime, ir::VectorSize};
...
impl VectorSizeOverrides {
    ...
    pub fn mapping<R: Runtime>(&self, context: &Context<RudaFusionHandle<R>>) -> Self {
        match &self.state {
            Some(state) => {
            ...
            match input {
                ...
                VectorizationHandle::QuantValues(handle, tensor_ir) => {
                    ...
                    let num_quants = match tensor_ir.dtype {
                        ruda_core::tensor::DType::QFloat(quant_scheme) => quant_scheme.num_quants(),
                        _ => panic!(""),
                    };
                    let val = match val {
...
fn vectorization_input<R: Runtime>(
    ...
    // Reflect changes from https://github.com/shuqi2077/RUDA/blob/main/THIRD_PARTY_NOTICES.md
    ...
    if let ruda_core::tensor::DType::QFloat(quant_scheme) = handle.dtype {
