...
use crate::device::engine::launch::{QuantParamsHandleInput, QuantValuesHandleInput};
use crate::device::engine::trace::block::FuseBlock;
use crate::device::engine::trace::{FuseResources, RegisterTensor, TensorView};
use crate::stream::Context;
use ruda_tensor::graph::{TensorIr, TensorStatus};
...
impl<'a, R: Runtime> InputPlanner<'a, R> {
    ...
    pub fn run(self, context: &mut Context<RudaFusionHandle<R>>, plan: &mut LaunchPlan<'a, R>) {
            ...
            match input {
                ...
                RegisterTensor::QuantValues(tensor_relative) => {
                    ...
                    let scheme = match tensor_relative.dtype {
                        ruda_core::tensor::DType::QFloat(scheme) => scheme,
                    ...
                    let missing_dims = plan.rank.saturating_sub(tensor_global.shape.rank());
                    if missing_dims > 0 {
                        let mut values_shape = tensor_global.shape.clone();
                        if let Some(dim) = scheme.packing_dim() {
                            let axis = values_shape.rank() - dim - 1;
                            values_shape[axis] = values_shape[axis].div_ceil(scheme.num_quants());
                        }
                        let values_len = values_shape.num_elements();
                        let params_len =
                            params_shape(&tensor_global.shape, scheme.level).num_elements();
                        for _ in 0..missing_dims {
                            tensor_global.shape.insert(0, 1);
                            handle.strides.insert(0, values_len);
                            params.strides.insert(0, params_len);
                        }
                    }
