...
use ruda_core::tensor::{
    Strides, ReshapeAction, contiguous_strides, is_contiguous, reshape_action,
};
use ruda_kernel::dsl::{Runtime, client::ComputeClient, ir::StorageType};
...
impl<'a, R: Runtime> OutputPlanner<'a, R> {
    ...
    fn swapped_dims_output(
        ...
        let mut handle = RudaFusionHandle {
            client: client.clone(),
            handle: original_handle.handle.handle.clone(),
            device: device.clone(),
            strides,
