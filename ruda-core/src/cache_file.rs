...
pub struct CacheFile {
    ...
    /// cascaded into "Task failed: Any { .. }" warnings across ruda's
...
impl CacheFile {
    ...
    pub fn new<P: Into<PathBuf>>(path: P, lock_max_duration: Duration) -> Self {
        ...
        log::debug!("ruda cache: CacheFile::new({:?})", path);
        ...
        if !fs::exists(&path).unwrap_or(false) {
            ...
            {
                log::error!(
                    "ruda cache: create_dir_all({:?}) failed: kind={:?} raw_os_error={:?} err={}; \
    ...
    pub fn lock(&mut self) -> Option<BufReader<File>> {
        ...
        if !self.lock.lock() {
            ...
            log::error!(
                "ruda cache: FileLock::lock({:?}) gave up; disabling cache entry",
                self.path
            );
            self.valid = false;
            return None;
        ...
        let mut file = match File::open(&self.path) {
            ...
            Err(err) => {
                ...
                log::error!(
                    "ruda cache: File::open({:?}) failed during lock(): kind={:?} \
        ...
        let end = match file.metadata() {
            ...
            Err(err) => {
                log::error!(
                    "ruda cache: metadata({:?}) failed: kind={:?} raw_os_error={:?} err={}",
                    self.path,
                    err.kind(),
                    err.raw_os_error(),
                    err
        ...
        if let Err(err) = file.seek(SeekFrom::Start(self.cursor)) {
            log::error!(
                "ruda cache: seek({:?}, cursor={}) failed: kind={:?} raw_os_error={:?} err={}",
                self.path,
                self.cursor,
                err.kind(),
                err.raw_os_error(),
    ...
    pub fn write(&mut self, content: &[u8]) {
        ...
        let mut file = match fs::OpenOptions::new().append(true).open(&self.path) {
            ...
            Err(err) => {
                let path_exists = self.path.exists();
                log::error!(
                    "ruda cache: append-open({:?}) failed: kind={:?} raw_os_error={:?} \
                     path_exists={} err={}",
                    self.path,
                    err.kind(),
                    err.raw_os_error(),
        ...
        match file.write(content) {
            ...
            Err(err) => {
                log::error!(
                    "ruda cache: write({:?}, {} bytes) failed: kind={:?} raw_os_error={:?} err={}",
                    self.path,
                    content.len(),
                    err.kind(),
                    err.raw_os_error(),
                ...
                Err(err) => match err.kind() {
                    std::io::ErrorKind::AlreadyExists => {
                        if let Ok(true) = self.maybe_cleanup_frozen_lock() {
                            ...
                            {
                                log::error!(
                                    "ruda cache: FileLock::lock({:?}) timed out after {:?} \
                    ...
                    _ => {
                        if !other_error_logged {
                            ...
                            log::error!(
                                "ruda cache: FileLock::lock({:?}) unexpected I/O error: \
                        ...
                        if waiting_total.elapsed().unwrap_or_default() > self.lock_max_duration {
                            ...
                            log::error!(
                                "ruda cache: FileLock::lock({:?}) giving up after {:?} \
