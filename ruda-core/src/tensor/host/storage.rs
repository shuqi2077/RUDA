...
impl HostTensor {
    ...
    fn copy_contiguous<E: Element + bytemuck::Pod>(&self) -> Self {
        ...
        if shape.len() <= 1 && all_positive {
            ...
            // `copy_2d_tiled` accepts an initialized slice. Pod implies
            // Zeroable, so initialize before creating that slice. Preserve the
            // tiled traversal; any uninitialized-buffer optimization needs its
            // own MaybeUninit interface and proof of complete initialization.
...
/// strided iterator. Ruda tensors are capped at 8 dims in practice.
...
fn collapse_for_copy(shape: &[usize], strides: &[isize]) -> CollapsedLayout {
    ...
    // fit our stack buffer. In practice this never triggers (ruda
...
///
/// Matches `ruda_core::DType::size()` semantics: Bool(Native) and Bool(U8) are
...
/// # Panics
///
/// Panics if the dtype has a zero-byte element size. `ruda_core::DType::size()`
/// returns 0 for sub-byte quantized dtypes (Q4F, Q4S, Q2F, Q2S, and most
/// `QuantStore::PackedNative` variants). ruda-tensor-host does not yet support these
...
pub fn dtype_size(dtype: DType) -> usize {
    // Delegate to ruda-core's canonical size to stay in sync.
    let size = dtype.size();
    assert!(
        size > 0,
