...
use crate::tensor::{INLINE_DIMS, MetadataError, indexing::AsSize};
...
macro_rules! strides {
    (@one $x:expr) => (1usize);
    () => (
        $crate::tensor::Strides::new_raw($crate::tensor::SmallVec::new())
    );
    ($elem:expr; $n:expr) => ({
        $crate::tensor::Strides::new_raw($crate::tensor::smallvec!($elem; $n))
    });
    ($($x:expr),+$(,)?) => ({
        $crate::tensor::Strides::new_raw($crate::tensor::smallvec!($($x),*))
    });
}
