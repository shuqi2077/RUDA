...
pub use super::slice::SliceOps;
pub use crate::shape;

pub use crate::tensor::errors::ExpressionError;
use crate::tensor::{
    INLINE_DIMS,
...
macro_rules! shape {
    (@one $x:expr) => (1usize);
    () => (
        $crate::tensor::Shape::new_raw($crate::tensor::SmallVec::new())
    );
    ($elem:expr; $n:expr) => ({
        $crate::tensor::Shape::new_raw($crate::tensor::smallvec!($elem; $n))
    });
    ($($x:expr),+$(,)?) => ({
        $crate::tensor::Shape::new_raw($crate::tensor::smallvec!($($x),*))
    });
}
