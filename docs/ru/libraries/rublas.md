...
[English](../../en/libraries/rublas.md) | [简体中文](../../zh/libraries/rublas.md) | [日本語](../../ja/libraries/rublas.md) | [Deutsch](../../de/libraries/rublas.md) | **Русский**

[Вычислительные библиотеки](README.md) · [Среда выполнения API](../runtime-api.md) · [中文](../../zh/libraries/rublas.md)
...
Пакет Cargo — `rublas`. Выберите необходимые функции и отключите ненужные значения по умолчанию для путей общего назначения. См. [Cargo.toml](../../../ruBLAS/Cargo.toml).
...
См. [точку входа умножения матрицы](../../../ruBLAS/src/tensor_matmul/base.rs).

### Векторное произведение

`rublas::tensor_vector::cross(lhs, rhs, dim)` требует длину 3 выбранного измерения и возвращает тензор устройства. Вычисление вдоль измерения, отличного от последнего, включает перестановку осей и преобразование в непрерывное размещение. См. [cross.rs](../../../ruBLAS/src/tensor_vector/cross.rs).
...
K, N и G должны быть ненулевыми; K должен делиться на G и N на 8. Конечная размерность входных данных — K, заменяемая на N на выходе. Все операнды должны использовать общее устройство. Порядок упакованных битов должен соответствовать [ядру AWQ](../../../ruBLAS/src/tensor_int4/kernel.rs); произвольный файл INT4 не может использоваться непосредственно как qweight.

См. [Интерфейс INT4](../../../ruBLAS/src/tensor_int4/mod.rs) для объекта и его проверок.
...
Источник: [сгруппированный интерфейс](../../../ruBLAS/src/tensor_grouped/mod.rs) и [ядро](../../../ruBLAS/src/tensor_grouped/kernel.rs).

## 5. Интеграция с другими библиотеками

[ruDNN MoE](rudnn.md) использует групповое умножение для экспертных прогнозов. Квантованные веса используют отдельный модуль `tensor_int4`; обычное групповое умножение с плавающей запятой не является экспертным вычислением INT4.
