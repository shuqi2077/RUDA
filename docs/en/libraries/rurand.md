...
The Cargo package is `ruRAND`; its Rust import name is `rurand`. Feature `tensor` enables device tensor interfaces. This configuration places the application directory alongside the `RUDA` source directory. See [Getting started](../getting-started.md) for NVIDIA setup.
...
API reference: [Tensor interfaces](../../../ruRAND/src/tensor/mod.rs), [Seed state](../../../ruRAND/src/state.rs), [Distributions](../../../ruRAND/src/distributions/mod.rs).
