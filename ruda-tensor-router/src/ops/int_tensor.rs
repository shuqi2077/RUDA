...
use ruda_tensor::tensor::{BoolTensor, Device, FloatTensor, IndexingUpdateOp, IntElem, IntTensor};
use ruda_tensor::{
...
impl<R: RunnerChannel> IntTensorOps<Self> for BackendRouter<R> {
    ...
    fn int_cast(tensor: IntTensor<Self>, dtype: ruda_tensor::IntDType) -> IntTensor<Self> {
        let client = tensor.client.clone();
