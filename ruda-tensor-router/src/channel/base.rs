...
use ruda_tensor::{DType, Element, Shape, backend::DeviceOps};
...
pub trait RunnerChannel: Clone + Send + Sync + 'static + Sized {
    ...
    fn change_client_backend(
        ...
        } else if desc.dtype.is_int() || desc.dtype.is_uint() {
        ...
        } else if matches!(desc.dtype, DType::QFloat(_)) {
            handle = Self::Bridge::change_backend_quantized(handle, desc.shape.clone(), device);
        } else {
            unimplemented!()
        }
