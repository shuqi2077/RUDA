...
mod tests {
    ...
    fn test_dataloader_slice_multi_device() {
        ...
        let (device1, device2) = (ruda_tensor_host::HostDevice, ruda_tensor_host::HostDevice);

        #[cfg(all(test, feature = "test-tch"))]
        let (device1, device2) = (
            ruda_tensor_tch::LibTorchDevice::Cuda(0),
            ruda_tensor_tch::LibTorchDevice::Cuda(1),
        );

        #[cfg(all(test, feature = "test-wgpu"))]
        let (device1, device2) = (
            ruda_tensor_wgpu::WgpuDevice::DiscreteGpu(0),
            ruda_tensor_wgpu::WgpuDevice::DiscreteGpu(1),
        );

        #[cfg(all(test, feature = "test-cuda"))]
        let (device1, device2) = (ruda_tensor_device::cuda::CudaDevice::new(0), ruda_tensor_device::cuda::CudaDevice::new(1));
