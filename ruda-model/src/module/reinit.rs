...
#[derive(Debug)]
/// Overrides float and int tensors of [ruda modules](super::Module).
///
...
impl<B: Backend> ModuleMapper<B> for Reinitializer<B> {
    ...
    fn map_int<const D: usize>(
        &mut self,
        param: super::Param<Tensor<B, D, ruda_tensor::api::Int>>,
    ) -> super::Param<Tensor<B, D, ruda_tensor::api::Int>> {
        let (id, tensor, mapper) = param.consume();
        let device = tensor.device();
        let shape = tensor.shape();
        let num_elements = shape.num_elements();
    ...
    fn map_bool<const D: usize>(
        &mut self,
        param: super::Param<Tensor<B, D, ruda_tensor::api::Bool>>,
    ) -> super::Param<Tensor<B, D, ruda_tensor::api::Bool>> {
        let (id, tensor, mapper) = param.consume();
        super::Param::from_mapped_value(id, tensor, mapper)
    }
}
