...
impl<T: Parameter> Param<T> {
    ...
    /// by `ruda` and should handle those details.
...
impl<T: Parameter> Clone for Param<T> {
    fn clone(&self) -> Self {
        ...
        param.require_grad = self.require_grad;
        param
    }
}
