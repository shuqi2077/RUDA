...
macro_rules! empty {
    (module) => {
        type Record = $crate::module::EmptyRecord;

        fn visit<V: $crate::module::ModuleVisitor<B>>(&self, _visitor: &mut V) {
            // Nothing to do
        }

        fn map<M: $crate::module::ModuleMapper<B>>(self, _mapper: &mut M) -> Self {
            self
        }
        ...
        fn collect_devices(&self, devices: $crate::module::Devices<B>) -> $crate::module::Devices<B> {
            devices
        }
    };
    ...
    ($type:ty) => {
        impl<B: $crate::tensor::backend::Backend> $crate::module::Module<B> for $type {
            $crate::empty!(module);
        }

        impl<B: $crate::tensor::backend::AutodiffBackend> $crate::module::AutodiffModule<B> for $type {
            $crate::empty!(ad_module, $type);
        }

        impl $crate::module::ModuleDisplayDefault for $type {
            fn content(&self, content: $crate::module::Content) -> Option<$crate::module::Content> {
                let string = format!("{}", self);
                content.add_formatted(&string).optional()
            }
        }

        impl $crate::module::ModuleDisplay for $type {}
    };
}
...
impl ruda_model::module::ModuleDisplay for str {}
impl ruda_model::module::ModuleDisplayDefault for str {
    fn content(&self, content: ruda_model::module::Content) -> Option<ruda_model::module::Content> {
...
mod tests {
    ...
    use ruda_tensor::api::{Device, Tensor};
