...
pub use ruda_model_macros::Module;
use ruda_tensor::api::{Bool, Int, Tensor, ops::Device};
...
///
/// Modules should be created using the [derive](ruda_model_macros::Module) attribute.
...
/// parameter B. This will be used by the [derive](ruda_model_macros::Module) attribute to generate the code
...
pub trait Module<B: Backend>: Clone + Send + core::fmt::Debug {
    ...
    fn no_grad(self) -> Self {
        module!(
            map = self,
            ops = |param: Param<Tensor<B, D>>| param.set_require_grad(false)
        )
    }
