    ...
    Send + Sync + core::default::Default + core::fmt::Debug + Clone
{
        ...
        R: Record<B>,
    {
        ...
        let item = RudaRecord::new::<Self>(item);
    ...
    where
        R: Record<B>,
    {
        let item: RudaRecord<R::Item<Self::Settings>, B> =
            self.load_item(&mut args).map_err(|err| {
                if let Ok(record) = self.load_item::<RudaRecordNoItem>(&mut args) {
                    ...
                    if metadata.version != record.metadata.version {
                        message += format!(
                            "\nMetadata has a different Ruda version: Actual {:?}, Expected {:?}",
...
fn recorder_metadata<R, B>() -> RudaMetadata
where
    R: Recorder<B>,
    B: Backend,
{
    RudaMetadata::new(
        ...
        super::format::record_type_name(type_name::<R>()),
...
pub struct RudaMetadata {
    ...
    /// Ruda record version used to record the item.
    pub version: String,
...
pub struct RudaRecord<I, B: Backend> {
    /// Metadata of the record.
    pub metadata: RudaMetadata,
...
impl<I, B: Backend> RudaRecord<I, B> {
...
mod tests {
    static FILE_PATH: &str = "/tmp/ruda_test_record";

    use crate::TestBackend;

    use super::*;
    use ruda_tensor::api::{Device, ElementConversion};
