...
mod state {
    use ruda_model::module::EmptyRecord;
...
#[cfg(feature = "std")]
mod require_grad {
    use ruda_tensor::api::{TensorData, backend::AutodiffBackend};
    use rand::{
        SeedableRng,
        rngs::{StdRng, SysRng},
    };
...
mod grad_distributed {
    ...
    use ruda_tensor::api::backend::distributed::DistributedBackend;
    use ruda_tensor::api::backend::distributed::{DistributedParamId, ReduceOperation};
    use ruda_tensor::api::{Device, Tolerance};
    use ruda_tensor::api::{TensorData, backend::AutodiffBackend};
    use rand::{
        SeedableRng,
        rngs::{StdRng, SysRng},
    ...
    fn sharded_module_should_sync_gradients_sum_activation() {
        compare_sync_gradients::<TestAutodiffBackend>(ReduceOperation::Sum, |weights, x| {
            let y = weights.clone().matmul(x);
            let y = y.add(weights);
            ruda_tensor::api::activation::relu(y)
        });
    }
    ...
    fn compare_sync_gradients<B: AutodiffBackend + DistributedBackend>(
        ...
        use ruda_tensor::api::backend::distributed::DistributedConfig;
