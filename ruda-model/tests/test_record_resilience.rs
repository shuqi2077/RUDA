...
mod tests {
    ...
    use ruda_tensor_host::HostDevice;
    ...
    fn test_tensor_serde() {
        let tensor: ruda_tensor::api::Tensor<TestBackend, 1> =
            ruda_tensor::api::Tensor::ones([1], &HostDevice);
        let encoded = serde_json::to_string(&tensor).unwrap();
        let decoded: ruda_tensor::api::Tensor<TestBackend, 1> = serde_json::from_str(&encoded).unwrap();
