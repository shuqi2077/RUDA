...
#[derive(Debug)]
pub struct Stream {
    pub(crate) sys: ruda_hip_sys::hipStream_t,
...
impl EventStreamBackend for HipStreamBackend {
    ...
    fn create_stream(&self) -> Self::Stream {
        ...
        // this `Stream`.
        let stream = unsafe {
            let mut stream: ruda_hip_sys::hipStream_t = std::ptr::null_mut();
            let stream_status = ruda_hip_sys::hipStreamCreateWithFlags(
                &mut stream,
                ruda_hip_sys::hipStreamNonBlocking,
            );
