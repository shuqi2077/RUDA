...
use ruda_core::{backtrace::BackTrace, bytes::Bytes, stream_id::StreamId};
use ruda_kernel::dsl::{
    MemoryUsage,
...
impl<'a> Command<'a> {
    ...
    /// * `kernel` - The ruda task to compile if not cached.
...
pub(crate) unsafe fn write_to_cpu(
    ...
    if rank <= 1 {
        ...
        let status = unsafe {
            ruda_hip_sys::hipMemcpyDtoHAsync(
    ...
    unsafe {
        let status = ruda_hip_sys::hipMemcpy2DAsync(
            bytes.as_mut_ptr() as *mut _,
            width_bytes,
            resource_ptr,
            pitch,
        ...
        if status != HIP_SUCCESS {
            let status = ruda_hip_sys::hipMemcpyDtoHAsync(
                bytes.as_mut_ptr() as *mut _,
                resource_ptr,
                bytes.len(),
                stream,
...
unsafe fn write_to_gpu(
    ...
    if rank > 1 {
        ...
        unsafe {
            ...
            let status = ruda_hip_sys::hipMemcpyHtoDAsync(resource.ptr, ptr, data.len(), stream);
            assert_eq!(status, HIP_SUCCESS, "Should send data to device");
        }
    };
