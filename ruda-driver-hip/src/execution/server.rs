...
use crate::{
    execution::{command::Command, context::HipContext, fence::Fence, stream::HipStreamBackend},
    runtime::HipCompiler,
};
...
impl ComputeServer for HipServer {
    type Kernel = Box<dyn RudaTask<HipCompiler>>;
    ...
    fn start_profile(&mut self, stream_id: StreamId) -> Result<ProfilingToken, ServerError> {
        ruda_core::future::block_on(self.sync(stream_id))?;
    ...
    fn end_profile(
        ...
        if let Err(err) = ruda_core::future::block_on(self.sync(stream_id)) {
            self.ctx
                .timestamps
...
impl HipServer {
    ...
    fn launch_checked(
        &mut self,
        kernel: Box<dyn RudaTask<HipCompiler>>,
        count: RudaCount,
        bindings: KernelArguments,
        mode: ExecutionMode,
        stream_id: StreamId,
        ...
        let count = match count {
            ...
            RudaCount::Dynamic(binding) => {
                ...
                assert!(
                    data.len() == 3,
                    "Dynamic ruda count should contain 3 values"
                );
