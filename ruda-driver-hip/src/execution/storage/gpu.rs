...
pub struct GpuStorage {
    mem_alignment: usize,
    memory: HashMap<StorageId, ruda_hip_sys::hipDeviceptr_t>,
    deallocations: Vec<StorageId>,
    ptr_bindings: PtrBindings,
    stream: ruda_hip_sys::hipStream_t,
}
...
pub struct GpuResource {
    ...
    pub binding: ruda_hip_sys::hipDeviceptr_t,
...
impl GpuStorage {
    ...
    pub fn new(mem_alignment: usize, stream: ruda_hip_sys::hipStream_t) -> Self {
        Self {
            mem_alignment,
            memory: HashMap::new(),
    ...
    pub fn perform_deallocations(&mut self) {
        for id in self.deallocations.drain(..) {
            if let Some(ptr) = self.memory.remove(&id) {
                ...
                unsafe {
                    ruda_hip_sys::hipFreeAsync(ptr, self.stream);
                }
            }
        }
    }
...
impl ComputeStorage for GpuStorage {
    ...
    fn get(&mut self, handle: &StorageHandle) -> Self::Resource {
        ...
        GpuResource::new(
            *ptr as ruda_hip_sys::hipDeviceptr_t,
    ...
    fn alloc(&mut self, size: u64) -> Result<StorageHandle, IoError> {
        ...
        unsafe {
            ...
            let status = ruda_hip_sys::hipMallocAsync(&mut ptr, size as usize, self.stream);
            ...
            ruda_hip_sys::hipStreamSynchronize(self.stream);
