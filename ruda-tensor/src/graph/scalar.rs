...
impl Hash for ScalarIr {
    fn hash<H: core::hash::Hasher>(&self, state: &mut H) {
        match self {
            ...
            ScalarIr::Reference { id, dtype } => (id, dtype).hash(state),
        }
    }
}
...
impl ScalarIr {
    ...
    pub fn elem<E: Element>(self) -> E {
        match self {
            ...
            ScalarIr::Reference { .. } => panic!("A scalar reference must be resolved before conversion"),
        }
    }
}
...
impl From<ScalarIr> for Scalar {
    fn from(value: ScalarIr) -> Self {
        match value {
            ...
            ScalarIr::Reference { .. } => panic!("A scalar reference is not a literal value"),
        }
    }
}
