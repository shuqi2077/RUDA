...
pub use ruda_core::backtrace::BackTrace;
...
/// This trait defines all types and functions needed for a backend to be used with ruda.
///
/// ## Design
///
...
/// ### Eager Mode
///
/// Because ruda supports dynamic graphs, the backend trait is designed around kernel
...
/// with ruda. This pattern could also be used to create an operation fusion trait, which
...
/// struct in the `ruda-tensor` crate.
/// For modules, public functions are often created, which can be used by `ruda-model` modules.
pub trait Backend:
    BackendTypes
    + FloatTensorOps<Self>
    + BoolTensorOps<Self>
...
pub use ruda_core::tensor::execution::ExecutionError;
