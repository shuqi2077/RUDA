...
pub trait FloatTensorOps<B: Backend> {
    ...
    fn float_prod(tensor: FloatTensor<B>) -> FloatTensor<B> {
        ...
        let tensor = B::float_reshape(tensor, Shape::new([len]));
        B::float_prod_dim(tensor, 0)
    }
    ...
    fn float_prod_dim(tensor: FloatTensor<B>, dim: usize) -> FloatTensor<B> {
        ...
        if len == 0 {
            shape[dim] = 1;
            return B::float_ones(shape, &B::float_device(&tensor), tensor.dtype().into());
        }
        let mut slices = alloc::vec![Slice::full(); shape.num_dims()];
        slices[dim] = Slice::from(len - 1..len);
        B::float_slice(B::float_cumprod(tensor, dim), &slices)
    }
    ...
    fn float_rsqrt(tensor: FloatTensor<B>) -> FloatTensor<B> {
        Self::float_recip(Self::float_sqrt(tensor))
    }
