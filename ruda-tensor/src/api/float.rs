use crate::api::AsIndex;
use crate::api::Cast;
use crate::api::Tensor;
use crate::api::cast::ToElement;
use crate::api::check;
use crate::api::check::TensorCheck;
use crate::api::ops::GridSampleOptions;
use crate::api::quantization::{QuantScheme, QuantizationParameters};
use crate::api::backend::Backend;
use crate::api::stats;
use crate::api::{Distribution, TensorData};
use crate::api::{Bool, Float, Int, TensorPrimitive};
#[cfg(feature = "api-distributed")]
use crate::AutodiffBackend;
use crate::ElementConversion;
use crate::Scalar;
use crate::TensorMetadata;
#[cfg(feature = "api-distributed")]
use crate::distributed::DistributedParamId;
use crate::get_device_settings;
use crate::tensor::FloatMathOps;
use crate::tensor::quantization::QuantizationParametersPrimitive;
use core::f32;
...
"#
    ...
    /// Applies the reciprocal square root element-wise, preserving shape and dtype.
    pub fn rsqrt(self) -> Self {
        Self::new(TensorPrimitive::Float(B::float_rsqrt(self.primitive.tensor())))
    }
    ...
    pub fn median(self, dim: usize) -> Self {
        // TODO: Allow backend specialization. Optimally, implement a median kernel for ruda
    ...
    /// ```rust
    /// use ruda_tensor::api::backend::Backend;
    /// use ruda_tensor::api::{Tensor, FloatDType, IntDType};
    ///
    /// fn example<B: Backend>() {
    ///     let device = Default::default();
    ...
    pub fn quantize(
        ...
        let tensor = self.primitive.tensor();
        let scales = qparams.scales.primitive.tensor();
        let scales_shape = crate::quantization::params_shape(&tensor.shape(), scheme.level);
        assert_eq!(
            scales.shape().num_elements(),
            scales_shape.num_elements(),
            "Quantization scale count must match the parameter shape"
        );
        let scales = B::float_reshape(scales, scales_shape);
        Tensor::new(TensorPrimitive::QFloat(B::quantize(
            tensor,
            scheme,
            QuantizationParametersPrimitive { scales },
        )))
    }
    ...
    ///
    /// # Notes
    /// This uses [min-max calibration](crate::api::quantization::Calibration::MinMax).
    ...
    /// ```rust
    /// use ruda_tensor::api::backend::Backend;
    /// use ruda_tensor::api::{Tensor, Shape};
    ///
    /// fn example<B: Backend>() {
    ///    let device = B::Device::default();
    ...
    /// ```rust
    /// use ruda_tensor::api::backend::Backend;
    /// use ruda_tensor::api::{Tensor, Bool, Shape};
    ///
    /// fn example<B: Backend>() {
    ///    let device = B::Device::default();
    ...
    ///
    /// ```ignore
    /// use ruda_tensor::api::ops::{GridSampleOptions, GridSamplePaddingMode, InterpolateMode};
    ///
