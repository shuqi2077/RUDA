use crate::api::{Bool, Cast, Int, Shape, Tensor, TensorData, TensorPrimitive, backend::Backend};
use alloc::{vec, vec::Vec};
use crate::get_device_settings;

use crate::api::try_read_sync;
    ...
    B: Backend,
{
    ...
    ///
    /// Supports casting to [`IntDType`](crate::api::IntDType) (producing an int tensor)
    /// or [`FloatDType`](crate::api::FloatDType) (producing a float tensor).
    ///
    /// # Example
    ///
    /// ```rust
    /// use ruda_tensor::api::backend::Backend;
    /// use ruda_tensor::api::{Tensor, Bool, IntDType, FloatDType};
    ///
    /// fn example<B: Backend>() {
    ///     let device = Default::default();
    ///     let bool_tensor = Tensor::<B, 1, Bool>::from_bool([true, false, true].into(), &device);
