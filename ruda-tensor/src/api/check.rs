use crate::api::{BasicOps, Shape, Slice, Tensor, backend::Backend, cast::ToElement};
use alloc::format;
use alloc::string::{String, ToString};
use alloc::vec;
use alloc::vec::Vec;
use crate::tensor::Ordered;
use ruda_core::tensor::DType;

/// The struct should always be used with the [check](crate::api::check) macro.
///
...
pub(crate) fn unwrap_shape_reshape(result: Result<Shape, ruda_core::tensor::shape::MetadataError>) -> Shape {
    match result {
        Ok(shape) => shape,
        // `shape.reshape(new_shape)` should only return `MetadataError::Invalid`.
        Err(ruda_core::tensor::shape::MetadataError::Invalid { reason }) => {
            macros::check!({
                TensorCheck::Ok.register("Reshape", crate::api::check::TensorError::new(reason))
            });
            unreachable!()
        }
        Err(e) => panic!("{e:?}"),
