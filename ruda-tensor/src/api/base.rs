...
use crate::api::backend::ExecutionError;
use crate::api::check::unwrap_shape_reshape;
...
use ruda_core::{tensor::SliceOps, stub::RwLock};
use core::iter::repeat;
use core::{fmt::Debug, ops::Range};
use serde::{Deserialize, Deserializer};

use crate::api::{AsIndex, Slice, SliceArg, wrap_index};
use crate::api::{
    Bool, ElementConversion, Float, Int, Shape, TensorData, TensorKind, TensorMetadata,
    backend::Backend, check,
};
use crate::api::{DType, Element};
use crate::api::{IndexingUpdateOp, TensorCreationOptions};
use crate::api::{cast::ToElement, check::TensorCheck};
    ...
    K::Elem: Element,
{
    ...
    /// ```rust
    /// use ruda_tensor::api::backend::Backend;
    /// use ruda_tensor::api::{Tensor, Shape, s};
    ///
    /// fn example<B: Backend>() {
    ///     let device = B::Device::default();
    ///
    ///     // Single dimension slicing - no brackets needed!
    ///     let tensor = Tensor::<B, 1, ruda_tensor::api::Int>::arange(0..10, &device);
    ...
    /// ```rust
    /// use ruda_tensor::api::backend::Backend;
    /// use ruda_tensor::api::{Tensor, Shape, Bool};
    ///
    /// fn example<B: Backend>() {
    ///   let device = B::Device::default();
    ///   let tensor = Tensor::<B, 2>::from_data([[1.0, -2.0, 3.0], [5.0, 9.0, 6.0]], &device);
    ...
    /// # Note
    ///
    /// For better performance, prefer using a [Transaction](crate::api::Transaction) when reading multiple
    ...
    pub fn try_into_data(self) -> Result<TensorData, ExecutionError> {
        crate::api::try_read_sync(self.into_data_async()).expect(
    ...
    /// ```rust
    /// use ruda_tensor::api::backend::Backend;
    /// use ruda_tensor::api::{Tensor, Bool};
    ///
    /// fn example<B: Backend>() {
    ///   let device = Default::default();
    ...
    pub fn into_scalar(self) -> K::Elem {
        crate::api::try_read_sync(self.into_scalar_async())
            .expect(
    ...
    pub fn try_into_scalar(self) -> Result<K::Elem, ExecutionError> {
        crate::api::try_read_sync(self.into_scalar_async()).expect(
            "Failed to read tensor data synchronously. This can happen on platforms
            that don't support blocking futures like WASM. Try into_scalar_async instead.",
        )
    }
    ...
    <K as BasicOps<B>>::Elem: Debug,
{
    ...
    fn fmt_inner_tensor(
        ...
        for i in start..end {
            ...
            let data = ruda_core::reader::try_read_sync(self.clone().slice(range).into_data_async());
