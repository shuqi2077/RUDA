/// Tests for loading `ruda-tensor.toml` from disk.
///
/// The `ruda` feature toggles whether `RudaTensorConfig` has a `ruda` sub-config. We still
/// want `ruda-tensor.toml` files that contain `[ruda.*]` sections to load cleanly either way —
/// serde's default behaviour is to ignore unknown top-level fields, and these tests lock
/// that in.
#[cfg(feature = "std")]
mod config_loading {
    use ruda_tensor_config::autodiff::AutodiffLogLevel;
    use ruda_tensor_config::fusion::FusionLogLevel;
    use ruda_tensor_config::{RudaTensorConfig, RuntimeConfig};
    use std::io::Write;
...
"#;
    ...
    fn load_empty_toml_uses_defaults() {
        ...
        let config = RudaTensorConfig::from_file_path(file.path()).expect("parse empty toml");
    ...
    fn load_minimal_toml_overrides_defaults() {
        ...
        let config = RudaTensorConfig::from_file_path(file.path()).expect("parse minimal toml");
    ...
    /// The important case: a single `ruda-tensor.toml` with both Ruda and Ruda sections must
    /// load whether or not the `ruda` feature is enabled on `ruda-core`.
    #[test]
    fn load_full_toml_ignores_ruda_when_feature_off() {
        let file = write_toml(FULL_TOML);
        let config = RudaTensorConfig::from_file_path(file.path()).expect("parse full toml");
