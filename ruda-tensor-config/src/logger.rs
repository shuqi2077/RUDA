...
use super::{autodiff::AutodiffLogLevel, base::RudaTensorConfig, fusion::FusionLogLevel};

static RUDA_LOGGER: spin::Mutex<Option<Logger>> = spin::Mutex::new(None);

#[cfg(feature = "std")]
std::thread_local! {
    static LOCAL_CONFIG: std::cell::OnceCell<Arc<RudaTensorConfig>> =
        const { std::cell::OnceCell::new() };
}

/// Returns the current [`RudaTensorConfig`], cached in thread-local storage on native targets.
///
/// On the first call from a given thread this fetches the global config via
/// [`RudaTensorConfig::get`] (which locks a spin mutex) and caches the `Arc` thread-locally.
/// Subsequent calls on the same thread only pay an `Arc` clone. On `no_std` builds this
/// is equivalent to [`RudaTensorConfig::get`].
///
/// Safe because [`RudaTensorConfig::set`] panics after the first read, so the cached snapshot
/// matches the global singleton for the whole program lifetime.
pub fn config() -> Arc<RudaTensorConfig> {
    #[cfg(feature = "std")]
    {
        LOCAL_CONFIG.with(|cell| cell.get_or_init(RudaTensorConfig::get).clone())
    }
    #[cfg(not(feature = "std"))]
    {
        RudaTensorConfig::get()
    }
}

/// Central logging utility for Ruda, managing one sink registry shared across subsystems.
#[derive(Debug)]
pub struct Logger {
    sinks: LoggerSinks,
    ...
    pub config: Arc<RudaTensorConfig>,
}
    ...
    F: FnOnce() -> String,
{
    ...
    let msg = f();
    let mut guard = RUDA_LOGGER.lock();
