...
use ruda_tensor::ExecutionError;
use ruda_tensor::IntDType;
use ruda_tensor::Scalar;
use ruda_tensor::tensor::BoolTensor;
use ruda_tensor::tensor::IntTensor;
use ruda_tensor::{BoolDType, FloatDType};
use ruda_tensor::{Shape, TensorData, TensorMetadata, ops::BoolTensorOps};

impl<E: TchElement> BoolTensorOps<Self> for LibTorch<E> {
    fn bool_from_data(data: TensorData, device: &LibTorchDevice) -> TchTensor {
        match data.dtype {
            ruda_tensor::DType::Bool(BoolStore::Native) => {
    ...
    fn bool_slice(tensor: TchTensor, slices: &[ruda_tensor::Slice]) -> TchTensor {
