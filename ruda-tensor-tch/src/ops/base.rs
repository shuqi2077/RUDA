...
use ruda_tensor::{Shape, TensorMetadata};
use tch::Scalar;
...
impl TchOps {
    ...
    pub fn slice_with_steps(tensor: TchTensor, slices: &[ruda_tensor::Slice]) -> TchTensor {
    ...
    fn generate_slice_indices(slice: &ruda_tensor::Slice, dim_size: usize) -> Vec<i64> {
        let step = slice.step;
        let range = slice.to_range(dim_size);
