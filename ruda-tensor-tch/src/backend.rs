...
use ruda_tensor::backend::{Backend, BackendTypes, DeviceId, DeviceOps, ExecutionError};
...
///
/// ```no_run
/// use ruda_tensor_tch::LibTorchDevice;
///
...
impl ruda_tensor::Device for LibTorchDevice {
...
/// [ruda-tensor-tch](https://github.com/shuqi2077/RUDA/blob/main/THIRD_PARTY_NOTICES.md).
///
...
impl<E: TchElement> Backend for LibTorch<E> {
    ...
    fn sync(device: &Self::Device) -> Result<(), ExecutionError> {
        match device {
            ...
            _ => {
                ...
                ruda_tensor::read_sync(Self::int_into_data(Self::int_zeros(
                    [1].into(),
                    device,
                    <Self::IntElem as ruda_tensor::Element>::dtype().into(),
                )))
                .unwrap();
            }
        };
    ...
    fn dtype_usage(
        ...
        if dtype.try_into_kind().is_ok() {
            ...
            ruda_tensor::DTypeUsageSet::empty()
        }
    }
