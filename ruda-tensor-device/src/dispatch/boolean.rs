...
use ruprim::elementwise::binary::numeric::{AndOp, OrOp};
use ruda_tensor::{
    ExecutionError, Slice,
    ops::BoolTensorOps,
    tensor::{BoolTensor, Device, FloatTensor, IntTensor},
};
use ruda_tensor::{Scalar, Shape, TensorData};
use ruda_core::tensor::{BoolDType, BoolStore, DType, FloatDType, IntDType};
use ruda_kernel::dsl::prelude::InputScalar;
use std::ops::Range;

use super::{expand, numeric, permute, unfold};

impl<R, F, I, BT> BoolTensorOps<Self> for DeviceBackend<R, F, I, BT>
where
    R: DeviceRuntime,
    F: FloatElement,
    I: IntElement,
    BT: BoolElement,
{
    ...
    fn bool_into_int(tensor: BoolTensor<Self>, out_dtype: IntDType) -> IntTensor<Self> {
        ruprim::elementwise::cast::bool_cast(tensor, out_dtype.into())
    }
    ...
    fn bool_equal(lhs: BoolTensor<Self>, rhs: BoolTensor<Self>) -> BoolTensor<Self> {
        let dtype = lhs.dtype;
        ruprim::elementwise::comparison::equal(lhs, rhs, dtype)
    }
    ...
    fn bool_not(tensor: BoolTensor<Self>) -> BoolTensor<Self> {
        ...
        ruprim::elementwise::comparison::equal_elem(tensor, scalar, dtype)
    }

    fn bool_and(lhs: BoolTensor<Self>, rhs: BoolTensor<Self>) -> BoolTensor<Self> {
        ruprim::elementwise::binary::numeric::launch_binop::<R, AndOp>(lhs, rhs)
    }

    fn bool_or(lhs: BoolTensor<Self>, rhs: BoolTensor<Self>) -> BoolTensor<Self> {
        ruprim::elementwise::binary::numeric::launch_binop::<R, OrOp>(lhs, rhs)
    }
    ...
    fn bool_select_or(
        ...
        ruprim::indexing::select_assign(tensor, dim, indices, value, true)
    }

    fn bool_flip(tensor: BoolTensor<Self>, axes: &[usize]) -> BoolTensor<Self> {
        let dtype = tensor.dtype;
        ruprim::indexing::flip(tensor, axes, dtype)
    }
    ...
    fn bool_mask_where(
        ...
        let dtype = tensor.dtype;
        ruprim::elementwise::mask::mask_where_auto(tensor, mask, value, dtype)
    }
    ...
    fn bool_mask_fill(
        ...
        let dtype = tensor.dtype;
        ruprim::elementwise::mask::mask_fill_auto(tensor, mask, InputScalar::new(value, dtype), dtype)
    }
    ...
    fn bool_scatter_or(
        ...
        ruprim::indexing::scatter(dim, tensor, indices, value, true)
    }

    fn bool_equal_elem(lhs: BoolTensor<Self>, rhs: Scalar) -> BoolTensor<Self> {
        let dtype = lhs.dtype;
        ruprim::elementwise::comparison::equal_elem(lhs, InputScalar::new(rhs, dtype), dtype)
    }
}
