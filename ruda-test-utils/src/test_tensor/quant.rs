...
use ruda_kernel::quantization::scheme::{QuantLevel, QuantStore};

use crate::{HostData, QuantizationInfo, TestInput, TestTensor};

/// Quantize an already-built [`TestTensor`] in place: launches the ruda-kernel quantization
...
pub(crate) fn apply_quantization(
    ...
    let output_storage_type = match &scheme.store {
        QuantStore::PackedU32(_) => StorageType::Scalar(ElemType::UInt(ruda_kernel::dsl::ir::UIntKind::U32)),
    ...
    ruda_kernel::quantization::quantize::launch_ref(
        client,
...
/// matching the reference pattern used by the ruda-kernel quantization symmetric tests.
