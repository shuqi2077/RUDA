...
impl Default for RudaTestConfig {
    fn default() -> Self {
        Self {
            test: TestSection {
...
pub fn config() -> &'static RudaTestConfig {
    static CACHE: OnceLock<RudaTestConfig> = OnceLock::new();
    CACHE.get_or_init(load_config)
}

fn load_config() -> RudaTestConfig {
    let Some(path) = find_ruda_test_toml() else {
        return RudaTestConfig::default();
    };
    let text = std::fs::read_to_string(&path)
        .unwrap_or_else(|e| panic!("cannot read {}: {e}", path.display()));
    parse_ruda_test_toml(&text)
        .unwrap_or_else(|e| panic!("invalid ruda-test.toml ({}): {e}", path.display()))
}

fn find_ruda_test_toml() -> Option<PathBuf> {
    let mut cur = std::env::current_dir().ok()?;
    loop {
        let candidate = cur.join("ruda-test.toml");
...
fn parse_ruda_test_toml(text: &str) -> Result<RudaTestConfig, String> {
    let sections = parse_sections(text)?;

    let mut cfg = RudaTestConfig::default();
...
fn strip_comment(line: &str) -> &str {
    ...
    // scalars and strings without '#' inside; that's enough for ruda-test.toml.
...
filter = "0,1-2"
"#;
        let cfg = parse_ruda_test_toml(text).unwrap();
    ...
    fn empty_file_gives_defaults() {
        let cfg = parse_ruda_test_toml("").unwrap();
    ...
    fn rejects_unknown_section() {
        let err = parse_ruda_test_toml("[bogus]\nx=1\n").unwrap_err();
        assert!(err.contains("unknown section"), "{}", err);
    }

    #[test]
    fn rejects_unknown_key() {
        let err = parse_ruda_test_toml("[print]\nbogus = true\n").unwrap_err();
        assert!(err.contains("unknown key"), "{}", err);
    }

    #[test]
    fn rejects_bad_policy() {
        let err = parse_ruda_test_toml("[test]\npolicy = \"loose\"\n").unwrap_err();
