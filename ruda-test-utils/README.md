...
## Configuration: `ruda-test.toml`

Configure test policy and tensor printing with a `ruda-test.toml` file at
the workspace root. On first access, the loader walks up from the current
working directory until it finds the file, then caches the configuration.
...
With printing enabled and `force-fail = true`, the `correct` and `strict`
policies reject passing outcomes and compilation errors, even if rendering
was skipped. The `fail-if-run` policy is unchanged by this setting.
...
- Two tensors of the **same rank and shape** → finite values are colored green
  when `Δ ≤ max(ε, ε × |expected|)` and red otherwise. With `show-expected = true` the cell shows
...
- Printing filter rank ≠ tensor rank → rendering is skipped. A mismatched
  comparison filter returns `ValidationResult::Error`.
...
`assert_equals_approx` returns a `ValidationResult`, collecting up to **8**
mismatches plus aggregate stats. Calling `.as_test_outcome().enforce()`
applies the test policy and panics on a rejected result:
...
Run from the RUDA workspace root, selecting one test runtime:

```sh
cargo test --locked -p ruda-test-utils --test lib --features ruda-test-runtime/cuda
```

The runtime feature may be `cpu`, `cuda`, `hip`, or `wgpu`.
