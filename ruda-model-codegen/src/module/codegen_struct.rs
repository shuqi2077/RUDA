...
impl ModuleCodegen for StructModuleCodegen {
    ...
    fn gen_num_params(&self, paths: &crate::DerivePaths) -> TokenStream {
        ...
        let body = self.gen_fields_fn(|name, field_type| {
            if field_type.is_parameter_module() || field_type.maybe_generic_module() {
                quote! {
                    num_params += #model::module::Module::<B>::num_params(&self.#name);
                }
            } else {
    ...
    fn gen_visit(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        ...
        let body = self.gen_fields_fn(|name, field_type| {
            if field_type.is_parameter_module() || field_type.maybe_generic_module() {
                ...
                quote! {
                    ...
                    #model::module::Module::visit(&self.#name, visitor);
        ...
        quote! {
            fn visit<Visitor: #model::module::ModuleVisitor<B>>(&self, visitor: &mut Visitor) {
                #body
            }
        }
    }

    fn gen_collect_devices(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let body = self.gen_fields_fn(|name, field_type| {
            if field_type.is_module || field_type.maybe_generic_module() {
                quote! {
                    let devices = #model::module::Module::<B>::collect_devices(&self.#name, devices);
                }
            } else {
                quote! {}
            }
        ...
        quote! {
            fn collect_devices(
                &self,
                devices: #model::module::Devices<B>
            ) -> #model::module::Devices<B> {
                #body
                devices
            }
        }
    ...
    fn gen_to_device(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let (names, body) = self.gen_fields_fn_names(|name, field_type| {
            if field_type.is_module || field_type.maybe_generic_module() {
                quote! {
                    let #name = #model::module::Module::<B>::to_device(self.#name, device);
                }
            } else {
    ...
    fn gen_fork(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let (names, body) = self.gen_fields_fn_names(|name, field_type| {
            if field_type.is_module || field_type.maybe_generic_module() {
                quote! {
                    let #name = #model::module::Module::<B>::fork(self.#name, device);
                }
            } else {
                quote! { let #name = self.#name; }
            }
    ...
    fn gen_map(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let struct_name = self.name.to_string();
        let container_type = format!("Struct:{}", struct_name);
        let (names, body) = self.gen_fields_fn_names(|name, field_type| {
            if field_type.is_parameter_module() || field_type.maybe_generic_module() {
                ...
                quote! {
                    ...
                    let #name = #model::module::Module::<B>::map(self.#name, mapper);
        ...
        quote! {
            fn map<Mapper: #model::module::ModuleMapper<B>>(self, mapper: &mut Mapper) -> Self {
                #body
                Self { #(#names),* }
            }
        }
    ...
    fn gen_valid(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let (names, body) = self.gen_fields_fn_names(|name, field_type| {
            if field_type.is_module || field_type.maybe_generic_module() {
                quote! {
                    let #name = #model::module::AutodiffModule::<B>::valid(&self.#name);
                }
            } else {
    ...
    fn gen_from_inner(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let (names, body) = self.gen_fields_fn_names(|name, field_type| {
            if field_type.is_module || field_type.maybe_generic_module() {
                quote! {
                    let #name = #model::module::AutodiffModule::<B>::from_inner(#name);
                }
            } else {
                quote! { let #name = #name; }
            }
    ...
    fn gen_into_record(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let body = self.gen_fields_fn(|name, field_type| {
            if field_type.is_persistent_module() || field_type.maybe_generic_module() {
                quote! { #name: #model::module::Module::<B>::into_record(self.#name), }
            } else {
                match field_type.attr {
                    // Default (None) gets skipped
                    None | Some(ModuleFieldAttribute::Skip) => {
                        quote! { #name: #model::module::EmptyRecord::new(), }
                    }
                }
            }
        });
    ...
    fn gen_load_record(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let body = self.gen_fields_fn(|name, field_type| {
            if field_type.is_persistent_module() || field_type.maybe_generic_module() {
                quote! { #name: #model::module::Module::<B>::load_record(self.#name, record.#name), }
            } else {
                match field_type.attr {
                    // Default (None) gets skipped
                    None | Some(ModuleFieldAttribute::Skip) => {
    ...
    fn gen_display(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let struct_name = self.name.to_string();
        ...
        quote! {
            fn content(&self, mut content: #model::module::Content) -> Option<#model::module::Content> {
                content
...
impl StructModuleCodegen {
    pub fn from_ast(ast: &syn::DeriveInput, _paths: &crate::DerivePaths) -> syn::Result<Self> {
...
fn type_matches_ident(ty: &syn::Type, idents: &[&str]) -> bool {
    if let syn::Type::Path(type_path) = ty {
        // Look at the last segment of the path (e.g., 'Param' in 'ruda_model::module::Param')
