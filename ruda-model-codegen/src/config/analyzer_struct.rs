...
impl ConfigAnalyzer for ConfigStructAnalyzer {
    ...
    fn gen_serde_impl(&self, paths: &crate::DerivePaths) -> TokenStream {
        ...
        let serialize_gen = self.gen_serialize_fn(&struct_name, &struct_gen, &names, paths);
        let deserialize_gen = self.gen_deserialize_fn(&struct_name, &struct_gen, &names, paths);
