...
impl ConfigEnumAnalyzer {
    ...
    fn gen_serde_enum(&self, paths: &crate::DerivePaths) -> TokenStream {
    ...
    fn gen_serialize_fn(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let enum_name = self.serde_enum_ident();
        ...
        quote! {
            impl #model::serde::Serialize for #name {
                fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
                where
                    S: #model::serde::Serializer {
    ...
    fn gen_deserialize_fn(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let enum_name = self.serde_enum_ident();
        let variants = self.data.variants.iter().map(|variant| {
            let variant_name = &variant.ident;
        ...
        quote! {
            impl<'de> #model::serde::Deserialize<'de> for #name {
                fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
                where
                    D: #model::serde::Deserializer<'de> {
...
impl ConfigAnalyzer for ConfigEnumAnalyzer {
    fn gen_serde_impl(&self, paths: &crate::DerivePaths) -> TokenStream {
        let struct_gen = self.gen_serde_enum(paths);
        let serialize_gen = self.gen_serialize_fn(paths);
        let deserialize_gen = self.gen_deserialize_fn(paths);
    ...
    fn gen_display_impl(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let name = &self.name;
        ...
        quote! {
            impl core::fmt::Display for #name {
                fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
                    f.write_str(&#model::config::config_to_json(self))
                }
            }
        }
    }

    fn gen_config_impl(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        let name = &self.name;

        quote! {
            impl #model::config::Config for #name {
            }
        }
    }
}
