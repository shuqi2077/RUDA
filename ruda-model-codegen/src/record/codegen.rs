...
pub(crate) fn generate_record<G: RecordItemCodegen>(ast: &syn::DeriveInput, paths: &crate::DerivePaths) -> TokenStream {
    let record_gen: syn::Result<RecordCodegen<G>> = RecordCodegen::from_ast(ast, paths);
    match record_gen {
        Ok(record_gen) => {
            let item_type = record_gen.gen_record_type(paths);
            let record_impl = record_gen.gen_impl_record(paths);
...
impl<G: RecordItemCodegen> RecordCodegen<G> {
    ...
    pub(crate) fn gen_record_type(&self, paths: &crate::DerivePaths) -> TokenStream {
        ...
        self.codegen
            .gen_item_type(&self.ty.item, &generics, self.ty.has_backend, paths)
    }

    /// Generate the implementation for the Record trait.
    pub(crate) fn gen_impl_record(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        // Capture the record type's generics and bounds in where clauses
        let item_generics = self.record_item_generics(paths);
        ...
        let impl_generics = if let Some(impl_generic) = self.impl_generics(paths) {
            impl_generic
        } else {
            quote! { #impl_generics }
        };
        ...
        let into_item_fn = self.codegen.gen_into_item(name_item, paths);
        let from_item_fn = self.codegen.gen_from_item(paths);

        // Return the generated stream of token trees (i.e., code to be generated)
        let name = &self.ty.name;
        quote! {
            impl #impl_generics #model::record::Record<B> for #name #ty_generics #where_clause {
                type Item<S: #model::record::PrecisionSettings> = #name_item #ty_generics_item;
    ...
    fn impl_generics(&self, paths: &crate::DerivePaths) -> Option<TokenStream> {
        let model = &paths.model;
    ...
    fn record_item_generics(&self, paths: &crate::DerivePaths) -> Generics {
        let model = &paths.model;
        let param: syn::Generics = parse_quote! { <S: #model::record::PrecisionSettings >};
        let mut generics = self.ty.generics.clone();
        for param in param.params.into_iter() {
    ...
    pub(crate) fn from_ast(ast: &syn::DeriveInput, paths: &crate::DerivePaths) -> syn::Result<Self> {
        Ok(Self {
            ty: RecordType::from_ast(ast, paths),
            codegen: G::from_ast(ast, paths)?,
        })
    }
}
...
impl RecordType {
    fn from_ast(ast: &syn::DeriveInput, _paths: &crate::DerivePaths) -> Self {
