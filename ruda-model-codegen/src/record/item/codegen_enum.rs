...
impl RecordItemCodegen for EnumRecordItemCodegen {
    ...
    fn gen_item_type(
        ...
        for variant in self.variants.iter() {
            ...
            variants.extend(quote! {
                ...
                #name(<#ty as #model::record::Record<B>>::Item<S>),
            });

            // Item types must implement serialization/deserialization
            serde_bounds.extend(quote! {
                <#ty as #model::record::Record<B>>::Item<S>: #model::serde::Serialize + #model::serde::de::DeserializeOwned,
            });
            clone_bounds.push(parse_quote! {
                <#ty as #model::record::Record<B>>::Item<S>: Clone
            });
        ...
        let deserialize_impl = super::enum_deserialize::generate(
            item_name, &generics, &self.variants, paths,
        );
    ...
    fn gen_into_item(&self, _item_name: &Ident, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        ...
        for variant in self.variants.iter() {
            ...
            into_item_match_arms.extend(quote! {
                Self::#name(record) => Self::Item::#name(#model::record::Record::<B>::into_item::<S>(record)),
            });
        }
    ...
    fn gen_from_item(&self, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        ...
        for variant in self.variants.iter() {
            ...
            from_item_match_arms.extend(quote! {
                Self::Item::#name(item) => Self::#name(#model::record::Record::<B>::from_item::<S>(item, device)),
            });
        }
