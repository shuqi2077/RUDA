...
impl RecordItemCodegen for StructRecordItemCodegen {
    ...
    fn gen_item_type(
        ...
        for field in self.fields.iter() {
            ...
            fields.extend(quote! {
                ...
                pub #name: <#ty as #model::record::Record<B>>::Item<S>,
            });
    ...
    fn gen_into_item(&self, item_name: &Ident, paths: &crate::DerivePaths) -> TokenStream {
        let model = &paths.model;
        ...
        for field in self.fields.iter() {
            ...
            body_into_item.extend(quote! {
                #name: #model::record::Record::<B>::into_item::<S>(self.#name),
            });
        }
    ...
    fn gen_from_item(&self, paths: &crate::DerivePaths) -> TokenStream {
        ...
        for field in self.fields.iter() {
            ...
            body_from_item.extend(quote! {
                #name: #model::record::Record::<B>::from_item::<S>(item.#name, device),
            });
        }
