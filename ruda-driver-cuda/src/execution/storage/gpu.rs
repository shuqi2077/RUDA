...
impl PtrBindings {
    ...
    fn new() -> Self {
        Self {
            // Binding slots are ordinary Rust integers even before registration.
            // Do not expose uninitialized elements by increasing a Vec's length.
            slots: vec![0; crate::device::CUDA_MAX_BINDINGS as usize],
            cursor: 0,
        }
    }
...
#[cfg(test)]
mod binding_safety_tests {
    use super::*;

    #[test]
    fn pointer_slots_are_initialized_before_registration() {
        let bindings = PtrBindings::new();
        assert!(!bindings.slots.is_empty());
        assert!(bindings.slots.iter().all(|&ptr| ptr == 0));
    }

    #[test]
    fn pointer_ring_wraps_without_moving_its_allocation() {
        let mut bindings = PtrBindings::new();
        let base = bindings.slots.as_ptr();
        let len = bindings.slots.len();
        for i in 0..(len * 2 + 1) {
            assert_eq!(*bindings.register(i as u64 + 1), i as u64 + 1);
            assert_eq!(bindings.slots.as_ptr(), base);
        }
        assert_eq!(bindings.cursor, 1);
    }
}
