//! Two independent read-only-input branches and an explicit join. Not a model benchmark.
...
fn main()->Result<(),Box<dyn std::error::Error>> {
    ...
    let mut graph=unsafe {CudaGraph::build_dag_tracked(&c,nodes,vec![vec![],vec![],vec![0,1]])}?;
