...
#[cfg(feature = "distributed")]
use ruda_tensor::{DeviceOps, distributed::DistributedBackend};
...
impl Gradients {
    ...
    /// Repeated calls after this backward pass has synchronized are no-ops.
    pub fn sync_collective<B: DistributedBackend>(&mut self, device: &B::Device) {
        if let Some(registration) = self.distributed_registration.as_mut() {
            registration.sync(core::any::TypeId::of::<B>(), device.id(), &mut self.container);
        }
    }
}
