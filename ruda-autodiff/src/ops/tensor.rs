...
use ruda_tensor::{Scalar, ops::unfold::calculate_unfold_windows};
use ruda_core::tensor::{BoolDType, FloatDType, IntDType, Shape, Slice};

#[cfg(feature = "distributed")]
use ruda_tensor::distributed::DistributedParams;
...
impl<B: Backend, C: CheckpointStrategy> FloatTensorOps<Self> for Autodiff<B, C> {
    fn float_grid_sample_2d(
        ...
        options: ruda_tensor::ops::GridSampleOptions,
    ) -> FloatTensor<Self> {
        super::grid_sample::sample::<B, C>(tensor, grid, options)
    }
    ...
    fn float_random(
        shape: Shape,
        distribution: ruda_tensor::Distribution,
        device: &Device<Self>,
        dtype: FloatDType,
    ) -> FloatTensor<Self> {
    ...
    fn float_scatter_nd(
        ...
        use ruda_tensor::tensor::IndexingUpdateOp;
            ...
            fn backward(
                ...
                unary::<B, _>(ops.parents, ops.node, grads, |grad| {
                    ...
                    B::float_scatter_nd(
                        ...
                        indices,
                        grad,
                        ruda_tensor::tensor::IndexingUpdateOp::Add,
                    )
                });
            }
        }
    ...
    fn float_mask_fill(
        ...
        impl<B: Backend> Backward<B, 1> for MaskFill {
            ...
            fn backward(
                ...
                let (mask, shape) = ops.state;
                unary::<B, _>(ops.parents, ops.node, grads, |grad| {
                    let grad = B::float_mask_fill(grad, mask, 0f32.into());
                    broadcast_shape::<B>(grad, &shape)
                });
            }
        }
        ...
        {
            OpsKind::Tracked(prep) => prep.finish(
                (mask.clone(), tensor.primitive.shape()),
    ...
    fn float_prod(tensor: FloatTensor<Self>) -> FloatTensor<Self> {
        super::product::product::<B, C>(tensor, None)
    }

    fn float_prod_dim(tensor: FloatTensor<Self>, dim: usize) -> FloatTensor<Self> {
        super::product::product::<B, C>(tensor, Some(dim))
    }
    ...
    fn float_cumprod(tensor: FloatTensor<Self>, dim: usize) -> FloatTensor<Self> {
        super::cumprod::cumprod::<B, C>(tensor, dim)
    }
    ...
    fn float_topk(tensor: FloatTensor<Self>, dim: usize, k: usize) -> FloatTensor<Self> {
        if !tensor.is_tracked() {
            return AutodiffTensor::new(B::float_topk(tensor.primitive, dim, k));
        }
        let device = B::float_device(&tensor.primitive);
        let dtype = get_device_settings::<B>(&device).int_dtype;
        let indices = B::float_argtopk(tensor.primitive.clone(), dim, k, dtype);
        // Gather retains the selected indices for its scatter-add backward,
        // including the backend's lowest-index tie-breaking rule.
    ...
    fn float_powi_scalar(tensor: FloatTensor<Self>, value: Scalar) -> FloatTensor<Self> {
        super::integer_power::scalar::<B, C>(tensor, value, false)
    }

    fn float_powi_scalar_impl(tensor: FloatTensor<Self>, value: Scalar) -> FloatTensor<Self> {
        super::integer_power::scalar::<B, C>(tensor, value, true)
    }
    ...
    fn float_rsqrt(tensor: FloatTensor<Self>) -> FloatTensor<Self> {
        ...
        retro_unary!(RetroRsqrt, B::float_rsqrt);

        impl<B: Backend> Backward<B, 1> for Rsqrt {
            type State = NodeId;
            ...
            fn backward(
                ...
                let input = checkpointer.retrieve_node_output(ops.state);
                let output = B::float_rsqrt(input);
                unary::<B, _>(ops.parents, ops.node, grads, |grad| {
                    let ruda = B::float_powi_scalar(output, 3.into());
                    let scale = B::float_mul_scalar(grad, (-0.5f32).into());
                    B::float_mul(scale, ruda)
                });
            }
        }
        ...
        match Rsqrt
            .prepare::<C>([tensor.node.clone()])
            .memory_bound()
            .retro_forward(RetroRsqrt::<B>::new(tensor.node.id))
            .parents([&tensor])
            .stateful()
        {
            OpsKind::Tracked(mut prep) => {
                let state = prep.checkpoint(&tensor);
                prep.finish(state, B::float_rsqrt(tensor.primitive))
            }
            OpsKind::UnTracked(prep) => prep.finish(B::float_rsqrt(tensor.primitive)),
        }
    }
    ...
    fn float_powi(lhs: FloatTensor<Self>, rhs: IntTensor<Self>) -> FloatTensor<Self> {
        super::integer_power::tensor::<B, C>(lhs, rhs)
    }
    ...
    fn float_repeat_dim(tensor: FloatTensor<Self>, dim: usize, times: usize) -> FloatTensor<Self> {
        ...
        impl<B: Backend> Backward<B, 1> for Repeat {
            ...
            fn backward(
                ...
                let (shape, dim, times) = ops.state;

                unary::<B, _>(ops.parents, ops.node, grads, |grad| {
                    if times == 0 || shape.num_elements() == 0 {
                        return B::float_zeros(
                            shape,
                            &B::float_device(&grad),
                            grad.dtype().into(),
                        );
        ...
        {
            OpsKind::Tracked(prep) => prep.finish(
                (tensor.primitive.shape(), dim, times),
    ...
    fn float_cast(tensor: FloatTensor<Self>, dtype: ruda_core::tensor::FloatDType) -> FloatTensor<Self> {
        #[derive(Debug)]
        struct Cast;
