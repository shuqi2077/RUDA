...
use ruda_tensor::{Backend, ops::ActivationOps, tensor::FloatTensor};
    ...
    fn silu(tensor: FloatTensor<Self>) -> FloatTensor<Self> {
        ...
        impl<B: Backend> Backward<B, 1> for Silu {
            ...
            fn backward(
                ...
                unary::<B, _>(ops.parents, ops.node, grads, |grad| {
                    let sigmoid = B::sigmoid(input.clone());
                    let direct = B::float_mul(grad.clone(), sigmoid.clone());
                    let through_sigmoid = B::sigmoid_backward(sigmoid, B::float_mul(grad, input));
                    B::float_add(direct, through_sigmoid)
                });
            }
        }
            ...
            .stateful()
        {
            OpsKind::Tracked(mut prep) => {
                ...
                prep.finish(state, B::silu(tensor.primitive))
            }
