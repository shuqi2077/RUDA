...
impl LrScheduler for ComposedLrScheduler {
    ...
    fn load_record<B: Backend>(mut self, record: Self::Record<B>) -> Self {
        self.try_load_record::<B>(record)
            .unwrap_or_else(|error| panic!("Invalid scheduler state: {error}"));
        self
    }
}

impl ComposedLrScheduler {
    /// Restore scheduler state after checking the child count and variant order.
    ///
    /// An incompatible record leaves the scheduler unchanged. The original
    /// scheduler configurations and reduction must be supplied when constructing
    /// the scheduler; they are not part of the state record.
    pub fn try_load_record<B: Backend>(
        &mut self,
        record: ComposedLrSchedulerRecord<B>,
    ) -> Result<(), String> {
        if self.schedulers.len() != record.schedulers.len() {
            return Err(format!(
                "Scheduler count mismatch: expected {}, got {}",
                self.schedulers.len(),
                record.schedulers.len()
            ));
        }
        for (index, (scheduler, state)) in self
            .schedulers
            .iter()
            ...
            .enumerate()
        {
            let compatible = matches!(
                (scheduler, state),
                (LrSchedulerItem::Linear(_), LrSchedulerRecord::Linear(_))
                    | (LrSchedulerItem::Cosine(_), LrSchedulerRecord::Cosine(_))
                    | (LrSchedulerItem::Exponential(_), LrSchedulerRecord::Exponential(_))
                    | (LrSchedulerItem::Noam(_), LrSchedulerRecord::Noam(_))
            );
            if !compatible {
                return Err(format!("Scheduler variant mismatch at index {index}"));
            }
        }
        ...
        self.schedulers = self
            ...
            .map(|scheduler| match scheduler {
                ...
                _ => unreachable!("scheduler variants were validated before loading"),
            })
            .collect();
