...
impl OpArgs {
    fn generate_into(&self) -> TokenStream {
        ...
        for field in self.data.as_ref().take_struct().unwrap().fields {
            ...
            tokens.extend(quote![args.extend(crate::ir::FromArgList::as_arg_list(&self.#ident));]);
        }
        quote! {
            #tokens
            args
    ...
    fn generate_from(&self) -> TokenStream {
        ...
        for field in self.data.as_ref().take_struct().unwrap().fields {
            let ident = field.ident.as_ref().unwrap();
            tokens.extend(quote![#ident: crate::ir::FromArgList::from_arg_list(&mut args),]);
        }
        quote! {
    ...
    fn generate_args_impl(&self) -> TokenStream {
        ...
        quote![impl #generics crate::ir::OperationArgs for #name #generic_names #where_clause {
            fn from_args(args: &[Variable]) -> Option<Self> {
                Some({#from})
            }

            fn as_args(&self) -> Option<alloc::vec::Vec<crate::ir::Variable>> {
                Some({#into})
            }
        }]
    }
