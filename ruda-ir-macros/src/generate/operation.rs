...
impl Operation {
    ...
    fn generate_opcode_impl(&self) -> TokenStream {
        ...
        let match_variants = variants.iter().map(|variant| {
            ...
            if variant.nested.is_present() {
                quote![Self::#ident(child) => #opcode::#ident(crate::ir::OperationReflect::op_code(child))]
    ...
    fn generate_args_impl(&self) -> TokenStream {
        ...
        let match_variants = variants.iter().map(|variant| {
            let ident = &variant.ident;
            if variant.nested.is_present() {
                quote![Self::#ident(child) => crate::ir::OperationReflect::args(child)]
            } else if variant.fields.is_empty() {
                ...
                let body = quote![{
                    ...
                    #(args.extend(crate::ir::FromArgList::as_arg_list(#names));)*
                    Some(args)
                }];
                quote![Self::#ident { #(#names),* } => #body]
            } else {
                quote![Self::#ident(args) => crate::ir::OperationArgs::as_args(args)]
            }
        });
        quote! {
            match self {
    ...
    fn generate_from_args_impl(&self) -> TokenStream {
        ...
        let match_variants = variants.iter().map(|variant| {
            let ident = &variant.ident;
            if variant.nested.is_present() {
                quote![#opcode::#ident(child) => Some(Self::#ident(crate::ir::OperationReflect::from_code_and_args(child, args)?))]
            } else if variant.fields.is_empty() {
                ...
                let fields = variant
                    ...
                    .iter()
                    .map(|it| it.ident.clone().unwrap())
                    .map(|it| quote![#it: crate::ir::FromArgList::from_arg_list(&mut args)]);
                quote![#opcode::#ident => {
                    let mut args: alloc::collections::VecDeque<crate::ir::Variable> = args.iter().cloned().collect();
                    Some(Self::#ident {
                        #(#fields),*
                    })
                }]
            } else {
                quote![#opcode::#ident => crate::ir::OperationArgs::from_args(args).map(Self::#ident)]
            }
        });
        quote! {
            match op_code {
        ...
        if flag_global(self).is_present() {
            ...
            let variants = variants.iter().map(|variant| {
                ...
                if variant.nested.is_present() && !value {
                    quote![Self::#ident(child) => crate::ir::OperationReflect::#func_ident(child)]
                } else if variant.fields.is_empty() {
    ...
    fn generate_operation_impl(&self) -> TokenStream {
        ...
        quote![impl #generics crate::ir::OperationReflect for #name #generic_names #where_clause {
            type OpCode = #opcode_name;

            fn op_code(&self) -> Self::OpCode {
                #opcode_impl
            }
            fn args(&self) -> Option<alloc::vec::Vec<crate::ir::Variable>> {
                #args_impl
            }
            fn from_code_and_args(op_code: Self::OpCode, args: &[crate::ir::Variable]) -> Option<Self> {
                #from_args_impl
            }
    ...
    fn generate_opcode(&self) -> TokenStream {
        ...
        let variants = variants.iter().map(|variant| {
            ...
            if variant.nested.is_present() {
                ...
                quote![#ident(<#child_ty as crate::ir::OperationReflect>::OpCode)]
            } else {
                quote![#ident]
            }
        });
