...
fn type_hash_struct(
    ...
    quote! {
        impl #impl_generics crate::ir::TypeHash for #ident #ty_generics #where_clause {
...
fn write_field_hash(field: &TypeHashField) -> TokenStream {
    ...
    if let Some(ty) = &field.r#as {
        quote! {
            #field_name
            <#ty as crate::ir::TypeHash>::write_hash(hasher);
        }
        ...
        quote! {
            #field_name
            <#field_type as crate::ir::TypeHash>::write_hash(hasher);
        }
    }
}
