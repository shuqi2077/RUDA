...
use crate::optimizer::{Optimizer, VarId};
...
/// operator (bounds it to `0..m`), or `UNIT_POS` (bounded by `RudaDim`). Bounds can be transferred
    ...
    pub fn range_of(&self, opt: &Optimizer, var: &Variable) -> Range {
        match var.kind {
            ...
            VariableKind::Builtin(builtin) => match builtin {
                Builtin::UnitPos => Range::uint(opt.ruda_dim.num_elems() as u64 - 1),
                Builtin::UnitPosX => Range::uint(opt.ruda_dim.x as u64 - 1),
                Builtin::UnitPosY => Range::uint(opt.ruda_dim.y as u64 - 1),
                Builtin::UnitPosZ => Range::uint(opt.ruda_dim.z as u64 - 1),
                Builtin::RudaCount => Range::constant(opt.ruda_dim.num_elems() as u64),
                Builtin::RudaCountX => Range::constant(opt.ruda_dim.x as u64),
                Builtin::RudaCountY => Range::constant(opt.ruda_dim.y as u64),
                Builtin::RudaCountZ => Range::constant(opt.ruda_dim.z as u64),
                _ => Default::default(),
            },
            _ => Default::default(),
        }
