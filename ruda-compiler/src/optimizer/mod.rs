//! # `Ruda` Optimizer
//!
//! A library that parses `Ruda` IR into a
...
pub use crate::optimizer::analyses::liveness::shared::{SharedLiveness, SharedMemory};
...
pub struct Optimizer {
    ...
    /// The `RudaDim` used for range analysis
    pub(crate) ruda_dim: RudaDim,
...
impl Optimizer {
    /// Create a new optimizer with the scope, `RudaDim` and execution mode passed into the compiler.
    ...
    /// Create a new optimizer with the scope, `RudaDim` and execution mode passed into the compiler.
    /// Parses the scope and runs several optimization and analysis loops.
    pub fn shared_only(expand: Scope, ruda_dim: RudaDim) -> Self {
        let mut opt = Self {
            root_scope: expand.clone(),
            ruda_dim,
            allocator: expand.allocator.clone(),
...
mod test {
    ...
    use ruda_core::ir::{ElemType, ManagedVariable, Type, UIntKind, Variable, VariableKind};
    ...
    fn test_pre() {
        ...
        let opt = Optimizer::new(ctx, RudaDim::new_1d(1), vec![], vec![]);
        println!("{opt}")
    }
}
