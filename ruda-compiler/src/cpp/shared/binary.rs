...
impl<D: Dialect> Binary<D> for Powi {
    fn format_scalar<Lhs: Component<D>, Rhs: Component<D>>(
        ...
        let elem = item.elem;
        let exponent_type = rhs.elem();
        let lhs = lhs.to_string();
        let rhs = rhs.to_string();
        match elem {
            Elem::F16 | Elem::BF16 | Elem::F32 | Elem::TF32 | Elem::F64 => {
                let compute = if matches!(elem, Elem::F64) { "double" } else { "float" };
                let uint = Elem::<D>::U64;
                write!(f, "([]({compute} base, {exponent_type} exponent) {{ \
                    {uint} magnitude = static_cast<{uint}>(exponent); \
                    {compute} result = {compute}(1); \
                    if (exponent < 0) {{ magnitude = 0ull - magnitude; base = {compute}(1) / base; }} \
                    while (magnitude != 0) {{ \
                        if ((magnitude & 1ull) != 0) result = result * base; \
                        magnitude >>= 1; \
                        if (magnitude != 0) base = base * base; \
                    }} \
                    return {elem}(result); \
                }}({compute}({lhs}), {rhs}))")
            }
