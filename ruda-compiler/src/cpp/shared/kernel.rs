...
use ruda_core::{launch::RudaDim, ir::Id, kernel::Visibility};
...
pub struct ComputeKernel<D: Dialect> {
    ...
    pub info: ruda_core::arguments::Info,
...
impl<D: Dialect> Display for ComputeKernel<D> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        // Body --------------------------------------------------------------
        f.write_str(" {\n")?;
        compile_ruda_builtin_bindings_decl::<D>(f, &self.flags)?;
        write!(f, "{}", self.body)?;
        f.write_str("\n}")?;
...
fn compile_ruda_builtin_bindings_decl<D: Dialect>(
    f: &mut core::fmt::Formatter<'_>,
    settings: &Flags<D>,
) -> core::fmt::Result {
    ...
    if settings.indexes.absolute_pos {
        ...
        let ruda_count_x = Variable::<D>::RudaCountX.fmt_cast_to(ty);
        let ruda_count_y = Variable::<D>::RudaCountY.fmt_cast_to(ty);
        let ruda_dim_x = Variable::<D>::RudaDimX.fmt_cast_to(ty);
        let ruda_dim_y = Variable::<D>::RudaDimY.fmt_cast_to(ty);
        writeln!(
            f,
            "{ty} {variable} = (
                {absolute_pos_z} * {ruda_count_x} * {ruda_dim_x} * {ruda_count_y} * {ruda_dim_y})
                + ({absolute_pos_y} * {ruda_count_x} * {ruda_dim_x})
                + {absolute_pos_x};"
        )?;
    }

    if settings.indexes.ruda_dim {
        let variable = Variable::<D>::RudaDim;
        let ty = variable.item();
        ...
        writeln!(
            f,
            "{ty} {variable} = {ruda_dim_x} * {ruda_dim_y} * {ruda_dim_z};"
        )?;
    }

    if settings.indexes.ruda_count {
        let variable = Variable::<D>::RudaCount(settings.address_type.elem);
        let ty = variable.item();
        let ruda_count_x = Variable::<D>::RudaCountX.fmt_cast_to(ty);
        let ruda_count_y = Variable::<D>::RudaCountY.fmt_cast_to(ty);
        let ruda_count_z = Variable::<D>::RudaCountZ.fmt_cast_to(ty);
        writeln!(
            f,
            "{ty} {variable} = {ruda_count_x} * {ruda_count_y} * {ruda_count_z};"
        )?;
    }

    if settings.indexes.ruda_pos {
        let variable = Variable::<D>::RudaPos(settings.address_type.elem);
        let ty = variable.item();
        let ruda_pos_x = Variable::<D>::RudaPosX.fmt_cast_to(ty);
        let ruda_pos_y = Variable::<D>::RudaPosY.fmt_cast_to(ty);
        let ruda_pos_z = Variable::<D>::RudaPosZ.fmt_cast_to(ty);
        let ruda_count_x = Variable::<D>::RudaCountX.fmt_cast_to(ty);
        let ruda_count_y = Variable::<D>::RudaCountY.fmt_cast_to(ty);
        writeln!(
            f,
            "{ty} {variable} = ({ruda_pos_z} * {ruda_count_y} * {ruda_count_x}) + ({ruda_pos_y} * {ruda_count_x}) + {ruda_pos_x};"
        )?;
    }
    ...
    if settings.indexes.plane_dim_checked {
        ...
        writeln!(
            f,
            "{ty} {variable} = min({plane_dim}, {ruda_dim_x} * {ruda_dim_y} * {ruda_dim_z});"
        )?;
    }
