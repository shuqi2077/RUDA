...
use crate::cpp::{
    Dialect,
    shared::{
        self, AtomicKind, Component, RudaIndexFlags, DialectBindings, DialectRudaBuiltins,
        DialectIncludes, DialectInstructions, DialectTypes,
...
use core::panic;
use ruda_core::ir::{self as gpu, features::MmaConfig};
use std::fmt::Display;
...
impl DialectTypes<Self> for MslDialect {
    ...
    fn compile_type_definitions(
        f: &mut std::fmt::Formatter<'_>,
        items: &std::collections::HashSet<crate::cpp::shared::Item<Self>>,
...
void {kernel_name}("
        ...
        let builtins = vec![
            ...
            (
                flags.indexes.ruda_dim_tuple,
                Variable::<Self>::RudaDimBaseName,
            ),
            (
                flags.indexes.ruda_count_tuple,
                Variable::<Self>::RudaCountBaseName,
            ),
            ...
            (
                flags.indexes.ruda_pos_tuple,
                Variable::<Self>::RudaPosBaseName,
            ),
            ...
            (flags.indexes.plane_count, Variable::<Self>::PlaneCount),
        ];
...
impl DialectRudaBuiltins<Self> for MslDialect {
    ...
    fn builtin_rules(flags: &RudaIndexFlags) -> RudaIndexFlags {
        ...
        let ruda_count_tuple = flags.ruda_count_tuple || ruda_count || ruda_pos || absolute_pos;
        let ruda_dim_tuple = flags.ruda_dim_tuple || ruda_dim || absolute_pos || plane_dim_checked;
...
impl DialectWmmaCompiler<Self> for MslDialect {
    ...
    fn compile_wmma_fragment_declaration(
        f: &mut std::fmt::Formatter<'_>,
        var: &crate::cpp::shared::Variable<MslDialect>,
    ) -> std::fmt::Result {
