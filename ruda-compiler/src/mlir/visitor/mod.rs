...
use super::{
    external_function::add_external_function_to_module, shared_memories::SharedMemories,
};
...
impl<'a> Visitor<'a> {
    ...
    pub(self) fn insert_builtin_loop(
        ...
        let ruda_count_dim_x = block.muli(
            args.get(Builtin::RudaCountX),
            args.get(Builtin::RudaDimX),
            location,
        )?;
        let ruda_count_dim_y = block.muli(
            args.get(Builtin::RudaCountY),
            args.get(Builtin::RudaDimY),
            location,
        )?;
        let ruda_count_dim_x_usize = args.as_address_type(ruda_count_dim_x, &block, location);
        let ruda_count_dim_y_usize = args.as_address_type(ruda_count_dim_y, &block, location);
        let ruda_count_dim_xy_usize =
            block.muli(ruda_count_dim_x_usize, ruda_count_dim_y_usize, location)?;

        let ruda_count_x_usize =
            args.as_address_type(args.get(Builtin::RudaCountX), &block, location);
        let ruda_count_y_usize =
            args.as_address_type(args.get(Builtin::RudaCountY), &block, location);
        let ruda_count_xy_usize = block.muli(ruda_count_x_usize, ruda_count_y_usize, location)?;

        block.append_operation(scf::r#for(
            start,
            args.get(Builtin::RudaCountZ),
            step,
            {
                let region = Region::new();
                let block = Block::new(&[(integer_type, location)]);
                args.set(Builtin::RudaPosZ, block.argument(0)?.into());

                let absolute_pos_tmp_z = block.muli(
                    args.get(Builtin::RudaPosZ),
                    args.get(Builtin::RudaDimZ),
                    location,
                )?;
                let absolute_pos_z =
                ...
                let absolute_pos_z_corrected_usize =
                    block.muli(absolute_pos_z_usize, ruda_count_dim_xy_usize, location)?;

                let ruda_pos_z_usize =
                    args.as_address_type(args.get(Builtin::RudaPosZ), &block, location);
                let ruda_pos_z_corrected_usize =
                    block.muli(ruda_pos_z_usize, ruda_count_xy_usize, location)?;
                ...
                block.append_operation(scf::r#for(
                    ...
                    step,
                    {
                        let region = Region::new();
                        let block = Block::new(&[(integer_type, location)]);
                        args.set(Builtin::RudaPosY, block.argument(0)?.into());

                        let absolute_pos_tmp_y = block.muli(
                            args.get(Builtin::RudaPosY),
                            args.get(Builtin::RudaDimY),
                            location,
                        )?;
                        ...
                        let absolute_pos_y_corrected_usize =
                            block.muli(absolute_pos_y_usize, ruda_count_dim_x_usize, location)?;
                        ...
                        let ruda_pos_y_usize =
                            args.as_address_type(args.get(Builtin::RudaPosY), &block, location);
                        let ruda_count_x_usize =
                            args.as_address_type(args.get(Builtin::RudaCountX), &block, location);
                        let ruda_pos_y_corrected_usize =
                            block.muli(ruda_pos_y_usize, ruda_count_x_usize, location)?;
                        let ruda_pos_yz_corrected_usize = block.addi(
                            ruda_pos_z_corrected_usize,
                            ruda_pos_y_corrected_usize,
                            location,
                        )?;
                        ...
                        block.append_operation(scf::r#for(
                            ...
                            step,
                            {
                                let region = Region::new();
                                let block = Block::new(&[(integer_type, location)]);
                                args.set(Builtin::RudaPosX, block.argument(0)?.into());

                                let absolute_pos_tmp_x = block.muli(
                                    args.get(Builtin::RudaPosX),
                                    args.get(Builtin::RudaDimX),
                                    location,
                                )?;
                                ...
                                let ruda_pos_x_usize = args.as_address_type(
                                    args.get(Builtin::RudaPosX),
                                    &block,
                                    location,
                                );
                                let ruda_pos_usize = block.addi(
                                    ruda_pos_yz_corrected_usize,
                                    ruda_pos_x_usize,
                                    location,
                                )?;
                                args.set(Builtin::RudaPos, ruda_pos_usize);
