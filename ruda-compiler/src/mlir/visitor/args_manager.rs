...
use ruda_core::{
    ...
    kernel::{KernelDefinition, ScalarKernelArg},
};
...
use ruda_core::arguments::cpu::BuiltinArray;
use crate::mlir::shared_memories::{SharedMemories, SharedMemory};
...
impl<'a> ArgsManager<'a> {
    ...
    pub fn compute_derived_args_builtin(
        ...
        let ruda_dim_xy = block
            .muli(
                self.get(Builtin::RudaDimX),
                self.get(Builtin::RudaDimY),
                location,
            )
            .unwrap();
        let ruda_dim = block
            .muli(ruda_dim_xy, self.get(Builtin::RudaDimZ), location)
            .unwrap();
        self.set(Builtin::RudaDim, ruda_dim);

        let unit_pos_z_corrected = block
            .muli(self.get(Builtin::UnitPosZ), ruda_dim_xy, location)
            .unwrap();
