use ruda_core::ir::{AtomicOp, ElemType, InstructionModes, IntKind, UIntKind, Variable};
...
impl<T: SpirvTarget> SpirvCompiler<T> {
    ...
    fn scope(&mut self, var: &crate::spirv::variable::Variable) -> Word {
        let value = self.scope_of(var) as u32;
        self.const_u32(value)
    }

    fn semantics_r(&mut self, var: &crate::spirv::variable::Variable) -> Word {
        let value = self.semantics_of(var) | MemorySemantics::ACQUIRE;
        self.const_u32(value.bits())
    }

    fn semantics_w(&mut self, var: &crate::spirv::variable::Variable) -> Word {
        let value = self.semantics_of(var) | MemorySemantics::RELEASE;
        self.const_u32(value.bits())
    }

    fn semantics_rw(&mut self, var: &crate::spirv::variable::Variable) -> Word {
        let value = self.semantics_of(var) | MemorySemantics::ACQUIRE_RELEASE;
        self.const_u32(value.bits())
    }

    fn scope_of(&mut self, var: &crate::spirv::variable::Variable) -> Scope {
        let id = var.id(self);
        *self
            .state
            .atomic_scopes
    ...
    fn semantics_of(&mut self, var: &crate::spirv::variable::Variable) -> MemorySemantics {
