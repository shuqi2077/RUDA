...
use ruda_core::{
    compiler::{Compiler, CompilationError, WgpuCompilationOptions},
    launch::{RudaDim, ExecutionMode},
    arguments::{Info, Metadata},
    ir::{self as core, ElemType, InstructionModes, StorageType, UIntKind, features::EnumSet, FastMath},
    kernel::KernelDefinition,
};
use crate::optimizer::{BasicBlock, NodeIndex, Optimizer, SharedLiveness, Uniformity};
use rspirv::{
    binary::Assemble,
...
impl<T: SpirvTarget> SpirvCompiler<T> {
    pub fn new(lowering: SpirvLowering) -> Self {
        Self {
            lowering,
            ...
            debug_symbols: (lowering.debug_symbols)(),
...
impl<Target: SpirvTarget> SpirvCompiler<Target> {
    pub fn compile_kernel(&mut self, kernel: KernelDefinition) -> (Module, Optimizer, usize) {
        ...
        self.debug_symbols = (self.lowering.debug_symbols)() || options.debug_symbols;
        ...
        let mut opt = (self.lowering.optimize)(&kernel, self.mode, &self.compilation_options);
        ...
        let ruda_dims = vec![kernel.ruda_dim.x, kernel.ruda_dim.y, kernel.ruda_dim.z];
        ...
        target.set_modes(self, main, builtins, ruda_dims);
