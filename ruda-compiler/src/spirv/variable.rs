...
use ruda_core::ir::{self, ConstantValue, Id};
use rspirv::{
    dr::Builder,
...
impl ConstVal {
    ...
    pub fn as_float(&self, width: u32, encoding: Option<FPEncoding>) -> f64 {
        match (width, encoding) {
            ...
            (_, Some(FPEncoding::Float8E4M3EXT)) => {
                ruda_core::e4m3::from_bits(self.as_u32() as u8).to_f64()
            }
            (_, Some(FPEncoding::Float8E5M2EXT)) => {
                ruda_core::e5m2::from_bits(self.as_u32() as u8).to_f64()
            }
            _ => unreachable!(),
        }
    }
    ...
    pub fn from_float(value: f64, width: u32, encoding: Option<FPEncoding>) -> Self {
        match (width, encoding) {
            ...
            (_, Some(FPEncoding::Float8E4M3EXT)) => {
                ConstVal::Bit32(ruda_core::e4m3::from_f64(value).to_bits() as u32)
            }
            (_, Some(FPEncoding::Float8E5M2EXT)) => {
                ConstVal::Bit32(ruda_core::e5m2::from_f64(value).to_bits() as u32)
            }
            _ => unreachable!(),
        }
    }
