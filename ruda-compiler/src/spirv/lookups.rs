...
use crate::optimizer::{ConstArray, NodeIndex, SharedMemory};
...
impl<T: SpirvTarget> SpirvCompiler<T> {
    pub fn init_state(&mut self, kernel: KernelDefinition) {
        ...
        let ruda_dims = [kernel.ruda_dim.x, kernel.ruda_dim.y, kernel.ruda_dim.z];
        self.state.ruda_dims = ruda_dims.iter().map(|dim| self.const_u32(*dim)).collect();
        self.state.ruda_size = self.const_u32(ruda_dims.iter().product());
