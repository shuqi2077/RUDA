...
use ruda_core::ir::{self as core, RudaFnSource, Id, SourceLoc, Variable};
...
use crate::spirv::{SpirvCompiler, SpirvTarget};
...
struct Definitions {
    ...
    functions: HashMap<RudaFnSource, FunctionDefinition>,
}
...
impl<T: SpirvTarget> SpirvCompiler<T> {
    ...
    fn collect_sources(&mut self) {
        let ruda_fns = self.opt.root_scope.debug.sources.borrow().clone();
        let mut sources = HashMap::new();
        for ruda_fn in ruda_fns.iter() {
            ...
            if ruda_fn.source_text.is_empty() {
                sources
                    .entry(ruda_fn.file.clone())
                    .or_insert(ruda_fn.source_text.clone());
            } else {
                sources.insert(ruda_fn.file.clone(), ruda_fn.source_text.clone());
            }
        }
        ...
        for ruda_fn in ruda_fns.into_iter() {
            let source = self.definitions().source_files[ruda_fn.file.as_ref()];
            let name = ruda_fn.function_name.as_ref();
            ...
            self.definitions().functions.insert(ruda_fn, function);
        }
    }
