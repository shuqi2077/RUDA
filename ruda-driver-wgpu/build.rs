...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=RUDA_DEBUG_SPIRV");
    println!("cargo:rerun-if-env-changed=RUDA_VULKAN_VALIDATE");
    println!("cargo:rerun-if-env-changed=CARGO_FEATURE_STD");

    if env::var("RUDA_DEBUG_SPIRV").is_ok() && env::var("CARGO_FEATURE_STD").is_ok() {
        println!("cargo:rustc-cfg=feature=\"spirv-dump\"");
    }

    if env::var("RUDA_VULKAN_VALIDATE").is_ok() {
