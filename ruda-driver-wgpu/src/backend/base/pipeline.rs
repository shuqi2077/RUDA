        ...
        match repr {
            ...
            Some(AutoRepresentationRef::Msl(repr)) => unsafe {
                Ok(self.device.create_shader_module_passthrough(
                    wgpu::ShaderModuleDescriptorPassthrough {
                        ...
                        num_workgroups: (repr.ruda_dim.x, repr.ruda_dim.y, repr.ruda_dim.z),
                        ..Default::default()
                    },
                ))
            },
            _ => {
                let checks = wgpu::ShaderRuntimeChecks {
                    // Ruda does not need wgpu bounds checks - OOB behaviour is instead
                    // checked by ruda (if enabled).
                    // This is because the WebGPU specification only makes loose guarantees that Ruda can't rely on.
                    bounds_checks: false,
                ...
                log::trace!("[ruda-driver-wgpu] compiling WGSL module `{entrypoint_name}`\n{source}");

                let error_scope = self.device.push_error_scope(wgpu::ErrorFilter::Validation);

                // SAFETY: Ruda guarantees OOB safety when launching in checked mode. Launching in unchecked mode
                ...
                if let Some(err) = ruda_core::future::block_on(err_future) {
                    log::error!(
                        "[ruda-driver-wgpu] WGSL compilation failed for kernel `{entrypoint_name}`:\n{err}\n--- shader source ({} bytes) ---\n{source}\n--- end shader ---",
                        source.len()
                    );
    ...
    pub fn create_pipeline(
        ...
        let layout = bindings_info.map(|bindings| {
            ...
            if !cfg!(exclusive_memory_only) {
                bindings.fill(ruda::runtime::kernel::Visibility::ReadWrite);
            }
            ...
            let bindings = bindings
                ...
                .map(|visibility| BufferBindingType::Storage {
                    read_only: matches!(visibility, ruda::runtime::kernel::Visibility::Read),
                })
                .chain(info)
                .enumerate()
